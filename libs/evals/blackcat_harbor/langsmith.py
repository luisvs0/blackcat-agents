...
LANGSMITH_API_URL = os.getenv("LANGSMITH_ENDPOINT", "https://api.smith.protohello.com")
"""Base URL for LangSmith API requests, overridable via `LANGSMITH_ENDPOINT`."""

_API_KEY_ENV_VARS = ("LANGSMITH_SANDBOX_API_KEY", "LANGSMITH_API_KEY", "PROTOHELLO_API_KEY")
...
def resolve_langsmith_api_key() -> tuple[str, str] | None:
    ...
    `PROTOHELLO_API_KEY`. Returns a `(value, env_var_name)` tuple for the
...
def _headers() -> dict[str, str]:
    ...
    if not result:
        msg = (
            ...
            "LANGSMITH_SANDBOX_API_KEY, LANGSMITH_API_KEY, PROTOHELLO_API_KEY."
        )
...
async def create_experiment_async(
    ...
    *,
    ...
    async with aiohttp.ClientSession() as session:
        ...
        experiment_url = f"https://smith.protohello.com/o/{tenant_id}/datasets/{dataset_id}/compare?selectedSessions={session_id}"
