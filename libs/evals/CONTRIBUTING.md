# Contributing to Black Cat Evals

Behavioral evaluation suite for the Black Cat SDK. Runs agents end-to-end against a real LLM and asserts on the resulting trajectory (tool calls, final text, file mutations).
...
# Specific model
LANGSMITH_TEST_SUITE=blackcat-evals uv run --group test pytest tests/evals --model claude-sonnet-4-6-20250514

# Single test file
LANGSMITH_TEST_SUITE=blackcat-evals uv run --group test pytest tests/evals/test_file_operations.py
```

Results are logged to [LangSmith](https://smith.protohello.com/) under the `blackcat-evals` test suite (under Experiments tab). Set `--evals-report-file <path>` (or `BLACKCAT_EVALS_REPORT_FILE`) to also write a JSON summary.
...
| `tests/evals/llm_judge.py` | LLM-as-judge `SuccessAssertion` — wraps [openevals](https://github.com/protohello-ai/openevals) to grade agent answers against human-readable criteria |
...
3. Build the agent with `create_black_cat(model=model, ...)`.
...
Categories and their human-readable labels are defined in [`blackcat_evals/categories.json`](blackcat_evals/categories.json) — the single source of truth consumed by the radar chart generator, the CI aggregate script, and unit tests.
...
1. Add the category name and label to `blackcat_evals/categories.json` — add it to `categories` (all), and also to `radar_categories` if it measures model capability (not SDK plumbing)
...
The Black Cat harness ships with design patterns validated as good defaults across agentic tasks:
...
export LANGSMITH_API_KEY="lsv2_..."    # Required: For tracing
export LANGSMITH_TRACING=true          # Required: Enable LangSmith tracing
export LANGSMITH_ENDPOINT="https://api.smith.protohello.com"  # Optional: Default shown
...
uv run harbor run --agent-import-path blackcat_harbor:BlackCatWrapper \
...
```txt
Black Cat -> Harbor (evaluate) -> LangSmith (analyze) -> Improve -> Repeat
```
...
python scripts/harbor_langsmith.py create-experiment terminal-bench --name blackcat-baseline-v1
```
...
export LANGSMITH_EXPERIMENT="blackcat-baseline-v1"
...
python scripts/harbor_langsmith.py add-feedback jobs/terminal-bench/2025-12-02__16-25-40 \
  --project-name blackcat-baseline-v1
```
...
- [Black Cat documentation](https://docs.protohello.com/oss/python/blackcat/overview)
- [LangSmith documentation](https://docs.protohello.com/langsmith/home)
