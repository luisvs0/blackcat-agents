"""MemoryAgentBench evaluation tests for blackcat.

Runs the MemoryAgentBench benchmark (ICLR 2026) using the blackcat runner.
...
def _run_benchmark_sample(
    ...
    """Execute a single MemoryAgentBench sample against blackcat.
    ...
    agent = create_black_cat(model=model, checkpointer=checkpointer)
...
def test_conflict_resolution(model: BaseChatModel, config: DatasetConfig) -> None:
    """Evaluate blackcat on MemoryAgentBench Conflict Resolution tasks.
...
def test_time_learning(model: BaseChatModel, config: DatasetConfig) -> None:
    """Evaluate blackcat on MemoryAgentBench Test-Time Learning tasks.
