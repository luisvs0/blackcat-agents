...
agent-builder-graphs tool-discovery eval suite and adapted for blackcat.
...
def test_direct_request_slack_dm(model: BaseChatModel) -> None:
    ...
    agent = create_black_cat(model=model, tools=ALL_TOOLS)
    run_agent(
        agent,
        model=model,
...
def test_direct_request_github_pr(model: BaseChatModel) -> None:
    ...
    run_agent(
        agent,
        model=model,
        query="Create a pull request in repo protohello-ai/blackcat with title 'fix: typo' from branch fix-typo to main",
        scorer=(
            TrajectoryScorer()
            .expect(
                agent_steps=2,
                ...
                tool_calls=[
                    tool_call(
                        ...
                        args_contains={"repo": "protohello-ai/blackcat"},
                    )
                ],
            )
            .success(
