"""Primary graph assembly module for Black Cat.

Provides `create_black_cat`, the main entry point for constructing a fully
configured Black Cat with planning, filesystem, subagent, and summarization
middleware.
"""
...
from protohello.agents import AgentState, create_agent
from protohello.agents.middleware import HumanInTheLoopMiddleware, InterruptOnConfig, TodoListMiddleware
from protohello.agents.middleware.types import AgentMiddleware, ResponseT, _InputAgentState, _OutputAgentState
from protohello.agents.structured_output import ResponseFormat
from protohello_anthropic import ChatAnthropic
from protohello_anthropic.middleware import AnthropicPromptCachingMiddleware
from protohello_core.language_models import BaseChatModel
from protohello_core.messages import SystemMessage
from protohello_core.tools import BaseTool
...
from blackcat._harness_profiles import HarnessProfile, get_harness_profile
from blackcat._models import get_model_identifier, get_model_provider, resolve_model
from blackcat._version import __version__
from blackcat.backends import StateBackend
from blackcat.backends.protocol import BackendFactory, BackendProtocol
from blackcat.middleware._tool_exclusion import _ToolExclusionMiddleware
...
from blackcat.middleware.patch_tool_calls import PatchToolCallsMiddleware
from blackcat.middleware.permissions import FilesystemPermission, _PermissionMiddleware
from blackcat.middleware.skills import SkillsMiddleware
from blackcat.middleware.subagents import (
    GENERAL_PURPOSE_SUBAGENT,
    CompiledSubAgent,
    SubAgent,
    SubAgentMiddleware,
)
from blackcat.middleware.summarization import create_summarization_middleware

logger = logging.getLogger(__name__)

BASE_AGENT_PROMPT = """You are a Black Cat, an AI assistant that helps users accomplish tasks using tools. You respond with text and tool calls. The user can see your responses and tool outputs in real time.
...
"""Default base system prompt for every Black Cat.

When a caller passes `system_prompt` to `create_black_cat`, the custom prompt
...
def get_default_model() -> ChatAnthropic:
    """Get the default model for Black Cat.

    Used as a fallback when `model=None` is passed to `create_black_cat`.
...
def _tool_name(tool: BaseTool | Callable | dict[str, Any]) -> str | None:
    ...
    Args:
        tool: A tool in any of the forms accepted by `create_black_cat`.
...
def create_black_cat(  # noqa: C901, PLR0912, PLR0915  # Complex graph assembly logic with many conditional branches
    ...
    *,
    ...
    !!! warning "Black Cat require a LLM that supports tool calling!"
    ...
    Args:
        model: The model to use.
            ...
            [`init_chat_model`][protohello.chat_models.init_chat_model(model_provider)]
            for supported values. You can also pass a pre-initialized
            [`BaseChatModel`][protohello.chat_models.BaseChatModel] instance directly.
        ...
        subagents: Subagent specs available to the main agent.
            ...
            - [`SubAgent`][blackcat.middleware.subagents.SubAgent]: A declarative synchronous subagent spec.
            - [`CompiledSubAgent`][blackcat.middleware.subagents.CompiledSubAgent]: A pre-compiled runnable subagent.
            - [`AsyncSubAgent`][blackcat.middleware.async_subagents.AsyncSubAgent]: A remote/background subagent spec.
        ...
        context_schema: Schema class that defines immutable run-scoped context.
            ...
            Passed through to [`create_agent`][protohello.agents.create_agent].
    ...
    Returns:
        A configured Black Cat.

    Raises:
        ImportError: If a required provider package is missing or below the
            minimum supported version (e.g., `protohello-openrouter`).
    """
    ...
    blackcat_middleware: list[AgentMiddleware[Any, Any, Any]] = [
        TodoListMiddleware(),
    ]
    if skills is not None:
        blackcat_middleware.append(SkillsMiddleware(backend=backend, sources=skills))
    blackcat_middleware.extend(
        [
            FilesystemMiddleware(
                backend=backend,
                custom_tool_descriptions=_profile.tool_description_overrides,
    ...
    if async_subagents:
        ...
        blackcat_middleware.append(AsyncSubAgentMiddleware(async_subagents=async_subagents))

    if middleware:
        blackcat_middleware.extend(middleware)
    ...
    blackcat_middleware.extend(_resolve_extra_middleware(_profile))
    if _profile.excluded_tools:
        blackcat_middleware.append(_ToolExclusionMiddleware(excluded=_profile.excluded_tools))
    # Unconditional prompt caching (see general-purpose subagent comment).
    blackcat_middleware.append(AnthropicPromptCachingMiddleware(unsupported_model_behavior="ignore"))
    if memory is not None:
        blackcat_middleware.append(MemoryMiddleware(backend=backend, sources=memory))
    if interrupt_on is not None:
        blackcat_middleware.append(HumanInTheLoopMiddleware(interrupt_on=interrupt_on))
    # _PermissionMiddleware must be last so it sees all tools from prior middleware
    if permissions:
        blackcat_middleware.append(_PermissionMiddleware(rules=permissions, backend=backend))
    ...
    return create_agent(
        ...
        tools=_tools,
        middleware=blackcat_middleware,
    ...
    ).with_config(
        {
            ...
            "metadata": {
                "ls_integration": "blackcat",
                "versions": {"blackcat": __version__},
                "lc_agent_name": name,
            },
        }
    )
