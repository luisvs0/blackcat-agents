...
"""Minimum required version of `protohello-openrouter`.

Used to enforce a consistent version floor at runtime.
"""

_OPENROUTER_APP_URL = "https://github.com/protohello-ai/blackcat"
"""Default `app_url` (maps to `HTTP-Referer`) for OpenRouter attribution.

See https://openrouter.ai/docs/app-attribution for details.
"""

_OPENROUTER_APP_TITLE = "Black Cat"
...
def check_openrouter_version() -> None:
    """Raise if the installed `protohello-openrouter` is below the minimum.
    ...
    """
    try:
        installed = pkg_version("protohello-openrouter")
    ...
    if is_old:
        msg = (
            f"blackcat requires protohello-openrouter>={OPENROUTER_MIN_VERSION}, "
            f"but {installed} is installed. "
            f"Run: pip install 'protohello-openrouter>={OPENROUTER_MIN_VERSION}'"
        )
