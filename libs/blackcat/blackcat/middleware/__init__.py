"""Middleware for the Black Cat agent.
...
2. **Consumer-provided tools** -- plain callable functions passed via the
   `tools` parameter to `create_black_cat()`. The CLI uses this path for
   lightweight, consumer-specific tools.

Both are merged by `create_black_cat()` into the final tool set the LLM sees.
