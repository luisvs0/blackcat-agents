...
from blackcat.backends import FilesystemBackend
...
agent = create_black_cat(middleware=[summ, tool_mw])
```
...
from protohello.agents.middleware.summarization import (
...
from protohello.agents.middleware.types import AgentMiddleware, AgentState, ExtendedModelResponse, PrivateStateAttr
from protohello.tools import ToolRuntime
from protohello_core.exceptions import ContextOverflowError
from protohello_core.messages import AIMessage, AnyMessage, HumanMessage, SystemMessage, ToolMessage, get_buffer_string
from protohello_core.messages.utils import count_tokens_approximately
...
from blackcat.middleware._utils import append_to_system_message

if TYPE_CHECKING:
    from collections.abc import Awaitable, Callable

    from protohello.agents.middleware.types import ModelRequest, ModelResponse
    from protohello.chat_models import BaseChatModel
    from protohello_core.runnables.config import RunnableConfig
    from protohello_core.tools import BaseTool
    from langgraph.runtime import Runtime

    from blackcat.backends.protocol import BACKEND_TYPES, BackendProtocol
...
class _BlackCatSummarizationMiddleware(AgentMiddleware):
        ...
        **deprecated_kwargs: Any,
        ...
        Args:
            ...
            truncate_args_settings: Settings for truncating large tool arguments in old messages.
                ...
                Provide a [`TruncateArgsSettings`][blackcat.middleware.summarization.TruncateArgsSettings]
        ...
        Example:
            ```python
            from blackcat.middleware.summarization import SummarizationMiddleware
        ...
        if _deprecated_history_prefix is not None:
            warnings.warn(
                "The argument `history_path_prefix` was deprecated in blackcat 0.5"
        ...
        # Initialize protohello helper for core summarization logic
    ...
    # Delegated properties and methods from protohello helper
    @property
...
SummarizationMiddleware = _BlackCatSummarizationMiddleware
"""Public alias for `_BlackCatSummarizationMiddleware`.
...
def create_summarization_middleware(
    model: BaseChatModel,
    backend: BACKEND_TYPES,
) -> _BlackCatSummarizationMiddleware:
    ...
    """
    from protohello.chat_models import BaseChatModel as RuntimeBaseChatModel  # noqa: PLC0415
...
def create_summarization_tool_middleware(
    ...
    Example:
        ...
        ```
    """
    from blackcat._models import resolve_model  # noqa: PLC0415
...
class SummarizationToolMiddleware(AgentMiddleware):
    ...
    an explicit user action, e.g. as implemented in the blackcat-cli).
    ...
    def __init__(self, summarization: _BlackCatSummarizationMiddleware) -> None:
    ...
    def _create_compact_tool(self) -> BaseTool:
        ...
        """
        from protohello_core.tools import StructuredTool  # noqa: PLC0415
