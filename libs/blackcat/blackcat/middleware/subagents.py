...
from protohello.agents.middleware import HumanInTheLoopMiddleware, InterruptOnConfig
from protohello.agents.middleware.types import AgentMiddleware, ContextT, ModelRequest, ModelResponse, ResponseT
from protohello.tools import BaseTool, ToolRuntime
...
from protohello_core.tools import StructuredTool
...
class SubAgent(TypedDict):
    ...
    When using `create_black_cat`, subagents automatically receive a default middleware
...
class CompiledSubAgent(TypedDict):
    ...
    1. ProtoHello's [`create_agent()`](https://docs.protohello.com/oss/python/protohello/quickstart)
    2. A custom graph using [`langgraph`](https://docs.protohello.com/oss/python/langgraph/quickstart)
...
class SubAgentMiddleware(AgentMiddleware[Any, ContextT, ResponseT]):
    ...
    Example:
        ```python
        from blackcat.middleware import SubAgentMiddleware
        from protohello.agents import create_agent
