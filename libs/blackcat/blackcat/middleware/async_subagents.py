...
[Agent Protocol](https://github.com/protohello-ai/agent-protocol) servers.
...
from protohello.agents.middleware.types import AgentMiddleware, AgentState, ContextT, ModelRequest, ModelResponse, ResponseT
...
class AsyncSubAgent(TypedDict):
    ...
    `PROTOHELLO_API_KEY`). For self-hosted servers, pass custom auth via
    `headers`.
    """
...
class AsyncSubAgentMiddleware(AgentMiddleware[Any, ContextT, ResponseT]):
    ...
    Example:
        ```python
        from blackcat.middleware.async_subagents import AsyncSubAgentMiddleware
