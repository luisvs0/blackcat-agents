...
from blackcat.backends import CompositeBackend, StateBackend, StoreBackend
from blackcat.backends.protocol import ExecuteResponse, SandboxBackendProtocol
from blackcat.graph import create_black_cat
from blackcat.middleware.filesystem import (
    FileData,
...
@pytest.mark.requires("protohello_anthropic")
class TestFilesystem:
    ...
    def test_longterm_memory_multiple_tools_blackcat(self):
        checkpointer = MemorySaver()
        store = InMemoryStore()
        ...
        agent = create_black_cat(backend=backend, checkpointer=checkpointer, store=store)
        assert_longterm_mem_tools(agent, store)

    def test_shortterm_memory_multiple_tools_blackcat(self):
        checkpointer = MemorySaver()
        store = InMemoryStore()
        agent = create_black_cat(backend=StateBackend(), checkpointer=checkpointer, store=store)
