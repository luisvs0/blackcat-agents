...
from protohello_core.runnables.config import var_child_runnable_config
...
def test_create_black_cat_with_memory_and_filesystem_backend(tmp_path: Path) -> None:
    """Test end-to-end: create_black_cat with memory parameter and FilesystemBackend."""
    # Create memory on filesystem
    backend = FilesystemBackend(root_dir=str(tmp_path), virtual_mode=False)
    memory_path = str(tmp_path / "user" / "AGENTS.md")
    memory_content = make_memory_content("Black Cat Test", "- Use deep agents wisely")
...
def test_create_black_cat_with_memory_missing_files(tmp_path: Path) -> None:
...
def test_create_black_cat_with_memory_default_backend() -> None:
    """Test create_black_cat with memory parameter using default backend (StateBackend).
    ...
    agent = create_black_cat(
        memory=["/user/.blackcat/AGENTS.md"],
    ...
    assert "/user/.blackcat/AGENTS.md" in checkpoint["channel_values"]["files"]
    assert "memory_contents" in checkpoint["channel_values"]
    assert "/user/.blackcat/AGENTS.md" in checkpoint["channel_values"]["memory_contents"]
