"""Unit tests for blackcat.graph module."""
...
from protohello_core.messages import AIMessage, SystemMessage
from protohello_core.tools import BaseTool, StructuredTool

from blackcat._harness_profiles import _HARNESS_PROFILES, HarnessProfile, get_harness_profile, register_harness_profile
from blackcat._version import __version__
from blackcat.graph import (
    BASE_AGENT_PROMPT,
...
if TYPE_CHECKING:
    ...
    from protohello.agents.middleware.types import ModelRequest
...
class TestCreateBlackCatMetadata:
    ...
    def test_ls_integration_metadata_preserved(self) -> None:
        ...
        agent = create_black_cat(model=model)
        assert agent.config is not None
        assert agent.config["metadata"]["ls_integration"] == "blackcat"
...
class TestToolDescriptionOverrides:
    ...
    These test the helper directly rather than going through `create_black_cat`
...
class TestToolDescriptionOverrideWiring:
    ...
    def test_create_black_cat_passes_overrides_to_filesystem_and_task(self) -> None:
        original = dict(_HARNESS_PROFILES)
        try:
            ...
            with (
                patch("blackcat.graph.resolve_model", return_value=fake_model),
                patch("blackcat.graph.FilesystemMiddleware", side_effect=[MagicMock(), MagicMock()]) as mock_fs,
                patch("blackcat.graph.SubAgentMiddleware", return_value=MagicMock()) as mock_subagents,
                patch("blackcat.graph.TodoListMiddleware", return_value=MagicMock()),
                patch("blackcat.graph.PatchToolCallsMiddleware", return_value=MagicMock()),
                patch("blackcat.graph.create_summarization_middleware", return_value=MagicMock()),
                patch("blackcat.graph.create_agent", return_value=fake_agent),
            ):
                result = create_black_cat(model="testprov:some-model")
...
class TestSystemPromptAssembly:
    ...
    def _build_and_capture_system_prompt(self, profile_key: str, profile: HarnessProfile, **kwargs: Any) -> str | SystemMessage:
        """Register a profile, call create_black_cat, return the system_prompt passed to create_agent."""
        original = dict(_HARNESS_PROFILES)
        try:
            ...
            with (
                patch("blackcat.graph.resolve_model", return_value=fake_model),
                patch("blackcat.graph.FilesystemMiddleware", side_effect=[MagicMock(), MagicMock()]),
                patch("blackcat.graph.SubAgentMiddleware", return_value=MagicMock()),
                patch("blackcat.graph.TodoListMiddleware", return_value=MagicMock()),
                patch("blackcat.graph.PatchToolCallsMiddleware", return_value=MagicMock()),
                patch("blackcat.graph.create_summarization_middleware", return_value=MagicMock()),
                patch("blackcat.graph.create_agent", return_value=fake_agent) as mock_create,
            ):
                create_black_cat(model=f"{profile_key}:some-model", **kwargs)
...
class TestToolExclusionWiring:
    """Tests that excluded_tools on a profile wires _ToolExclusionMiddleware into create_black_cat."""
    ...
    def test_exclusion_middleware_added_when_profile_has_excluded_tools(self) -> None:
        ...
        try:
            ...
            with (
                ...
                result = create_black_cat(model="exclprov:some-model")
    ...
    def test_no_exclusion_middleware_when_no_excluded_tools(self) -> None:
        ...
        try:
            ...
            with (
                ...
                create_black_cat(model="noxprov:some-model")
