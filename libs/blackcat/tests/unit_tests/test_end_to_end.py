"""End-to-end unit tests for blackcat with fake LLM models."""
...
import pytest
from protohello.agents.middleware import AgentMiddleware
from protohello.agents.middleware.types import ModelRequest, ModelResponse
from protohello.tools import ToolRuntime
from protohello_core.language_models import LanguageModelInput
from protohello_core.language_models.fake_chat_models import GenericFakeChatModel
from protohello_core.messages import AIMessage, HumanMessage, SystemMessage
from protohello_core.runnables import Runnable
from protohello_core.tools import BaseTool, tool
from langgraph.checkpoint.memory import InMemorySaver
from langgraph.store.memory import InMemoryStore

from blackcat.backends import CompositeBackend, FilesystemBackend
from blackcat.backends.protocol import BackendProtocol, ExecuteResponse, SandboxBackendProtocol
from blackcat.backends.state import StateBackend
from blackcat.backends.store import StoreBackend
from blackcat.backends.utils import TOOL_RESULT_TOKEN_LIMIT, create_file_data
from blackcat.graph import create_black_cat
from blackcat.middleware.filesystem import NUM_CHARS_PER_TOKEN
from blackcat.middleware.permissions import FilesystemPermission
from blackcat.middleware.subagents import SubAgent  # noqa: TC001
from blackcat.middleware.summarization import create_summarization_tool_middleware
from tests.unit_tests.chat_model import GenericFakeChatModel as FakeChatModelWithHistory
from tests.utils import SampleMiddlewareWithTools, SampleMiddlewareWithToolsAndState, assert_all_blackcat_qualities
...
class TestBlackCatEndToEnd:
    """Test suite for end-to-end blackcat functionality with fake LLM."""

    def test_black_cat_with_fake_llm_basic(self) -> None:
        """Test basic blackcat functionality with a fake LLM model.

        This test verifies that a blackcat can be created and invoked with
        ...
        agent = create_black_cat(model=model)
    ...
    def test_main_agent_streaming_metadata_includes_tags_and_config_metadata(self) -> None:
        ...
        assert first_metadata.get("ls_integration") == "protohello_chat_model"
    ...
    def test_tool_runtime_config_includes_default_graph_metadata(self) -> None:
        """Test tool runtime config includes the defaults from `create_black_cat`."""
        ...
        assert captured_config["metadata"]["ls_integration"] == "blackcat"
        assert captured_config["metadata"]["lc_agent_name"] == "supervisor"
        assert "blackcat" in captured_config["metadata"]["versions"]

    def test_black_cat_with_fake_llm_with_tools(self) -> None:
        """Test blackcat with tools using a fake LLM model.

        This test verifies that a blackcat can handle tool calls correctly
        ...
        agent = create_black_cat(model=model, tools=[sample_tool])
    ...
    def test_black_cat_with_fake_llm_filesystem_tool(self) -> None:
        """Test blackcat with filesystem tools using a fake LLM model.

        This test verifies that a blackcat can use the built-in filesystem
    ...
    def test_black_cat_with_fake_llm_multiple_tool_calls(self) -> None:
        """Test blackcat with multiple tool calls using a fake LLM model.

        This test verifies that a blackcat can handle multiple sequential
    ...
    def test_black_cat_with_string_model_name(self) -> None:
        """Test that create_black_cat resolves string model names correctly.
        ...
        with patch("blackcat.graph.resolve_model", return_value=fake_model):
            agent = create_black_cat(model="claude-sonnet-4-6", tools=[sample_tool])
    ...
    def test_black_cat_truncate_lines(self, tmp_path: Path, backend: BackendProtocol) -> None:
        ...
        agent = create_black_cat(model=model, backend=backend)
    ...
    def test_black_cat_read_empty_file(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_black_cat_with_system_message(self) -> None:
        """Test that create_black_cat accepts a SystemMessage for system_prompt."""
        model = FixedGenericFakeChatModel(
            messages=iter(
                [
        ...
        agent = create_black_cat(model=model, system_prompt=system_msg, middleware=[capturing_middleware])
        assert_all_blackcat_qualities(agent)
        ...
        assert "You are a Black Cat" in content

    def test_black_cat_with_system_message_string_content(self) -> None:
        """Test that create_black_cat accepts a SystemMessage with string content."""
        model = FixedGenericFakeChatModel(
            messages=iter(
                [
    ...
    def test_black_cat_two_turns_no_initial_files(self) -> None:
        """Test blackcat with two conversation turns without specifying files on invoke.
    ...
    def test_black_cat_two_turns_state_backend_edge_case(self) -> None:
    ...
    async def test_black_cat_two_turns_no_initial_files_async(self) -> None:
        """Async version: Test blackcat with two conversation turns without specifying files.
    ...
    async def test_black_cat_two_turns_state_backend_edge_case_async(self) -> None:
    ...
    def test_black_cat_read_file_truncation(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_black_cat_read_file_no_truncation_small_file(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_black_cat_read_file_truncation_with_offset(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    async def test_black_cat_read_file_truncation_async(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_black_cat_read_file_single_long_line_behavior(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_read_large_single_line_file_returns_reasonable_size(self) -> None:
        ...
        agent = create_black_cat(model=fake_model)
    ...
    def test_black_cat_read_file_invalid_args_returns_tool_message(self, tmp_path: Path, backend: BackendProtocol) -> None:
        ...
        agent = create_black_cat(model=fake_model, backend=backend)
    ...
    def test_black_cat_read_image_file(self, tmp_path: Path, backend: BackendProtocol) -> None:
...
class TestBlackCatPermissionsEndToEnd:
    """End-to-end tests for create_black_cat with FilesystemPermission."""
...
class TestLargeHumanMessageEviction:
    ...
    def test_multi_turn_eviction(self) -> None:
        ...
        agent = create_black_cat(model=fake_model, checkpointer=InMemorySaver())
...
class TestSummarizationOffloadToState:
    ...
    def test_offloaded_file_persisted_in_state(self) -> None:
        ...
        Uses ``create_black_cat`` with default ``StateBackend`` so that
...
class TestArtifactsRoot:
    ...
    def test_black_cat_artifacts_root_system_prompt_and_eviction(self) -> None:
    ...
    def test_black_cat_artifacts_root_eviction_then_read(self) -> None:
    ...
    def test_black_cat_artifacts_root_conversation_history_offload(self) -> None:
    ...
    def test_create_black_cat_no_composite_backend(self) -> None:
        """create_black_cat with a non-composite backend defaults artifacts_root to '/'."""
        backend = StateBackend()
        capturing_middleware = SystemMessageCapturingMiddleware()
        agent = create_black_cat(
    ...
    def test_create_black_cat_composite_backend_default_artifacts_root(self) -> None:
        """create_black_cat with CompositeBackend without artifacts_root defaults to '/'."""
    ...
    def test_human_message_eviction_uses_artifacts_root(self) -> None:
        ...
        agent = create_black_cat(model=model, backend=backend, checkpointer=InMemorySaver())
...
class TestAsyncSubagentEndToEnd:
    ...
    These tests wire up ``create_black_cat`` with ``AsyncSubAgent`` specs and
    ...
    @patch("blackcat.middleware.async_subagents.get_sync_client")
