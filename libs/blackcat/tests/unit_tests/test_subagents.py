...
import pytest
from protohello.agents import create_agent
from protohello.agents.middleware import TodoListMiddleware
from protohello.agents.structured_output import ToolStrategy
from protohello.tools import ToolRuntime
from protohello_core.callbacks import BaseCallbackHandler
from protohello_core.messages import AIMessage, HumanMessage
from protohello_core.runnables import RunnableConfig, RunnableLambda
from protohello_core.tools import tool
...
class TestSubAgents:
    ...
    def test_create_black_cat_routes_async_subagents_from_subagents_param(self) -> None:
    ...
    def test_create_black_cat_keeps_sync_subagents_in_task_middleware(self) -> None:
        agent = create_black_cat(
            model=GenericFakeChatModel(messages=iter([AIMessage(content="done")])),
            subagents=[
                {
    ...
    def test_subagent_propagates_callbacks_to_model_calls(self) -> None:
        ...
        Regression test for https://github.com/protohello-ai/blackcat/issues/2315.
        """
    ...
    def test_custom_subagent_does_not_inherit_skills(self, tmp_path: Path) -> None:
        """Test that custom subagents do NOT inherit skills middleware from create_black_cat.

        This test verifies that:
        1. When create_black_cat is called with skills, only the general-purpose subagent gets SkillsMiddleware
    ...
    def test_general_purpose_subagent_inherits_skills_from_main_agent(self, tmp_path: Path) -> None:
        ...
        This test verifies that:
        1. When create_black_cat is called with skills, the general-purpose subagent gets SkillsMiddleware
