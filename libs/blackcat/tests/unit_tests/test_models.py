"""Tests for blackcat._models helpers."""
...
from blackcat._harness_profiles import (
    _HARNESS_PROFILES,
    HarnessProfile,
    _merge_profiles,
    get_harness_profile,
...
class TestResolveModel:
    ...
    def test_openai_prefix_uses_responses_api(self) -> None:
        with patch("blackcat._models.init_chat_model") as mock:
    ...
    def test_openrouter_env_var_overrides_app_url(self) -> None:
        ...
        with (
            patch("blackcat._models.init_chat_model") as mock,
...
class TestCheckOpenRouterVersion:
    ...
    def test_passes_when_not_installed(self) -> None:
        with patch(
            ...
            side_effect=PackageNotFoundError("protohello-openrouter"),
        ):
    ...
    def test_passes_when_version_above_minimum(self) -> None:
        with patch("blackcat._openrouter.pkg_version", return_value="99.0.0"):
            check_openrouter_version()  # should not raise

    def test_raises_when_version_too_old(self) -> None:
        with (
            patch("blackcat._openrouter.pkg_version", return_value="0.0.1"),
            pytest.raises(ImportError, match="protohello-openrouter>="),
        ):
            check_openrouter_version()

    def test_skips_check_for_invalid_version(self) -> None:
        with patch("blackcat._openrouter.pkg_version", return_value="not-a-version"):
            check_openrouter_version()  # should not raise

    def test_resolve_model_calls_check(self) -> None:
        with (
            patch("blackcat._harness_profiles.check_openrouter_version") as mock_check,
            patch("blackcat._models.init_chat_model") as mock_init,
        ):
