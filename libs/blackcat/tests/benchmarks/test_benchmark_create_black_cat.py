"""Wall-time benchmarks for `create_black_cat` graph construction.
...
`create_black_cat` under various configurations. They do NOT invoke the
...
if TYPE_CHECKING:
    from blackcat.middleware.subagents import SubAgent
...
def _build_kwargs(**overrides: Any) -> dict[str, Any]:
    """Build default kwargs for `create_black_cat`, merged with overrides."""
...
class TestCreateBlackCatBenchmark:
    """Wall-time benchmarks for `create_black_cat` graph construction."""
    ...
    def test_create_black_cat_minimal(self, benchmark: BenchmarkFixture) -> None:
        """Baseline: no user-supplied tools, subagents, or middleware."""
        kwargs = _build_kwargs()

        @benchmark  # type: ignore[misc]
        def _() -> None:
            create_black_cat(**kwargs)
    ...
    def test_with_string_model_resolution(self, benchmark: BenchmarkFixture) -> None:
        ...
        fake = _fake_model()
        with patch("blackcat.graph.resolve_model", return_value=fake):
...
@pytest.mark.benchmark
class TestCreateBlackCatScaling:
