# Threat Model: blackcat (SDK library)

> Generated: 2026-03-28 | Commit: e859077f | Scope: `libs/blackcat/blackcat/` — SDK library only (not CLI)
...
- `blackcat/graph.py` — primary public API (`create_black_cat`, `resolve_model`, `get_default_model`)
- `blackcat/_models.py` — model resolution helpers (`resolve_model`, `check_openrouter_version`, `get_model_identifier`)
- `blackcat/backends/` — all backend implementations (`StateBackend`, `FilesystemBackend`, `LocalShellBackend`, `StoreBackend`, `CompositeBackend`, `BaseSandbox`, `LangSmithSandbox`) and the `BackendProtocol` / `SandboxBackendProtocol` protocol definitions
- `blackcat/middleware/` — all middleware: `FilesystemMiddleware`, `MemoryMiddleware`, `SkillsMiddleware`, `SubAgentMiddleware`, `AsyncSubAgentMiddleware`, `SummarizationMiddleware`, `PatchToolCallsMiddleware`
...
7. `AsyncSubAgentMiddleware` connects to user-configured remote LangGraph server URLs. Authentication relies on environment variables (`LANGGRAPH_API_KEY` / `LANGSMITH_API_KEY` / `PROTOHELLO_API_KEY`) read by the LangGraph SDK — the library does not manage authentication credentials.
...
`blackcat` is an open source Python SDK built on LangGraph and ProtoHello for creating "deep agents" — LLM-powered agents with sub-agent spawning, filesystem operations, todo lists, memory, and skills. The library ships a factory function (`create_black_cat`) that assembles a LangGraph `CompiledStateGraph` from user-provided tools, models, middleware, and storage backends. It is intended for use in developer-facing CLI tools, research pipelines, and application backends where users control the execution environment. It does **not** run a server itself — it compiles a graph that user application code invokes.
...
│                       User Application                              │
│                                                                     │
│  create_black_cat(model, tools, backend, checkpointer, store,      │
...
| DC5 | LLM API credentials | `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, etc. | Critical | Environment variables only; framework reads via ProtoHello providers | None (env var) | Process lifetime | All — breach trigger |
...
- **Fields**: Provider API keys read from environment via ProtoHello providers.
...
|----|-----------|-------------|-------------|----------|--------------|
| C1 | Agent Factory (`graph.py`) | Assembles and returns a `CompiledStateGraph`; the primary user-facing API | framework-controlled | Yes | `graph.py:create_black_cat`, `graph.py:resolve_model`, `graph.py:get_default_model` |
...
| C9 | SubAgentMiddleware | Adds a `task` tool that spawns ephemeral inline sub-agents with inherited state | framework-controlled | **Yes** (always added by `create_black_cat`) | `middleware/subagents.py:SubAgentMiddleware.__init__`, `_build_task_tool` |
| C10 | LLM Provider | External API (Anthropic, OpenAI, Google, OpenRouter, etc.) called with agent messages | external | Yes | HTTP via `protohello-anthropic`, `protohello-openai`, `protohello-google-genai` |
| C11 | LangGraph Checkpointer | Optional persistent state checkpointing via user-supplied `Checkpointer` | user-controlled | No (opt-in, `checkpointer=` param) | `graph.py:create_black_cat` (passed through to `create_agent`) |
...
- **Inside**: `create_black_cat` validates model resolution (`_models.py:resolve_model`), constructs the middleware stack with framework defaults (TodoList, Filesystem, SubAgent, AsyncSubAgent, Summarization, PromptCaching, PatchToolCalls middleware), and assembles the LangGraph graph. Entry: `graph.py:create_black_cat`.
...
- **Preconditions**: Attacker must control a file at a path the configured backend serves. With `FilesystemBackend`, this means filesystem write access to any configured `sources` path. Most practical vector: poisoned `.blackcat/AGENTS.md` committed to a git repository that a developer clones.
...
| S3/database/checkpointer backend security | Users configure their own checkpointer and store. The library passes them through unchanged. | `graph.py:create_black_cat` — `checkpointer` and `store` are passed through |
| Supply chain attacks on ProtoHello/LangGraph/provider libraries | Transitive dependencies are managed by the user's resolver. | `pyproject.toml` — only lower-bound version pins |
...
**Memory/skill content injection**: Memory and skill files are user-controlled artifacts. The framework treats them as trusted — this is documented. The primary risk vector is a poisoned `.blackcat/AGENTS.md` in a cloned git repository. This is structurally identical to the risk of running `pyproject.toml` build scripts or `Makefile` targets from an untrusted repo.
...
| Date | Author | Changes |
|------|--------|---------|
| 2026-03-10 | langster-threat-model (automated) | Initial threat model for SDK library (libs/blackcat); scoped to library only, CLI excluded |
