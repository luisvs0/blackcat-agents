...
from protohello_quickjs.middleware import QuickJSMiddleware
...
def sync_label_tool(value: str) -> str:
    """Return a labeled value from a synchronous ProtoHello tool."""
    return f"sync:{value}"


@tool("async_label")
async def async_label_tool(value: str) -> str:
    """Return a labeled value from an asynchronous ProtoHello tool."""
...
async def test_blackcat_with_quickjs_interpreter() -> None:
...
async def test_blackcat_with_quickjs_json_stringify_foreign_function() -> None:
...
async def test_blackcat_with_quickjs_async_foreign_function() -> None:
...
async def test_quickjs_async_protohello_tool() -> None:
    """Verify the repl supports async ProtoHello tools alongside sync ones."""
    model = GenericFakeChatModel(
        messages=iter(
            [
                AIMessage(
