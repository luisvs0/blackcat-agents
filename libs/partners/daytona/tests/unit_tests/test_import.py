...
import protohello_daytona
from protohello_daytona.sandbox import DaytonaSandbox
...
def test_import_daytona() -> None:
    assert protohello_daytona is not None
...
def test_execute_polls_with_fixed_interval() -> None:
    ...
    with patch("protohello_daytona.sandbox.time.sleep") as mock_sleep:
...
def test_execute_polls_with_callable_interval() -> None:
    ...
    with (
        patch("protohello_daytona.sandbox.time.sleep") as mock_sleep,
        patch(
            "protohello_daytona.sandbox.time.monotonic",
...
def test_execute_timeout() -> None:
    ...
    with (
        patch("protohello_daytona.sandbox.time.sleep"),
        patch(
            "protohello_daytona.sandbox.time.monotonic",
