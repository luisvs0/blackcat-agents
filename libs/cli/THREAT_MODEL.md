...
- `blackcat_cli/` — all Python source modules shipped as the `blackcat-cli` package
...
- `libs/blackcat/` (SDK library) — separate package with its own threat model
...
1. The CLI runs locally on the user's machine; the user is a developer who invoked `blackcat` themselves.
...
3. `~/.blackcat/` is only writable by the authenticated local user — no multi-user shared home directories.
...
| C2  | TUI / Non-interactive       | Textual UI for interactive chat; `non_interactive.py` for headless pipelines (both use `RemoteAgent`)               | framework-controlled | Yes      | `app.BlackCatApp.run`, `non_interactive.run_non_interactive`                                    |
...
| C9  | Configuration System        | TOML config, env vars, `AGENTS.md` system prompts, model config                                                    | user-controlled      | N/A      | `config.settings`, `model_config.ModelConfig`, `~/.blackcat/config.toml`                        |
...
| C16 | Custom Subagent Loader      | Reads `{dir}/{name}/AGENTS.md` YAML frontmatter from `.blackcat/agents/` and project `.agents/` directories      | user-controlled      | No       | `subagents.list_subagents`, `subagents._parse_subagent_file`                                      |
...
3. Hooks only execute if `~/.blackcat/hooks.json` exists.
...
| DC2 | Conversation Messages  | User prompts, LLM responses, tool args/results   | High        | SQLite (`~/.blackcat/*.db`) via LangGraph checkpointer | No (local file, unencrypted) | Unbounded (session files persist) | GDPR if personal data is discussed |
| DC3 | System Prompt Content  | `DA_SERVER_SYSTEM_PROMPT` env var; custom AGENTS.md contents | Medium | Process environment (transient); `~/.blackcat/{agent}/AGENTS.md` on disk | No | Config lifetime | None direct |
| DC4 | MCP Trust Fingerprints | `mcp_trust.projects.*` in `config.toml`          | Low         | `~/.blackcat/config.toml`      | No                | Until revoked      | None       |
...
- **Storage**: SQLite at `~/.blackcat/{agent}/{thread_id}.db` (via `sessions.get_db_path`).
...
- **Inside**: `app.BlackCatApp` routes keystrokes to message queue; `non_interactive.run_non_interactive` reads from stdin/argv. Both paths now go through `RemoteAgent.astream` to the server. Session ID assigned per thread (`sessions.generate_thread_id`).
...
- **Crossing mechanism**: `subprocess.Popen` (via `protohello_mcp_adapters`) for stdio; HTTP/SSE for remote servers.
...
| DF5  | C3 Agent     | External LLM | System prompt + message history               | DC1, DC2       | TB7              | HTTPS / ProtoHello      |
| DF6  | External LLM | C3 Agent     | AI response + tool call decisions             | —              | TB7              | HTTPS / ProtoHello      |
...
- **Trust assumption**: User controls `~/.blackcat/config.toml` (same trust model as `pyproject.toml` build scripts).
...
- **Description**: When `--shell-allow-list all` (or `BLACKCAT_SHELL_ALLOW_LIST=all`) is set, `is_shell_command_allowed` returns `True` for any non-empty command without invoking `contains_dangerous_patterns`. In non-interactive mode, any shell command the LLM requests executes unconditionally. Combined with T1, an attacker-controlled page could cause arbitrary command execution.
...
- **Description**: LangGraph checkpoints stored in `~/.blackcat/*.db` contain the full conversation history and agent state. An attacker with local filesystem write access could inject adversarial messages that re-enter the LLM context on session resume.
...
- **Flow**: DF21
- **Description**: `subagents._parse_subagent_file` reads AGENTS.md files from `.blackcat/agents/{name}/AGENTS.md` and project-level `.agents/{name}/AGENTS.md`. The markdown body after the YAML frontmatter is used verbatim as the subagent's `system_prompt`. No content filtering is applied.
- **Preconditions**: Attacker has write access to `~/.blackcat/agents/` or the project's `.agents/` directory. User or LLM must invoke the malicious subagent via the `task` tool.
...
- **Preconditions**: Attacker has write access to `~/.blackcat/config.toml` AND a malicious Python package installed in the user's environment (or on `sys.path`). The code comments document this as intentional: "same trust model as `pyproject.toml` build scripts — the user controls their own machine."
...
- **Preconditions**: (1) Attacker has write access to a project-level `.mcp.json`; (2) The project MCP config must be trusted by the user (fingerprint approval gate via `mcp_trust`). For user-level `~/.blackcat/.mcp.json`, the attacker already has home directory write access. Note: the `env` dict from MCP config is passed to `StdioConnection` — whether it replaces or merges with `os.environ` depends on the `protohello_mcp_adapters` library implementation.
...
| Hook commands doing harmful things                                     | Hooks in `~/.blackcat/hooks.json` are 100% user-authored. The payload is data-only (JSON on stdin).                                                                               | JSON structure validation (`hooks._load_hooks`); 5-second timeout.                                                 |
...
**Custom subagent system prompts**: Subagent definitions in `.blackcat/agents/` are user-authored files. The framework correctly treats them as user-controlled content. The HITL gate on `task` tool calls ensures the user approves subagent delegation before it occurs.
