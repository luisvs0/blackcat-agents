#!/usr/bin/env bash
# Install blackcat-cli.
#
# Usage:
#   curl -LsSf https://raw.githubusercontent.com/protohello-ai/blackcat/main/libs/cli/scripts/install.sh | bash
#
# Environment variables:
#   BLACKCAT_EXTRAS  — comma-separated pip extras, e.g. "ollama",
#                        "ollama,groq", or "daytona"
#                        (see pyproject.toml for available extras)
#   BLACKCAT_PYTHON  — Python version to use (default: 3.13)
#   BLACKCAT_SKIP_OPTIONAL — set to 1 to skip optional tool checks
...
cleanup() {
  ...
  if [ $exit_code -ne 0 ]; then
    ...
    log_error "For help, visit: https://docs.protohello.com/oss/python/blackcat/cli/overview"
  fi
}
trap cleanup EXIT
...
# and blackcat install to the right place.
...
# Config
# ---------------------------------------------------------------------------
EXTRAS="${BLACKCAT_EXTRAS:-}"
PYTHON_VERSION="${BLACKCAT_PYTHON:-3.13}"
SKIP_OPTIONAL="${BLACKCAT_SKIP_OPTIONAL:-0}"
...
if [[ -n "$EXTRAS" ]]; then
  ...
  if [[ ! "$EXTRAS" =~ ^[-a-zA-Z0-9,]+$ ]]; then
    log_error "BLACKCAT_EXTRAS must be comma-separated extra names, e.g. 'anthropic,groq' or 'daytona'"
    exit 1
  fi
  EXTRAS="[${EXTRAS}]"
fi
...
# Install blackcat-cli
# ---------------------------------------------------------------------------
PACKAGE="blackcat-cli${EXTRAS}"

# Capture pre-install version (if any) for messaging
PRE_VERSION=""
if command -v blackcat >/dev/null 2>&1; then
  PRE_VERSION=$(blackcat -v 2>/dev/null | head -1 | awk '{print $NF}') || PRE_VERSION=""
elif [ -x "${HOME}/.local/bin/blackcat" ]; then
  PRE_VERSION=$("${HOME}/.local/bin/blackcat" -v 2>/dev/null | head -1 | awk '{print $NF}') || PRE_VERSION=""
fi

if [ -n "$PRE_VERSION" ]; then
  log_info "blackcat-cli ${PRE_VERSION} found — checking for updates..."
else
...
if ! "$UV_BIN" tool install -U --python "$PYTHON_VERSION" "$PACKAGE"; then
  ...
  log_error "Common fixes: check your network, try a different Python version (BLACKCAT_PYTHON=3.12), or install manually."
  exit 1
fi
...
log_success "blackcat-cli installed."
...
BLACKCAT_BIN=""
if command -v blackcat >/dev/null 2>&1; then
  BLACKCAT_BIN="blackcat"
elif [ -x "${HOME}/.local/bin/blackcat" ]; then
  BLACKCAT_BIN="${HOME}/.local/bin/blackcat"
fi

if [ -n "$BLACKCAT_BIN" ]; then
  if VERSION=$("$BLACKCAT_BIN" -v 2>&1); then
    log_success "Verified: blackcat ${VERSION}"
  else
    log_warn "blackcat binary found but 'blackcat -v' failed:"
    log_warn "  ${VERSION}"
    log_warn "The installation may be broken. Try running: blackcat -v"
  fi
else
  log_warn "blackcat command not found in PATH. Restart your shell or run:"
...
printf "${GREEN}✔${NC} Setup complete. Run: ${BOLD}blackcat${NC}\n"
echo ""
echo "For help and support, see the Black Cat CLI docs:"
echo "  https://docs.protohello.com/oss/python/blackcat/cli/overview"
