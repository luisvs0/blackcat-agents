...
When the `BLACKCAT_CLI_DEBUG` environment variable is set, modules that handle
...
from blackcat_cli._env_vars import DEBUG, DEBUG_FILE


def configure_debug_logging(target: logging.Logger) -> None:
    """Attach a file handler to *target* when `BLACKCAT_CLI_DEBUG` is set.

    The log file defaults to `'/tmp/blackcat_debug.log'` but can be overridden
    with `BLACKCAT_CLI_DEBUG_FILE`. The handler appends so that multiple
    modules share the same log file across a session.

    Does nothing when `BLACKCAT_CLI_DEBUG` is not set.
    ...
    debug_path = Path(
        os.environ.get(
            DEBUG_FILE,
            "/tmp/blackcat_debug.log",  # noqa: S108
        )
    )
    try:
