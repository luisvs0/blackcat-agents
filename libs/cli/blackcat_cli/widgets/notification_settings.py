...
from blackcat_cli.config import get_glyphs, is_ascii_mode
...
class NotificationSettingsScreen(ModalScreen[None]):
    ...
    `~/.blackcat/config.toml`. Toggling a checkbox immediately
    persists the change.
    """
        ...
        async def _persist() -> None:
            ...
            if not ok:
                self.app.notify(
                    ...
                    "Check file permissions for ~/.blackcat/config.toml.",
                    severity="warning",
                    timeout=6,
                    markup=False,
                )
