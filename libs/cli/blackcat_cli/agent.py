...
from blackcat.backends import CompositeBackend, LocalShellBackend
from blackcat.backends.filesystem import FilesystemBackend
from blackcat.middleware import MemoryMiddleware, SkillsMiddleware

if TYPE_CHECKING:
    from collections.abc import Awaitable, Callable, Sequence

    from blackcat.backends.sandbox import SandboxBackendProtocol
    from blackcat.middleware.async_subagents import AsyncSubAgent
    from blackcat.middleware.subagents import CompiledSubAgent, SubAgent
    from protohello.agents.middleware import InterruptOnConfig
    from protohello.agents.middleware.types import AgentState
    from protohello.messages import ToolCall
    from protohello.tools import BaseTool
    ...
    from blackcat_cli.output import OutputFormat
...
from blackcat_cli.integrations.sandbox_factory import get_default_working_dir
from blackcat_cli.local_context import (
    LocalContextMiddleware,
    _AsyncExecutableBackend,
    _ExecutableBackend,
)
from blackcat_cli.project_utils import ProjectContext, get_server_project_context
from blackcat_cli.subagents import list_subagents
from blackcat_cli.unicode_security import (
    check_url_safety,
    ...
    **before execution**. Rejected commands are returned as error `ToolMessage`
    ...
    def _validate_tool_call(self, request: ToolCallRequest) -> ToolMessage | None:
        ...
        Returns:
            ...
            otherwise `None`.
        """
        from protohello_core.messages import ToolMessage as LCToolMessage

        from blackcat_cli.config import SHELL_TOOL_NAMES, is_shell_command_allowed
...
def load_async_subagents(config_path: Path | None = None) -> list[AsyncSubAgent]:
    ...
    if config_path is None:
        config_path = Path.home() / ".blackcat" / "config.toml"
...
def list_agents(*, output_format: OutputFormat = "text") -> None:
    ...
    """
    agents_dir = settings.user_blackcat_dir

    if not agents_dir.exists() or not any(agents_dir.iterdir()):
        if output_format == "json":
            from blackcat_cli.output import write_json

            write_json("list", [])
            return
        console.print("[yellow]No agents found.[/yellow]")
        console.print(
            "[dim]Agents will be created in ~/.blackcat/ "
    ...
    if source_agent:
        ...
        if not source_md.exists():
            console.print(
                ...
                "or has no AGENTS.md\n"
                "  Available agents: blackcat agents list"
            )
            raise SystemExit(1)
...
def get_system_prompt(
    ...
    *,
    ...
    skills_path = f"~/.blackcat/{assistant_id}/skills"
...
def create_cli_agent(
    ...
    *,
    ...
    This is the main entry point for creating a blackcat CLI agent, usable
    ...
    # read the channel.
    from blackcat_cli.token_state import TokenStateMiddleware
    ...
    if enable_skills:
        ...
        # built-in -> user .blackcat -> user .agents
        # -> project .blackcat -> project .agents
    ...
    if sandbox is None:
        ...
        if enable_shell:
            ...
            if settings.user_protohello_project:
                shell_env["LANGSMITH_PROJECT"] = settings.user_protohello_project
    ...
    if sandbox is None:
        ...
        large_results_backend = FilesystemBackend(
            root_dir=tempfile.mkdtemp(prefix="blackcat_large_results_"),
            virtual_mode=True,
        )
        conversation_history_backend = FilesystemBackend(
            root_dir=tempfile.mkdtemp(prefix="blackcat_conversation_history_"),
            virtual_mode=True,
        )
