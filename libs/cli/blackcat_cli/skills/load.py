...
blackcat.middleware.skills and adapts it for direct filesystem access
needed by CLI commands.

For middleware usage within agents, use
blackcat.middleware.skills.SkillsMiddleware directly.
"""
...
def list_skills(
    *,
    ...
    1. `user_skills_dir` (`~/.blackcat/{agent}/skills/`)
    2. `user_agent_skills_dir` (`~/.agents/skills/`)
    3. `project_skills_dir` (`.blackcat/skills/`)
    ...
    Args:
        ...
        user_skills_dir: Path to `~/.blackcat/{agent}/skills/`.
        project_skills_dir: Path to `.blackcat/skills/`.
        ...
        try:
            ...
            for skill in skills:
                ...
                if source_label == "built-in":
                    extra["metadata"] = {
                        **skill["metadata"],
                        "blackcat-cli-version": _cli_version,
                    }
...
def load_skill_content(
    ...
    *,
    ...
    if allowed_roots and not any(path.is_relative_to(root) for root in allowed_roots):
        ...
        from blackcat_cli._env_vars import EXTRA_SKILLS_DIRS
