"""MCP (Model Context Protocol) tools loader for blackcat CLI.

This module provides async functions to load and manage MCP servers using
`protohello-mcp-adapters`, supporting Claude Desktop style JSON configs.
...
if TYPE_CHECKING:
    ...
    from protohello_mcp_adapters.client import Connection, MultiServerMCPClient
...
def discover_mcp_configs(
    *, project_context: ProjectContext | None = None
    ...
    1. `~/.blackcat/.mcp.json` (user-level global)
    2. `<project-root>/.blackcat/.mcp.json` (project subdir)
    ...
    """
    user_dir = Path.home() / ".blackcat"
    project_root = _resolve_project_config_base(project_context)

    candidates = [
        user_dir / ".mcp.json",
        project_root / ".blackcat" / ".mcp.json",
...
def classify_discovered_configs(
    ...
    User-level configs live under `~/.blackcat/`. Everything else is
...
async def _load_tools_from_config(
    ...
    """
    from protohello_mcp_adapters.client import MultiServerMCPClient
    from protohello_mcp_adapters.sessions import (
        SSEConnection,
        StdioConnection,
        StreamableHttpConnection,
    )
    from protohello_mcp_adapters.tools import load_mcp_tools
    ...
    # Convert Claude Desktop format to protohello-mcp-adapters format
    connections: dict[str, Connection] = {}
    for server_name, server_config in config["mcpServers"].items():
        server_type = _resolve_server_type(server_config)

        if server_type in _SUPPORTED_REMOTE_TYPES:
            # protohello-mcp-adapters uses "streamable_http" for HTTP transport
            if server_type == "http":
...
async def get_mcp_tools(
    ...
    Returns:
        Tuple of `(tools_list, session_manager, server_infos)` where:
            - tools_list: List of ProtoHello `BaseTool` objects
