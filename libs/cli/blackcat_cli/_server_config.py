...
environment variables prefixed with `BLACKCAT_CLI_SERVER_`. This module
provides a single
...
def _read_env_bool(suffix: str, *, default: bool = False) -> bool:
    """Read a `BLACKCAT_CLI_SERVER_*` boolean from the environment.

    Boolean env vars use the `'true'` / `'false'` convention (case insensitive).
    Missing variables fall back to *default*.

    Args:
        suffix: Variable name suffix after the `BLACKCAT_CLI_SERVER_` prefix.
...
def _read_env_json(suffix: str) -> Any:  # noqa: ANN401
    """Read a JSON-encoded `BLACKCAT_CLI_SERVER_*` variable.
...
def _read_env_str(suffix: str) -> str | None:
    """Read an optional `BLACKCAT_CLI_SERVER_*` string variable.
...
def _read_env_optional_bool(suffix: str) -> bool | None:
    """Read a tri-state `BLACKCAT_CLI_SERVER_*` boolean (`True` / `False` / `None`).
...
class ServerConfig:
    ...
    Serialized to/from `BLACKCAT_CLI_SERVER_*` environment variables so
    that the server
    ...
    def to_env(self) -> dict[str, str | None]:
        """Serialize this config to a `BLACKCAT_CLI_SERVER_*` env-var mapping.
    ...
    def from_env(cls) -> ServerConfig:
        """Reconstruct a `ServerConfig` from `BLACKCAT_CLI_SERVER_*` env vars.
