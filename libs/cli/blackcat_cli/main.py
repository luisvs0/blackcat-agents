"""Main entry point and CLI loop for blackcat."""

# ruff: noqa: E402
# Imports placed after warning filters to suppress deprecation warnings

# Suppress deprecation warnings from protohello_core (e.g., Pydantic V1 on Python 3.14+)
import warnings

warnings.filterwarnings("ignore", module="protohello_core._api.deprecation")
...
if TYPE_CHECKING:
    from blackcat_cli.app import AppResult
    from blackcat_cli.mcp_tools import MCPServerInfo

# Suppress Pydantic v1 compatibility warnings from protohello on Python 3.14+
...
def check_cli_dependencies() -> None:
    ...
    if missing:
        ...
        print("\nThe following packages are required to use the blackcat CLI:")  # noqa: T201  # CLI output for missing dependencies
        for pkg in missing:
        ...
        print("  pip install blackcat[cli]")  # noqa: T201  # CLI output for missing dependencies
...
_SUPPRESS_HINT_CLI = (
    'To suppress, edit ~/.blackcat/config.toml:\n\\[warnings]\nsuppress = \\["<key>"]'
)
...
def check_optional_tools(*, config_path: Path | None = None) -> list[str]:
    ...
    Args:
        config_path: Path to config file.
            ...
            Defaults to `~/.blackcat/config.toml`.
...
async def _preload_session_mcp_server_info(
    *,
    ...
    from blackcat_cli.mcp_tools import resolve_and_load_mcp_tools
    from blackcat_cli.project_utils import ProjectContext
...
def parse_args() -> argparse.Namespace:
    ...
    """
    from blackcat_cli.deploy import setup_deploy_parsers
    from blackcat_cli.output import add_json_output_arg
    from blackcat_cli.skills import setup_skills_parser
    ...
    # `blackcat <subcommand> -h` shows context-specific help.
    ...
    parser = argparse.ArgumentParser(
        description=("Black Cat - AI Coding Assistant"),
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"blackcat-cli {__version__}\nblackcat (SDK) {sdk_version}",
    )
    parser.add_argument(
        "-h",
        "--help",
...
async def run_textual_cli_async(
    ...
    *,
    ...
    from blackcat_cli.app import AppResult, run_textual_app
    from blackcat_cli.config import (
        _get_default_model_spec,
        detect_provider,
        settings,
    )
    from blackcat_cli.model_config import ModelConfigError, ModelSpec

    # Resolve display-name cheaply (<1ms, no protohello) so the status
    ...
    try:
        ...
        from blackcat_cli.config import console
...
async def _run_acp_cli_async(
    ...
    *,
    ...
    """
    from blackcat_cli.agent import create_cli_agent, load_async_subagents
    ...
    from blackcat_cli.tools import fetch_url, web_search
...
def apply_stdin_pipe(args: argparse.Namespace) -> None:
    ...
    - If `non_interactive_message` is already set (`-n`), prepends the
        ...
        ```bash
        cat context.txt | blackcat -n "summarize this"
    ...
    - If `initial_prompt` is already set (`-m`, but not `-n`), prepends
        ...
        ```bash
        cat error.log | blackcat -m "explain this"
    ...
    - If `initial_skill` is already set (`--skill`, but not `-n`), stores the
        ...
        ```bash
        cat diff.txt | blackcat --skill code-review
    ...
    - Otherwise, sets `non_interactive_message` to the piped text, causing
        ...
        ```bash
        echo "fix the typo in README.md" | blackcat
    ...
    if is_tty:
        if explicit_stdin:
            console.print(
                ...
                "  cat prompt.txt | blackcat --stdin -q"
            )
            sys.exit(1)
        return
...
def _print_session_stats(stats: Any, console: Any) -> None:  # noqa: ANN401
    ...
    """
    from blackcat_cli.textual_adapter import SessionStats, print_usage_table
...
def _check_mcp_project_trust(*, trust_flag: bool = False) -> bool | None:
    ...
    """
    from blackcat_cli.mcp_tools import (
    ...
    # Check trust store
    from blackcat_cli.mcp_trust import (
...
def cli_main() -> None:
    ...
    # This ensures agent traces use BLACKCAT_CLI_LANGSMITH_PROJECT while
    ...
    if len(sys.argv) == 2 and sys.argv[1] in {"-v", "--version"}:  # noqa: PLR2004  # argv length check for fast-path
        try:
            ...
            sdk_version = "unknown"
        print(f"blackcat-cli {__version__}\nblackcat (SDK) {sdk_version}")  # noqa: T201  # CLI version output
        sys.exit(0)
    ...
    try:
        ...
        from blackcat_cli.config import console, settings
        ...
        if getattr(args, "acp", False):
            try:
                ...
                msg = (
                    ...
                    "Install with: pip install blackcat-acp\n"
                )
                sys.stderr.write(msg)
                sys.stderr.flush()
                sys.exit(1)
            ...
            if getattr(args, "no_mcp", False) and getattr(args, "mcp_config", None):
                msg = (
                    ...
                    "  blackcat --mcp-config path/to/config.json\n"
                    "  blackcat --no-mcp\n"
                )
                sys.stderr.write(msg)
                sys.stderr.flush()
        ...
        if args.shell_allow_list:
            from blackcat_cli.config import parse_shell_allow_list
        ...
        if (
            ...
            _Console(stderr=True).print(
                ...
                "--no-stream.\n"
                "  blackcat --skill code-review -m 'review this patch'\n"
                "  blackcat --skill code-review -n 'review this patch'"
            )
            sys.exit(2)
        ...
        if (args.quiet or args.no_stream) and not args.non_interactive_message:
            ...
            _Console(stderr=True).print(
                ...
                "  blackcat -n 'summarize README.md' --quiet"
            )
            sys.exit(2)
        ...
        if args.default_model is not None:
            ...
            from blackcat_cli.model_config import ModelSpec
        ...
        if args.command == "help":
            from blackcat_cli.ui import show_help

            show_help()
        elif args.command == "agents":
            from blackcat_cli.agent import list_agents, reset_agent
            from blackcat_cli.ui import show_agents_help
            ...
            from blackcat_cli.skills import execute_skills_command

            execute_skills_command(args)
        elif args.command == "init":
            from blackcat_cli.deploy import execute_init_command

            execute_init_command(args)
        elif args.command == "dev":
            from blackcat_cli.deploy import execute_dev_command

            execute_dev_command(args)
        elif args.command == "deploy":
            from blackcat_cli.deploy import execute_deploy_command
            ...
            from blackcat_cli.ui import show_threads_help
            ...
            if args.sandbox and args.sandbox not in {"none", "langsmith"}:
                from blackcat_cli.integrations.sandbox_factory import (
                    verify_sandbox_deps,
                )
            ...
            from blackcat_cli.non_interactive import run_non_interactive
            ...
            if thread_id and return_code == 0 and asyncio.run(thread_exists(thread_id)):
                ...
                hint = Text("blackcat -r ", style="cyan")
