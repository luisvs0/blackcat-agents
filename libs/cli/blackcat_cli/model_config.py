...
_ENV_PREFIX = "BLACKCAT_CLI_"


def resolve_env_var(name: str) -> str | None:
    """Look up an env var with `BLACKCAT_CLI_` prefix override.

    Checks `BLACKCAT_CLI_{name}` first, then falls back to `{name}`.
    ...
    set `BLACKCAT_CLI_X=""` to shadow a canonically-set key -- the function
    ...
    to avoid nonsensical `BLACKCAT_CLI_BLACKCAT_CLI_*` reads
...
class ProviderConfig(TypedDict, total=False):
    ...
    and all its models from the `/model` selector. Useful when a ProtoHello
...
DEFAULT_CONFIG_DIR = Path.home() / ".blackcat"
"""Directory for user-level Black Cat configuration (`~/.blackcat`)."""

DEFAULT_CONFIG_PATH = DEFAULT_CONFIG_DIR / "config.toml"
"""Path to the user's model configuration file (`~/.blackcat/config.toml`)."""
...
Providers not listed here fall through to the config-file check or the protohello
registry fallback.
"""
...
def _get_builtin_providers() -> dict[str, Any]:
    """Return protohello's built-in provider registry.

    Tries the newer `_BUILTIN_PROVIDERS` name first, then falls back to
    the legacy `_SUPPORTED_PROVIDERS` for older protohello versions.

    Results are cached after the first call; use `clear_caches()` to reset.

    Returns:
        The provider registry dict from `protohello.chat_models.base`.
    """
    ...
    # Deferred: protohello.chat_models pulls in heavy provider registry,
    # only needed when resolving provider names for model config.
    from protohello.chat_models import base
...
def _get_provider_profile_modules() -> list[tuple[str, str]]:
    """Build a `(provider, profile_module)` list from protohello's provider registry.

    Reads the built-in provider registry from `protohello.chat_models.base`
...
def _load_provider_profiles(module_path: str) -> dict[str, Any]:
    ...
    Args:
        module_path: Dotted module path (e.g., `"protohello_openai.data._profiles"`).
...
def _profile_module_from_class_path(class_path: str) -> str | None:
    ...
    Returns:
        Dotted module path like `protohello_baseten.data._profiles`, or None
...
def get_available_models() -> dict[str, list[str]]:
    """Get available models dynamically from installed ProtoHello provider packages.
    ...
    Returns:
        Dictionary mapping provider names to lists of model identifiers.
            Includes providers from the protohello registry, config-file
    ...
    # Try to load from protohello provider profile data.
    # Build the list dynamically from protohello's supported-provider registry
    # so new providers are picked up automatically when protohello adds them.
...
def has_provider_credentials(provider: str) -> bool | None:
    ...
    1. **Config-file providers** (`config.toml` `[providers.<name>]`):
        - If the section declares `api_key_env`, that env var is checked
            via `resolve_env_var()` (which honors `BLACKCAT_CLI_` prefixes).
...
class ModelConfig:
    ...
    def load(cls, config_path: Path | None = None) -> ModelConfig:
        ...
        Args:
            config_path: Path to config file. Defaults to ~/.blackcat/config.toml.
