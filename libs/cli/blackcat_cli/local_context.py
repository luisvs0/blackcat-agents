...
if TYPE_CHECKING:
    ...
    from blackcat.backends.protocol import ExecuteResponse
    from blackcat.middleware.summarization import SummarizationEvent
...
FILES=$(
  { ls -1 2>/dev/null; [ -e .blackcat ] && echo .blackcat; } |
  grep -vE "^(${EXCL})$" |
  sort -u
)
if [ -n "$FILES" ]; then
