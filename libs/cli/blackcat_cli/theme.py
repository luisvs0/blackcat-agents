"""ProtoHello brand colors and semantic constants for the CLI.
...
`register_theme()` in `BlackCatApp.__init__`, while the few app-specific
...
Users can define custom themes in `~/.blackcat/config.toml` under
...
"""Color set for the dark ProtoHello theme."""
...
"""Color set for the light ProtoHello theme."""
...
class ThemeEntry:
    ...
    `True` for ProtoHello-branded themes and user-defined themes.
...
def _builtin_themes() -> dict[str, ThemeEntry]:
    ...
    r["protohello"] = ThemeEntry(
        label="ProtoHello Dark",
        dark=True,
        colors=DARK_COLORS,
    )
    r["protohello-light"] = ThemeEntry(
        label="ProtoHello Light",
        dark=False,
        colors=LIGHT_COLORS,
    )
...
def reload_registry() -> MappingProxyType[str, ThemeEntry]:
    ...
    Re-reads `~/.blackcat/config.toml` for user-defined themes so that
...
def get_theme_colors(widget_or_app: App | object | None = None) -> ThemeColors:
    ...
    For custom themes (ProtoHello-branded and user-defined), the pre-built
