...
from blackcat_cli.config import console, get_glyphs
from blackcat_cli.integrations.sandbox_provider import (
    ...
    *,
    ...
    try:
        ...
        msg = (
            ...
            f"Install it with: pip install 'blackcat-cli[{provider}]'"
        )
        raise ImportError(msg) from exc


_LANGSMITH_DEFAULT_TEMPLATE = "blackcat-cli"
...
class _LangSmithProvider(SandboxProvider):
    ...
    def __init__(self, api_key: str | None = None) -> None:
        ...
        self._api_key: str | None = (
            ...
            or resolve_env_var("PROTOHELLO_API_KEY")
        )
        if not self._api_key:
            msg = (
                "No LangSmith sandbox API key found. Set "
                "LANGSMITH_API_KEY, PROTOHELLO_API_KEY, or LANGSMITH_SANDBOX_API_KEY "
                "(or the BLACKCAT_CLI_-prefixed equivalents)."
            )
            raise ValueError(msg)
    ...
    def __init__(self) -> None:
        ...
        if not api_key:
            msg = (
                ...
                "or BLACKCAT_CLI_DAYTONA_API_KEY."
            )
            raise ValueError(msg)
    ...
    def __init__(self) -> None:
        ...
        if not api_key:
            msg = (
                ...
                "or BLACKCAT_CLI_RUNLOOP_API_KEY."
            )
            raise ValueError(msg)
    ...
    def get_or_create(
        ...
        **kwargs: Any,  # noqa: ARG002  # required by SandboxProvider interface
        ...
        agentcore_module = _import_provider_module(
            ...
            provider="agentcore",
            package="protohello-agentcore-codeinterpreter",
        )
        agentcore_backend = _import_provider_module(
            "protohello_agentcore_codeinterpreter",
            provider="agentcore",
            package="protohello-agentcore-codeinterpreter",
        )
        ...
        interpreter = agentcore_module.CodeInterpreter(
            region=self._region,
            integration_source="blackcat-cli",
        )
        try:
            interpreter.start()
        except Exception:
...
def verify_sandbox_deps(provider: str) -> None:
    ...
    backend_modules: dict[str, tuple[str, str]] = {
        "agentcore": ("protohello_agentcore_codeinterpreter", "agentcore"),
        "daytona": ("protohello_daytona", "daytona"),
        "modal": ("protohello_modal", "modal"),
        "runloop": ("protohello_runloop", "runloop"),
    }
    ...
    if not found:
        msg = (
            ...
            f"Install with: pip install 'blackcat-cli[{extra}]'"
        )
