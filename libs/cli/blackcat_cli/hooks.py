...
Loads hook configuration from `~/.blackcat/hooks.json` and fires matching
...
Config format (`~/.blackcat/hooks.json`):
...
def _load_hooks() -> list[dict[str, Any]]:
    ...
    from blackcat_cli.model_config import DEFAULT_CONFIG_DIR
