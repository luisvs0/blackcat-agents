...
if TYPE_CHECKING:
    ...
    from blackcat_cli.mcp_tools import MCPSessionManager
...
def _set_or_clear_server_env(name: str, value: str | None) -> None:
    """Set or clear a `BLACKCAT_CLI_SERVER_*` environment variable.

    Args:
        name: Suffix after `BLACKCAT_CLI_SERVER_`.
...
def _apply_server_config(config: ServerConfig) -> None:
    """Write a `ServerConfig` to `BLACKCAT_CLI_SERVER_*` env vars.
...
def _write_checkpointer(work_dir: Path) -> None:
    ...
    `BLACKCAT_CLI_SERVER_*` env-var communication pattern used elsewhere.
...
def _write_pyproject(work_dir: Path) -> None:
    ...
    content = f"""[project]
name = "blackcat-server-runtime"
version = "0.0.1"
requires-python = ">=3.11"
dependencies = [
    "blackcat-cli @ file://{cli_dir}",
]
...
async def start_server_and_get_agent(
    *,
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="blackcat_server_"))
