"""Update lifecycle for `blackcat-cli`.
...
from blackcat_cli._version import PYPI_URL, USER_AGENT, __version__

if TYPE_CHECKING:
    from pathlib import Path

from blackcat_cli.model_config import DEFAULT_CONFIG_DIR, DEFAULT_CONFIG_PATH
...
_UPGRADE_COMMANDS: dict[InstallMethod, str] = {
    "uv": "uv tool upgrade blackcat-cli",
    "brew": "brew upgrade blackcat-cli",
    "pip": "pip install --upgrade blackcat-cli",
}
...
def get_latest_version(
    *,
    ...
    """Fetch the latest blackcat-cli version from PyPI, with caching.
...
def is_update_available(*, bypass_cache: bool = False) -> tuple[bool, str | None]:
    """Check whether a newer version of blackcat-cli is available.
...
def detect_install_method() -> InstallMethod:
    """Detect how `blackcat-cli` was installed.
...
def upgrade_command(method: InstallMethod | None = None) -> str:
    """Return the shell command to upgrade `blackcat-cli`.
...
async def perform_upgrade() -> tuple[bool, str]:
    """Attempt to upgrade `blackcat-cli` using the detected install method.
...
def is_update_check_enabled() -> bool:
    ...
    Checks `BLACKCAT_CLI_NO_UPDATE_CHECK` env var and the `[update].check` key
    in `config.toml`.

    Defaults to enabled.
    """
    from blackcat_cli._env_vars import NO_UPDATE_CHECK
...
def is_auto_update_enabled() -> bool:
    ...
    Opt-in via `BLACKCAT_CLI_AUTO_UPDATE=1` env var or
    ...
    """
    from blackcat_cli._env_vars import AUTO_UPDATE
    from blackcat_cli.config import _is_editable_install
