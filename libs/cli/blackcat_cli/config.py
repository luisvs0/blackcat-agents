...
from blackcat_cli._version import __version__
...
# callers that never touch `settings` (e.g. `blackcat --help`).
# ---------------------------------------------------------------------------
...
# Global user-level .env (~/.blackcat/.env); sentinel when Path.home() fails.
try:
    _GLOBAL_DOTENV_PATH = Path.home() / ".blackcat" / ".env"
except RuntimeError:
    _GLOBAL_DOTENV_PATH = Path("/nonexistent/.blackcat/.env")
...
def _load_dotenv(*, start_path: Path | None = None) -> bool:
    ...
    2. `~/.blackcat/.env` — global user defaults
    ...
    !!! note
        ...
        identically-named shell exports, use the `BLACKCAT_CLI_` env-var
        prefix (see `resolve_env_var` in `blackcat_cli.model_config`).
    ...
    # 2. Global (~/.blackcat/.env) — fills in any vars not already set by
...
def _ensure_bootstrap() -> None:
    ...
    with _bootstrap_lock:
        ...
        try:
            from blackcat_cli.project_utils import (
            ...
            from blackcat_cli._env_vars import LANGSMITH_PROJECT

            blackcat_project = os.environ.get(LANGSMITH_PROJECT)
            if blackcat_project:
                os.environ["LANGSMITH_PROJECT"] = blackcat_project
            ...
            # of the BLACKCAT_CLI_ prefix. Setting canonical vars here
            # bridges that gap.
            from blackcat_cli.model_config import _ENV_PREFIX
...
def _resolve_editable_info() -> tuple[bool, str | None]:
    ...
    try:
        dist = distribution("blackcat-cli")
...
def _is_editable_install() -> bool:
    """Check if blackcat-cli is installed in editable mode.
...
def build_stream_config(
    ...
    * `create_black_cat` bakes `versions: {"blackcat": "X.Y.Z"}` into the
    ...
    * Including the SDK version here ensures it survives the merge.
    ...
    versions: dict[str, str] = {"blackcat-cli": __version__}
    with contextlib.suppress(importlib_metadata.PackageNotFoundError):
        versions["blackcat"] = importlib_metadata.version("blackcat")

    metadata: dict[str, Any] = {
        "versions": versions,
        "ls_integration": "blackcat-cli",
    }
    from blackcat_cli._env_vars import USER_ID
...
def _read_config_toml_skills_dirs() -> list[str] | None:
    """Read `[skills].extra_allowed_dirs` from `~/.blackcat/config.toml`.
...
def _parse_extra_skills_dirs(
    ...
    The env var (`BLACKCAT_CLI_EXTRA_SKILLS_DIRS`, colon-separated) takes
    precedence: when set, `config.toml` values are ignored.

    Args:
        env_raw: Value of `BLACKCAT_CLI_EXTRA_SKILLS_DIRS` (colon-separated), or
            `None` if unset.
        config_toml_dirs: List of path strings from
            `[skills].extra_allowed_dirs` in `~/.blackcat/config.toml`.
...
@dataclass
class Settings:
    """Global settings and environment detection for blackcat-cli.
    ...
    blackcat_protohello_project: str | None
    """LangSmith project name for blackcat agent tracing."""

    user_protohello_project: str | None
    ...
    Set via `BLACKCAT_CLI_EXTRA_SKILLS_DIRS` env var (colon-separated) or
    `[skills].extra_allowed_dirs` in `~/.blackcat/config.toml`.
    """
    ...
    def from_environment(cls, *, start_path: Path | None = None) -> Settings:
        ...
        from blackcat_cli.model_config import resolve_env_var
        ...
        # BLACKCAT_CLI_LANGSMITH_PROJECT: Project for blackcat agent tracing
        # user_protohello_project: User's ORIGINAL LANGSMITH_PROJECT (before override)
        ...
        from blackcat_cli._env_vars import (
            EXTRA_SKILLS_DIRS,
            LANGSMITH_PROJECT,
            SHELL_ALLOW_LIST,
        )

        blackcat_protohello_project = resolve_env_var(LANGSMITH_PROJECT)
        user_protohello_project = _original_langsmith_project  # Use saved original!

        # Detect project
        from blackcat_cli.project_utils import find_project_root
        ...
        return cls(
            ...
            blackcat_protohello_project=blackcat_protohello_project,
            user_protohello_project=user_protohello_project,
    ...
    def reload_from_environment(self, *, start_path: Path | None = None) -> list[str]:
        ...
        (`user_protohello_project`) are intentionally preserved -- they are
        ...
        !!! note
            ...
            from `.env`, use the `BLACKCAT_CLI_` prefix (e.g.
            `BLACKCAT_CLI_OPENAI_API_KEY`).
        ...
        # the change
        new_project = refreshed["blackcat_protohello_project"]
        if new_project:
            os.environ["LANGSMITH_PROJECT"] = new_project
        elif previous["blackcat_protohello_project"]:
    ...
    @property
    def user_blackcat_dir(self) -> Path:
        """Get the base user-level .blackcat directory.

        Returns:
            Path to ~/.blackcat
        """
        return Path.home() / ".blackcat"
    ...
    def get_user_agent_md_path(agent_name: str) -> Path:
        ...
        Returns:
            Path to ~/.blackcat/{agent_name}/AGENTS.md
        """
        return Path.home() / ".blackcat" / agent_name / "AGENTS.md"

    def get_project_agent_md_path(self) -> list[Path]:
        """Get project-level AGENTS.md paths.

        Checks both `{project_root}/.blackcat/AGENTS.md` and
        ...
        if not self.project_root:
            return []
        from blackcat_cli.project_utils import find_project_agent_md
    ...
    def get_agent_dir(self, agent_name: str) -> Path:
        ...
        if not self._is_valid_agent_name(agent_name):
            ...
            raise ValueError(msg)
        return Path.home() / ".blackcat" / agent_name
    ...
    def get_user_skills_dir(self, agent_name: str) -> Path:
        ...
        Returns:
            Path to ~/.blackcat/{agent_name}/skills/
        """
    ...
    def get_project_skills_dir(self) -> Path | None:
        ...
        Returns:
            Path to {project_root}/.blackcat/skills/, or None if not in a project
        """
        if not self.project_root:
            return None
        return self.project_root / ".blackcat" / "skills"
    ...
    def get_user_agents_dir(self, agent_name: str) -> Path:
        ...
        Args:
            agent_name: Name of the CLI agent (e.g., "blackcat")

        Returns:
            Path to ~/.blackcat/{agent_name}/agents/
        """
    ...
    def get_project_agents_dir(self) -> Path | None:
        ...
        Returns:
            Path to {project_root}/.blackcat/agents/, or None if not in a project
        """
        if not self.project_root:
            return None
        return self.project_root / ".blackcat" / "agents"
    ...
    def get_extra_skills_dirs(self) -> list[Path]:
        ...
        Set via `BLACKCAT_CLI_EXTRA_SKILLS_DIRS` (colon-separated paths) or
        `[skills].extra_allowed_dirs` in `~/.blackcat/config.toml`.
...
def get_langsmith_project_name() -> str | None:
    ...
    `settings.blackcat_protohello_project` (from
    `BLACKCAT_CLI_LANGSMITH_PROJECT`), then `LANGSMITH_PROJECT` from the
    environment (note: this may already have been overridden at bootstrap time
    to match `BLACKCAT_CLI_LANGSMITH_PROJECT`), then `'blackcat-cli'`.
    ...
    return (
        _get_settings().blackcat_protohello_project
...
def fetch_langsmith_project_url(project_name: str) -> str | None:
    ...
    def _lookup_url() -> None:
        ...
        try:
            ...
            # and doesn't know about the BLACKCAT_CLI_ prefix.
...
def build_langsmith_thread_url(thread_id: str) -> str | None:
    ...
    return f"{project_url.rstrip('/')}/t/{thread_id}?utm_source=blackcat-cli"
...
def detect_provider(model_name: str) -> str | None:
    ...
    Intentionally duplicates a subset of ProtoHello's
...
def _get_default_model_spec() -> str:
    ...
    """
    from blackcat_cli.model_config import ModelConfig, ModelConfigError
...
_OPENROUTER_APP_URL = "https://pypi.org/project/blackcat-cli/"
"""Default `app_url` (maps to `HTTP-Referer`) for OpenRouter attribution.

See https://openrouter.ai/docs/app-attribution for details.
"""

_OPENROUTER_APP_TITLE = "Black Cat CLI"
...
def _get_provider_kwargs(
    ...
    """
    from blackcat_cli.model_config import ModelConfig
    ...
    from blackcat_cli.model_config import PROVIDER_API_KEY_ENV, resolve_env_var
    ...
    if provider == "openrouter":
        from blackcat._openrouter import (
...
def _create_model_from_class(
    ...
    """
    from protohello_core.language_models import (
        BaseChatModel as _BaseChatModel,  # Runtime import; module level is typing only
    )

    from blackcat_cli.model_config import ModelConfigError
...
def _create_model_via_init(
    ...
    """Create a model using protohello's `init_chat_model`.
    ...
    try:
        ...
        package_map = {
            "anthropic": "protohello-anthropic",
            "openai": "protohello-openai",
            "google_genai": "protohello-google-genai",
            "google_vertexai": "protohello-google-vertexai",
            "nvidia": "protohello-nvidia-ai-endpoints",
        }
        package = package_map.get(provider, f"protohello-{provider}")
...
def validate_model_capabilities(model: BaseChatModel, model_name: str) -> None:
    """Validate that the model has required capabilities for `blackcat`.
    ...
    if tool_calling is False:
        ...
        console.print(
            "\nBlack Cat requires tool calling for agent functionality. "
