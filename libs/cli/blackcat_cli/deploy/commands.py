"""CLI commands for `blackcat init`, `dev`, and `deploy`.
...
def setup_deploy_parsers(
    ...
    *,
    ...
    The three commands used to live under `blackcat deploy {init,dev}`
    but are now flat: `blackcat init`, `blackcat dev`, and
    `blackcat deploy`. This function registers all three on the root
    subparsers object.
    """
    # blackcat init
    ...
    dev_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to blackcat.toml (default: auto-discovered from cwd)",
    )
    dev_parser.add_argument(
        "--port",
        type=int,
...
_BETA_WARNING = (
    "\033[33mWarning: `blackcat deploy` is in beta. "
...
def execute_init_command(args: argparse.Namespace) -> None:
    """Execute the `blackcat init` command."""
    print(_BETA_WARNING)
    name = args.name
    if name is None:
        try:
...
def execute_dev_command(args: argparse.Namespace) -> None:
    """Execute the `blackcat dev` command."""
    print(_BETA_WARNING)
    _dev(
...
def execute_deploy_command(args: argparse.Namespace) -> None:
    """Execute the `blackcat deploy` command."""
    print(_BETA_WARNING)
    _deploy(
        config_path=args.config,
...
def _init_project(*, name: str, force: bool = False) -> None:
    ...
    print("\nNext steps:")
    print(f"  cd {name}")
    print("  # edit blackcat.toml and AGENTS.md")
    print("  blackcat deploy")
...
def _deploy(
    ...
    from blackcat_cli.deploy.bundler import bundle, print_bundle_summary
    from blackcat_cli.deploy.config import (
        DEFAULT_CONFIG_FILENAME,
        find_config,
        load_config,
    ...
    try:
        ...
        print(f"Run `blackcat init` to create a starter {DEFAULT_CONFIG_FILENAME}.")
        raise SystemExit(1) from None
    ...
    # Bundle
    build_dir = Path(tempfile.mkdtemp(prefix="blackcat-deploy-"))
...
def _dev(
    *,
    ...
    The bundle is identical to what `blackcat deploy` would ship, just
    ...
    edits to the source project (`blackcat.toml`, skills, `AGENTS.md`)
    require re-running `blackcat dev` to re-bundle.

    Args:
        config_path: Path to `blackcat.toml`, or `None` for default.
    ...
    build_dir = Path(tempfile.mkdtemp(prefix="blackcat-dev-"))
    try:
        bundle(config, project_root, build_dir)
        print_bundle_summary(config, build_dir)
