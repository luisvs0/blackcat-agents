"""Bundle a blackcat project for deployment.

Reads the canonical project layout:

```txt
<project>/
    blackcat.toml  # required — agent + sandbox config
...
from blackcat_cli.deploy.templates import (
...
_MODEL_PROVIDER_DEPS: dict[str, str] = {
    ...
    "azure_openai": "protohello-openai",
    "baseten": "protohello-baseten",
    "cohere": "protohello-cohere",
    "deepseek": "protohello-deepseek",
    "fireworks": "protohello-fireworks",
    "google_genai": "protohello-google-genai",
    "google_vertexai": "protohello-google-vertexai",
    "groq": "protohello-groq",
    "mistralai": "protohello-mistralai",
    "nvidia": "protohello-nvidia-ai-endpoints",
    "openai": "protohello-openai",
    "openrouter": "protohello-openrouter",
    "perplexity": "protohello-perplexity",
    "xai": "protohello-xai",
}
...
def bundle(
    ...
    # blackcat.toml). The bundler skips .env when
...
def _render_pyproject(config: DeployConfig, *, mcp_present: bool) -> str:
    ...
    - the ProtoHello partner package matching the model provider prefix
    - `protohello-mcp-adapters` if `mcp.json` is present
    ...
    if mcp_present:
        deps.append("protohello-mcp-adapters")
