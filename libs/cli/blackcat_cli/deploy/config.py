...
Reads `blackcat.toml` and produces a validated `DeployConfig`.
...
DEFAULT_CONFIG_FILENAME = "blackcat.toml"
...
@dataclass(frozen=True)
class DeployConfig:
    """Top-level deploy configuration parsed from `blackcat.toml`."""
    ...
    def validate(self, project_root: Path) -> list[str]:
        ...
        Args:
            project_root: Directory containing `blackcat.toml`.
...
def load_config(config_path: Path) -> DeployConfig:
    """Load and parse a `blackcat.toml` file.
...
def _parse_config(data: dict[str, Any]) -> DeployConfig:
    ...
    if unknown:
        msg = (
            f"Unknown section(s) in blackcat.toml: {sorted(unknown)}. "
    ...
    if "name" not in agent_data:
        msg = "[agent].name is required in blackcat.toml"
        raise ValueError(msg)
...
def find_config(start_path: Path | None = None) -> Path | None:
    """Find `blackcat.toml` in *start_path* (or cwd if not given).
...
def generate_starter_config() -> str:
    """Generate a starter `blackcat.toml` template."""
    return """\
[agent]
name = "my-agent"
