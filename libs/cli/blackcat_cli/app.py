"""Textual UI application for blackcat-cli."""
...
from blackcat_cli._cli_context import CLIContext
from blackcat_cli._session_stats import (
    SessionStats,
    SpinnerStatus,
    format_token_count,
)
...
from blackcat_cli._version import CHANGELOG_URL, DOCS_URL
from blackcat_cli.config import is_ascii_mode
from blackcat_cli.widgets.chat_input import ChatInput
...
from blackcat_cli.widgets.messages import (
    AppMessage,
    AssistantMessage,
    ErrorMessage,
    QueuedUserMessage,
...
from blackcat_cli.widgets.status import StatusBar
from blackcat_cli.widgets.welcome import WelcomeBanner
...
if TYPE_CHECKING:
    ...
    from blackcat.backends import CompositeBackend
    from protohello_core.runnables import RunnableConfig
    ...
    from blackcat_cli._ask_user_types import AskUserWidgetResult, Question
    from blackcat_cli.mcp_tools import MCPServerInfo
    from blackcat_cli.remote_client import RemoteAgent
    from blackcat_cli.server import ServerProcess
    from blackcat_cli.skills.load import ExtendedSkillMetadata
    from blackcat_cli.textual_adapter import TextualUIAdapter
    from blackcat_cli.widgets.approval import ApprovalMenu
    from blackcat_cli.widgets.ask_user import AskUserMenu
...
def _load_theme_preference() -> str:
    ...
    Returns:
        A Textual theme name (e.g., `'protohello'`, `'protohello-light'`).
    """
    import tomllib

    try:
        from blackcat_cli.model_config import DEFAULT_CONFIG_PATH
...
def save_theme_preference(name: str) -> bool:
    """Persist theme preference to `~/.blackcat/config.toml`.
...
def _new_thread_id() -> str:
    ...
    Returns:
        UUID7 string.
    """
    from blackcat_cli.sessions import generate_thread_id
...
_COMMAND_URLS: dict[str, str] = {
    ...
    "/docs": DOCS_URL,
    "/feedback": "https://github.com/protohello-ai/blackcat/issues/new/choose",
}
"""Slash-command to URL mapping for commands that just open a browser."""


class BlackCatApp(App):
    """Main Textual application for blackcat-cli."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize the Black Cat application.
        ...
        from blackcat_cli.input import MediaTracker
    ...
    async def on_mount(self) -> None:
        ...
        if self._discovered_skills:
            from blackcat_cli.command_registry import (
                SLASH_COMMANDS,
    ...
    async def _post_paint_init(self) -> None:
        ...
        from blackcat_cli.update_check import is_update_check_enabled
        ...
        # When connecting, defer until on_black_cat_app_server_ready fires.
    ...
    async def _init_session_state(self) -> None:
        """Create session state in a thread (imports blackcat_cli.sessions)."""
    ...
    async def _discover_skills(self) -> None:
        ...
        """
        from blackcat_cli.command_registry import SLASH_COMMANDS, build_skill_commands
    ...
    def _discover_skills_and_roots(
        ...
        """
        from blackcat_cli.skills.invocation import discover_skills_and_roots
    ...
    async def _resolve_resume_thread(self) -> None:
        ...
        """
        from blackcat_cli.sessions import (
    ...
    async def _start_server_background(self) -> None:
        ...
        so the protohello import + init doesn't block first paint.
        """
        ...
        # does the heavy protohello import + SDK init and may refine them
        # (e.g., context_limit from the model profile).
        if self._model_kwargs is not None:
            from blackcat_cli.config import create_model
            from blackcat_cli.model_config import ModelConfigError, save_recent_model
        ...
        from blackcat_cli.server_manager import start_server_and_get_agent
        ...
        if self._mcp_preload_kwargs is not None:
            from blackcat_cli.main import _preload_session_mcp_server_info
    ...
    def on_black_cat_app_server_ready(self, event: ServerReady) -> None:
    ...
    def on_black_cat_app_server_start_failed(self, event: ServerStartFailed) -> None:
    ...
    def _prewarm_deferred_imports() -> None:
        ...
        from blackcat_cli.clipboard import (
            copy_selection_to_clipboard,  # noqa: F401
        )
        from blackcat_cli.command_registry import ALWAYS_IMMEDIATE  # noqa: F401
        from blackcat_cli.config import settings  # noqa: F401
        from blackcat_cli.hooks import dispatch_hook  # noqa: F401
        from blackcat_cli.model_config import ModelSpec  # noqa: F401
        from blackcat_cli.textual_adapter import TextualUIAdapter  # noqa: F401
        from blackcat_cli.update_check import is_update_check_enabled  # noqa: F401
        ...
        try:
            ...
            # TUI from rendering.
            from blackcat.backends import DEFAULT_EXECUTE_TIMEOUT  # noqa: F401
            from protohello.agents.middleware.human_in_the_loop import (  # noqa: F401
                ApproveDecision,
            )
            from protohello_core.messages import AIMessage  # noqa: F401
        ...
        from blackcat_cli.widgets.approval import ApprovalMenu  # noqa: F401
        from blackcat_cli.widgets.ask_user import AskUserMenu  # noqa: F401
        from blackcat_cli.widgets.model_selector import (
            ModelSelectorScreen,  # noqa: F401
        )
        from blackcat_cli.widgets.thread_selector import (  # noqa: F401
    ...
    async def _prewarm_model_caches(self) -> None:
        ...
        try:
            from blackcat_cli.model_config import (
    ...
    async def _check_for_updates(self) -> None:
        ...
        try:
            from blackcat_cli.update_check import (
        ...
        try:
            from blackcat_cli._version import __version__ as cli_version

            if is_auto_update_enabled():
                from blackcat_cli.update_check import perform_upgrade
    ...
    async def _show_whats_new(self) -> None:
        ...
        try:
            from blackcat_cli.update_check import should_show_whats_new
        ...
        try:
            from blackcat_cli._version import __version__ as cli_version
            from blackcat_cli.update_check import mark_version_seen
    ...
    async def _handle_auto_update_toggle(self) -> None:
        ...
        try:
            from blackcat_cli.config import _is_editable_install
            from blackcat_cli.update_check import (
                is_auto_update_enabled,
                set_auto_update,
            )
    ...
    async def _request_approval(
        ...
        """
        from blackcat_cli.config import (
            SHELL_TOOL_NAMES,
    ...
    async def on_chat_input_submitted(self, event: ChatInput.Submitted) -> None:
        ...
        from blackcat_cli.hooks import dispatch_hook

        await dispatch_hook("user.prompt", {})

        # /quit and /q always execute immediately, even mid-thread-switch.
        from blackcat_cli.command_registry import ALWAYS_IMMEDIATE
        ...
        # once the server is ready (see on_black_cat_app_server_ready).
    ...
    async def _build_thread_message(prefix: str, thread_id: str) -> str | Content:
        ...
        """
        from blackcat_cli.config import build_langsmith_thread_url
    ...
    async def _handle_command(self, command: str) -> None:
        ...
        """
        from blackcat_cli.config import newline_shortcut, settings
        ...
        if cmd in {"/quit", "/q"}:
            ...
            try:
                from blackcat_cli._version import (
                    __version__ as cli_version,
                )

                cli_line = f"blackcat-cli version: {cli_version}"
            except ImportError:
                logger.debug("blackcat_cli._version module not found")
                cli_line = "blackcat-cli version: unknown"
            except Exception:
            ...
            try:
                ...
                sdk_version = _pkg_version("blackcat")
                sdk_line = f"blackcat (SDK) version: {sdk_version}"
            except PackageNotFoundError:
                logger.debug("blackcat SDK package not found in environment")
                sdk_line = "blackcat (SDK) version: unknown"
            except Exception:
            ...
            try:
                ...
                from blackcat_cli.model_config import clear_caches
    ...
    async def _invoke_skill(
        ...
        *,
        ...
        """
        from blackcat_cli.skills.invocation import build_skill_invocation_envelope
        from blackcat_cli.skills.load import load_skill_content
    ...
    async def _handle_skill_command(self, command: str) -> None:
        ...
        """
        from blackcat_cli.command_registry import parse_skill_command
    ...
    async def _get_conversation_token_count(self) -> int | None:
        ...
        if not self._agent:
            return None
        try:
            from protohello_core.messages.utils import (
    ...
    def _resolve_offload_budget_str(self) -> str | None:
        ...
        """
        from blackcat_cli.config import create_model, settings

        try:
            from blackcat.middleware.summarization import (
            ...
            from blackcat_cli.offload import format_offload_limit
    ...
    async def _handle_offload(self) -> None:
        ...
        from blackcat_cli.config import settings
        from blackcat_cli.offload import (
            OffloadModelError,
        ...
        try:
            ...
            from blackcat_cli.textual_adapter import _persist_context_tokens
    ...
    async def _run_agent_task(
        ...
        *,
        ...
        if self._ui_adapter is None:
            return
        from blackcat_cli.textual_adapter import execute_task_textual
    ...
    def _convert_messages_to_data(messages: list[Any]) -> list[MessageData]:
        """Convert ProtoHello messages into lightweight `MessageData` objects.
        ...
        Args:
            messages: ProtoHello message objects from a thread checkpoint.

        Returns:
            Ordered list of `MessageData` ready for `MessageStore.bulk_load`.
        """
        from protohello_core.messages import AIMessage, HumanMessage, ToolMessage
    ...
    async def _fetch_thread_history_data(self, thread_id: str) -> _ThreadHistoryPayload:
        ...
        # ProtoHello message objects so _convert_messages_to_data works.
        if messages and isinstance(messages[0], dict):
            from protohello_core.messages.utils import convert_to_messages
    ...
    async def _read_channel_values_from_checkpointer(thread_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            from blackcat_cli.sessions import get_db_path
    ...
    def action_interrupt(self) -> None:
        ...
        """
        from blackcat_cli.widgets.thread_selector import ThreadSelectorScreen
    ...
    def action_quit_app(self) -> None:
        ...
        from blackcat_cli.widgets.thread_selector import (
            DeleteThreadConfirmScreen,
            ThreadSelectorScreen,
        )
    ...
    def exit(
        ...
        from blackcat_cli.hooks import _dispatch_hook_sync, _load_hooks
    ...
    async def action_open_editor(self) -> None:
        ...
        from blackcat_cli.editor import open_in_editor
    ...
    def on_mouse_up(self, event: MouseUp) -> None:  # noqa: ARG002  # Textual event handler signature
        ...
        from blackcat_cli.clipboard import copy_selection_to_clipboard
    ...
    async def _show_model_selector(
        ...
        *,
        ...
        from blackcat_cli.config import settings
        from blackcat_cli.widgets.model_selector import ModelSelectorScreen
    ...
    async def _show_theme_selector(self) -> None:
        ...
        from blackcat_cli.widgets.theme_selector import ThemeSelectorScreen
    ...
    async def _show_notification_settings(self) -> None:
        ...
        from blackcat_cli.model_config import is_warning_suppressed
        from blackcat_cli.widgets.notification_settings import (
            WARNING_TOGGLES,
    ...
    async def _show_mcp_viewer(self) -> None:
        ...
        from blackcat_cli.widgets.mcp_viewer import MCPViewerScreen
    ...
    async def _show_thread_selector(self) -> None:
        ...
        from blackcat_cli.sessions import get_cached_threads, get_thread_limit
        from blackcat_cli.widgets.thread_selector import ThreadSelectorScreen
    ...
    async def _switch_model(
        ...
        *,
        ...
        Args:
            ...
            extra_kwargs: Extra constructor kwargs from `--model-params`.
        """
        from blackcat_cli.config import create_model, detect_provider, settings
        from blackcat_cli.model_config import (
            ModelSpec,
            ...
            if has_creds is False and provider is not None:
                ...
                detail = (
                    ...
                    if env_var
                        ...
                        "Add it to ~/.blackcat/config.toml with an "
                        "api_key_env field"
                    )
                )
                await self._mount_message(
            ...
            if not await asyncio.to_thread(save_recent_model, display):
                await self._mount_message(
                    ErrorMessage(
                        ...
                        "preference. Check permissions for ~/.blackcat/"
                    )
                )
            else:
    ...
    async def _set_default_model(self, model_spec: str) -> None:
        ...
        Updates `[models].default` in `~/.blackcat/config.toml` so that
        ...
        """
        from blackcat_cli.config import detect_provider
        from blackcat_cli.model_config import ModelSpec, save_default_model
        ...
        if await asyncio.to_thread(save_default_model, model_spec):
            ...
            await self._mount_message(
                ErrorMessage(
                    "Could not save default model. Check permissions for ~/.blackcat/"
                )
            )
    ...
    async def _clear_default_model(self) -> None:
        ...
        """
        from blackcat_cli.model_config import clear_default_model
        ...
        if await asyncio.to_thread(clear_default_model):
            ...
            await self._mount_message(
                ErrorMessage(
                    ...
                    "Check permissions for ~/.blackcat/"
                )
            )
