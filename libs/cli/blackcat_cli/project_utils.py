...
class ProjectContext:
    ...
    def project_skills_dir(self) -> Path | None:
        """Return the project `.blackcat/skills` directory, if any."""
        if self.project_root is None:
            return None
        return self.project_root / ".blackcat" / "skills"

    def project_agents_dir(self) -> Path | None:
        """Return the project `.blackcat/agents` directory, if any."""
        if self.project_root is None:
            return None
...
def find_project_agent_md(project_root: Path) -> list[Path]:
    ...
    1. project_root/.blackcat/AGENTS.md
    ...
    """
    candidates = [
        project_root / ".blackcat" / "AGENTS.md",
