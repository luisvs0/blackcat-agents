...
```bash
cat > /tmp/dev_blackcat.py << 'PYEOF'
"""Dev wrapper to run Black Cat CLI with textual devtools."""
import sys
sys.argv = ["blackcat"] + sys.argv[1:]
...
```bash
cd libs/cli && uv run --group test textual run --dev /tmp/dev_blackcat.py
```
...
`textual run --dev` handles the devtools connection, but it needs to run inside the project's virtualenv to import `blackcat_cli`. The wrapper script bridges the gap — `uv run --group test textual run --dev` ensures both `textual-dev` (from the `test` group) and `blackcat_cli` are available in the same environment.
