...
The CLI defers heavy dependencies (protohello, agent, sessions, etc.) so that
...
- **Timing failure** — an import or CLI command exceeded its threshold.
    Profile with `python -X importtime -c "import blackcat_cli.main"`
...
HEAVY_MODULES = frozenset(
    {
        ...
        "blackcat._models",
        "blackcat.backends",
        "blackcat.backends.utils",
        # protohello / langgraph stack
        "protohello",
        "protohello.chat_models",
        "protohello_core",
        "protohello_core.messages",
        "protohello_core.language_models",
        "protohello_core.runnables",
        "protohello_openai",
        "protohello_anthropic",
        "langgraph",
        # CLI runtime modules (deferred to agent.py)
        "blackcat_cli.agent",
        "blackcat_cli.sessions",
        "blackcat_cli.integrations.sandbox_factory",
        "blackcat_cli.tools",
        # Deferred from config.py module level to lazy local imports
        "dotenv",
        "dotenv.main",
        "blackcat_cli.model_config",
        "blackcat_cli.project_utils",
    }
)
...
# heavy protohello / agent / sessions stack.
# ---------------------------------------------------------------------------
...
class TestImportIsolation:
    ...
    Without deferred imports, `blackcat --help` takes 3+ seconds because
    protohello, agent, and sessions all load eagerly. These tests catch any
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "from blackcat_cli.main import check_cli_dependencies",
            "from blackcat_cli.config import is_ascii_mode",
            "import blackcat_cli.ui",
            "import blackcat_cli.skills.commands",
            "from blackcat_cli._cli_context import CLIContext",
            "import blackcat_cli._ask_user_types",
            "import blackcat_cli.textual_adapter",
            "import blackcat_cli.tool_display",
            "import blackcat_cli.file_ops",
        ],
        ids=[
            "main.parse_args",
...
class TestCLIStartupTime:
    ...
    def _time_cli_command(args: str) -> float:
        """Return wall-clock seconds to run `python -m blackcat_cli <args>`.
        ...
        code = f"""
            ...
            subprocess.run(
                [sys.executable, "-m", "blackcat_cli", {args!r}],
                capture_output=True,
                text=True,
                timeout=30,
            )
    ...
    def test_help_under_threshold(self) -> None:
        """`blackcat --help` should complete well under 1 s.

        Catches regressions where a heavy import is accidentally re-added at
        module level.
        """
        elapsed = self._time_cli_command("--help")
        assert elapsed < 1, f"`blackcat --help` took {elapsed:.2f}s — expected < 1s"

    def test_version_under_threshold(self) -> None:
        """`blackcat --version` should complete well under 1 s."""
        elapsed = self._time_cli_command("--version")
        assert elapsed < 1, (
            f"`blackcat --version` took {elapsed:.2f}s — expected < 1s"
        )
...
class TestImportTiming:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "blackcat_cli.ui",
            "blackcat_cli.config",
            "blackcat_cli.skills.commands",
            "blackcat_cli.tool_display",
        ],
        ids=[
            "main",
            "ui",
...
class TestDeferredImportsWork:
    ...
    def test_agent_import_loads_protohello(self) -> None:
        """Importing ``blackcat_cli.agent`` should pull in protohello."""
        loaded = _get_loaded_modules("import blackcat_cli.agent")
        protohello_modules = {m for m in loaded if m.startswith("protohello")}
        assert protohello_modules, (
            "`blackcat_cli.agent` should transitively load `protohello` modules"
        )

    def test_sessions_import_available(self) -> None:
        """`blackcat_cli.sessions` should be importable."""
        result = _run_python("import blackcat_cli.sessions")
        assert result.returncode == 0, (
            f"Cannot import `blackcat_cli.sessions`:\n{result.stderr}"
        )

    def test_configurable_model_middleware_loads_protohello(self) -> None:
        """Accessing `ConfigurableModelMiddleware` should trigger protohello import."""
        loaded = _get_loaded_modules(
            "from blackcat_cli.configurable_model import ConfigurableModelMiddleware"
        )
        protohello_modules = {m for m in loaded if m.startswith("protohello")}
        assert protohello_modules, (
            "Accessing `ConfigurableModelMiddleware` should load protohello modules"
        )

    def test_ask_user_middleware_loads_protohello(self) -> None:
        """Accessing `AskUserMiddleware` should trigger protohello import."""
        loaded = _get_loaded_modules(
            "from blackcat_cli.ask_user import AskUserMiddleware"
        )
        protohello_modules = {m for m in loaded if m.startswith("protohello")}
        assert protohello_modules, (
            "Accessing `AskUserMiddleware` should load protohello modules"
        )
