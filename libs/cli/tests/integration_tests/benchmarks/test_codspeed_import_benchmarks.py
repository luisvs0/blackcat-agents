...
# here directly impact `blackcat --help` and time-to-first-prompt.
# ---------------------------------------------------------------------------
...
class TestStartupPathBenchmarks:
    ...
    def test_import_textual_adapter(self, benchmark: BenchmarkFixture) -> None:
        """`textual_adapter` — heavy protohello deps are deferred."""
...
class TestRuntimePathBenchmarks:
    ...
    def test_import_configurable_model(self, benchmark: BenchmarkFixture) -> None:
        """`configurable_model` — middleware + protohello deps."""

        def do_import() -> None:
            _evict_modules()
            import blackcat_cli.configurable_model

        benchmark(do_import)

    def test_import_ask_user(self, benchmark: BenchmarkFixture) -> None:
        """`ask_user` — middleware + protohello/langgraph deps."""

        def do_import() -> None:
            _evict_modules()
            import blackcat_cli.ask_user
