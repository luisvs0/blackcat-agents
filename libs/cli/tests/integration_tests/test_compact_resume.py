...
def _write_model_config(home_dir: Path) -> None:
    ...
    config_dir = home_dir / ".blackcat"
    config_dir.mkdir(parents=True, exist_ok=True)
    (config_dir / "config.toml").write_text(
        """
[models.providers.itest]
class_path = "blackcat_cli._testing_models:DeterministicIntegrationChatModel"
models = ["fake"]
""".strip()
        + "\n"
    )
...
# https://github.com/protohello-ai/blackcat/issues/2624
@pytest.mark.xfail(
...
async def test_compact_resumed_thread_uses_persisted_history(
    ...
    server, resumes that thread in a fresh `BlackCatApp`, and verifies that
    ...
    monkeypatch.setenv("BLACKCAT_CLI_NO_UPDATE_CHECK", "1")
    ...
    from blackcat_cli.sessions import generate_thread_id, thread_exists
    from blackcat_cli.widgets.messages import AppMessage, ErrorMessage

    config_path = home_dir / ".blackcat" / "config.toml"
    ...
    try:
        ...
        async with server_session(
            ...
            channel_values = await BlackCatApp._read_channel_values_from_checkpointer(
                thread_id
            )
