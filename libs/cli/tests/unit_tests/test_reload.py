...
from blackcat_cli.command_registry import SLASH_COMMANDS
...
_RELOAD_ENV_KEYS = (
    ...
    "ANTHROPIC_API_KEY",
    "BLACKCAT_CLI_ANTHROPIC_API_KEY",
    "GOOGLE_API_KEY",
    "BLACKCAT_CLI_GOOGLE_API_KEY",
    "NVIDIA_API_KEY",
    "BLACKCAT_CLI_NVIDIA_API_KEY",
    "TAVILY_API_KEY",
    "BLACKCAT_CLI_TAVILY_API_KEY",
    "GOOGLE_CLOUD_PROJECT",
    "BLACKCAT_CLI_GOOGLE_CLOUD_PROJECT",
    "BLACKCAT_CLI_LANGSMITH_PROJECT",
    "BLACKCAT_CLI_SHELL_ALLOW_LIST",
)
...
class TestReloadFromEnvironment:
    ...
    def _stub_dotenv_load(
        ...
        monkeypatch.setattr(
            "blackcat_cli.config._GLOBAL_DOTENV_PATH",
    ...
    def test_preserves_model_state(
        ...
        settings.user_protohello_project = "my-project"
        ...
        assert settings.user_protohello_project == "my-project"
    ...
    def test_updates_shell_allow_list(
        ...
        monkeypatch.setenv("BLACKCAT_CLI_SHELL_ALLOW_LIST", "ls,cat")
        settings = Settings.from_environment(start_path=tmp_path)
        assert settings.shell_allow_list == ["ls", "cat"]

        monkeypatch.setenv("BLACKCAT_CLI_SHELL_ALLOW_LIST", "ls,grep")
        changes = settings.reload_from_environment(start_path=tmp_path)
    ...
    def test_loads_global_dotenv(
        ...
        monkeypatch.setattr("blackcat_cli.config._GLOBAL_DOTENV_PATH", global_env)
    ...
    def test_global_dotenv_oserror_does_not_crash(
        ...
        monkeypatch.setattr("blackcat_cli.config._GLOBAL_DOTENV_PATH", broken)
        ...
        with caplog.at_level(logging.WARNING, logger="blackcat_cli.config"):
            settings.reload_from_environment(start_path=tmp_path)
    ...
    def test_project_dotenv_beats_global(
        ...
        from blackcat_cli.config import _load_dotenv
    ...
    def test_shell_env_beats_global_dotenv(
        ...
        """Shell-exported vars should beat global `~/.blackcat/.env`."""
        from blackcat_cli.config import _load_dotenv
        ...
        # No project .env
        monkeypatch.setattr(
            "blackcat_cli.config._find_dotenv_from_start_path",
            lambda _: None,
        )
    ...
    def test_multiple_simultaneous_changes(
        ...
        monkeypatch.setenv("BLACKCAT_CLI_SHELL_ALLOW_LIST", "ls")
        changes = settings.reload_from_environment(start_path=tmp_path)
    ...
    def test_prefixed_env_var_beats_canonical(
        ...
        """BLACKCAT_CLI_ prefixed var should override canonical on reload."""
        settings = Settings.from_environment(start_path=tmp_path)
    ...
    def test_from_environment_uses_prefixed_var(
        ...
        """Settings.from_environment should honour the BLACKCAT_CLI_ prefix."""
        monkeypatch.setenv("OPENAI_API_KEY", "sk-canonical")
        monkeypatch.setenv("BLACKCAT_CLI_OPENAI_API_KEY", "sk-override")
...
class TestReloadErrorPaths:
    ...
    def test_invalid_shell_allow_list_keeps_previous(
        ...
        monkeypatch.setenv("BLACKCAT_CLI_SHELL_ALLOW_LIST", "all,ls")
        changes = settings.reload_from_environment(start_path=tmp_path)
    ...
    def test_deleted_cwd_keeps_previous_project_root(
        ...
        monkeypatch.setattr(
            "blackcat_cli.project_utils.find_project_root", _raise_oserror
        )
        changes = settings.reload_from_environment(start_path=tmp_path)
