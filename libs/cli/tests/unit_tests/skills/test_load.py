...
from blackcat_cli._version import __version__ as _cli_version
from blackcat_cli.config import Settings
from blackcat_cli.skills.load import list_skills
...
class TestListSkillsAliasDirectories:
    ...
    def test_user_agent_skills_dir_precedence(self, tmp_path: Path) -> None:
        """Test that `~/.agents/skills` overrides `~/.blackcat/agent/skills`."""
        user_blackcat_dir = tmp_path / "user_blackcat_skills"
        user_agent_dir = tmp_path / "user_agent_skills"

        # Create same skill in both directories
        _create_skill(
            user_blackcat_dir / "shared-skill",
            "shared-skill",
            "From blackcat user dir",
        )
        _create_skill(
        ...
        skills = list_skills(
            user_skills_dir=user_blackcat_dir,
    ...
    def test_project_agent_skills_dir_precedence(self, tmp_path: Path) -> None:
        """Test that `.agents/skills` overrides `.blackcat/skills`."""
        project_blackcat_dir = tmp_path / "project_blackcat_skills"
        project_agent_dir = tmp_path / "project_agent_skills"

        # Create same skill in both directories
        _create_skill(
            project_blackcat_dir / "shared-skill",
            "shared-skill",
            "From blackcat project dir",
        )
        _create_skill(
        ...
        skills = list_skills(
            ...
            project_skills_dir=project_blackcat_dir,
    ...
    def test_mixed_sources_with_aliases(self, tmp_path: Path) -> None:
        ...
        _create_skill(
            user_blackcat_dir / "skill-a",
            "skill-a",
            "Skill A from blackcat user",
        )
        _create_skill(
        ...
        _create_skill(
            project_blackcat_dir / "skill-c",
            "skill-c",
            "Skill C from blackcat project",
        )
        _create_skill(
...
class TestListSkillsBuiltIn:
    ...
    def test_real_skill_creator_ships(self) -> None:
        ...
        assert creator["compatibility"] == "designed for blackcat-cli"
        assert "blackcat-cli-version" in creator["metadata"]
        assert creator["metadata"]["blackcat-cli-version"] == _cli_version
    ...
    def test_real_remember_skill_ships(self) -> None:
        ...
        assert remember["compatibility"] == "designed for blackcat-cli"
        assert "blackcat-cli-version" in remember["metadata"]
        assert remember["metadata"]["blackcat-cli-version"] == _cli_version
    ...
    def test_oserror_in_one_source_does_not_break_others(self, tmp_path: Path) -> None:
        ...
        original_list = __import__(
            "blackcat.middleware.skills", fromlist=["_list_skills"]
        )._list_skills
        ...
        with patch("blackcat_cli.skills.load.list_skills_from_backend", patched_list):
            skills = list_skills(
                built_in_skills_dir=built_in_dir,
                user_skills_dir=user_dir,
                project_skills_dir=None,
