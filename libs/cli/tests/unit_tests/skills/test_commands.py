...
import pytest
from blackcat.middleware.skills import SkillMetadata, _parse_skill_metadata
...
class TestSkillsHelpFlag:
    """Test that `blackcat skills -h` shows skills-specific help."""

    def test_skills_help_shows_subcommands(self) -> None:
        """Running `blackcat skills -h` should show skills subcommands.
        ...
        with (
            patch("sys.argv", ["blackcat", "skills", "-h"]),
    ...
    def test_skills_list_help_shows_list_options(self) -> None:
        """Running `blackcat skills list -h` should show list-specific options."""
        buf = io.StringIO()
        test_console = Console(file=buf, highlight=False, width=120)

        with (
            patch("sys.argv", ["blackcat", "skills", "list", "-h"]),
            patch("blackcat_cli.ui.console", test_console),
            pytest.raises(SystemExit) as exc_info,
        ):
            parse_args()
...
class TestThreadsHelpFlag:
    """Test that `blackcat threads -h` shows threads-specific help."""

    def test_threads_help_shows_threads_content(self) -> None:
        """Running `blackcat threads -h` should show threads subcommands.
        ...
        with (
            patch("sys.argv", ["blackcat", "threads", "-h"]),
            patch("blackcat_cli.ui.console", test_console),
            pytest.raises(SystemExit) as exc_info,
        ):
            parse_args()
...
class TestThreadsListAlias:
    """Test that `blackcat threads ls` is parsed as a `list` alias."""

    def test_threads_ls_alias_parsed(self) -> None:
        """Verify `threads ls` sets threads_command to 'ls'."""
        with patch("sys.argv", ["blackcat", "threads", "ls"]):
            args = parse_args()
    ...
    def test_threads_list_still_works(self) -> None:
        ...
        with patch("sys.argv", ["blackcat", "threads", "list"]):
            args = parse_args()
        assert args.command == "threads"
...
class TestSkillsListAlias:
    """Test that `blackcat skills ls` is parsed as a `list` alias."""

    def test_skills_ls_alias_parsed(self) -> None:
        """Verify `skills ls` sets skills_command to 'ls'."""
        with patch("sys.argv", ["blackcat", "skills", "ls"]):
            args = parse_args()
    ...
    def test_skills_list_still_works(self) -> None:
        ...
        with patch("sys.argv", ["blackcat", "skills", "list"]):
            args = parse_args()
        assert args.command == "skills"
...
class TestInfoShadowWarning:
    ...
    def test_shadow_note_shown_when_project_overrides_user(
        ...
        mock_settings = patch(
            "blackcat_cli.config.Settings.from_environment",
            return_value=type(
                "FakeSettings",
                (),
                {
        ...
        with (
            mock_settings,
            patch("blackcat_cli.config.console") as mock_console,
        ):
...
class TestDeleteSkill:
    ...
    def test_delete_existing_skill_with_force(self, tmp_path: Path) -> None:
        ...
        user_skills_dir = tmp_path / ".blackcat" / "agent" / "skills"
        ...
        with patch("blackcat_cli.config.Settings") as mock_settings_cls:
    ...
    def test_delete_nonexistent_skill(self, tmp_path: Path) -> None:
        ...
        with (
            patch("blackcat_cli.config.Settings") as mock_settings_cls,
            patch("blackcat_cli.config.console") as mock_console,
        ):
            mock_settings_cls.from_environment.return_value = mock_settings
            mock_console.print = capture_print
    ...
    def test_delete_project_skill(self, tmp_path: Path) -> None:
        ...
        project_skills_dir = tmp_path / "project" / ".blackcat" / "skills"
        skill_dir = self._create_test_skill(project_skills_dir, "project-skill")
        assert skill_dir.exists()

        mock_settings = MagicMock()
        user_dir = tmp_path / ".blackcat" / "agent" / "skills"
    ...
    def test_delete_skill_for_specific_agent(self, tmp_path: Path) -> None:
        ...
        agent1_skills_dir = tmp_path / ".blackcat" / "agent1" / "skills"
        agent2_skills_dir = tmp_path / ".blackcat" / "agent2" / "skills"
...
class TestDeleteArgparsing:
    ...
    def test_delete_args_defaults(self) -> None:
        ...
        with patch("sys.argv", ["blackcat", "skills", "delete", "my-skill"]):
            args = parse_args()
    ...
    def test_delete_help_shows_delete_options(self) -> None:
        """Running `blackcat skills delete -h` should show delete options."""
        buf = io.StringIO()
        test_console = Console(file=buf, highlight=False, width=120)

        with (
            patch("sys.argv", ["blackcat", "skills", "delete", "-h"]),
            patch("blackcat_cli.ui.console", test_console),
            pytest.raises(SystemExit) as exc_info,
        ):
            parse_args()
