...
from blackcat_cli.main import apply_stdin_pipe, parse_args
...
def mock_argv() -> MockArgvType:
    ...
    def _mock_argv(*args: str) -> AbstractContextManager[object]:
        return patch.object(sys, "argv", ["blackcat", *args])
...
class TestNoStreamArgument:
    ...
    def test_requires_non_interactive(self) -> None:
        ...
        with (
            patch.object(sys, "argv", ["blackcat", "--no-stream"]),
...
class TestQuietRequiresNonInteractive:
    ...
    def test_quiet_without_non_interactive_exits(self) -> None:
        ...
        mock_stdin.isatty.return_value = True
        with (
            patch.object(sys, "argv", ["blackcat", "-q"]),
            patch.object(sys, "stdin", mock_stdin),
            pytest.raises(SystemExit) as exc_info,
        ):
            cli_main()
    ...
    def test_skill_allowed_with_non_interactive(self) -> None:
        ...
        with (
            patch.object(
                sys,
                "argv",
                ["blackcat", "--skill", "code-review", "-n", "review this"],
            ),
            patch.object(sys, "stdin", mock_stdin),
            patch("blackcat_cli.main.check_optional_tools", return_value=[]),
            patch(
                "blackcat_cli.non_interactive.run_non_interactive",
    ...
    def test_skill_with_no_stream_without_non_interactive_exits_2(self) -> None:
        ...
        with (
            patch.object(
                sys,
                "argv",
                ["blackcat", "--skill", "code-review", "--no-stream"],
            ),
            patch.object(sys, "stdin", mock_stdin),
            pytest.raises(SystemExit) as exc_info,
        ):
...
class TestProfileOverrideArgument:
    ...
    def test_invalid_json_exits(self) -> None:
        ...
        mock_stdin.isatty.return_value = True
        with (
            patch.object(sys, "argv", ["blackcat", "--profile-override", "{bad"]),
            patch.object(sys, "stdin", mock_stdin),
            pytest.raises(SystemExit) as exc_info,
        ):
            cli_main()
    ...
    def test_non_dict_json_exits(self) -> None:
        ...
        mock_stdin.isatty.return_value = True
        with (
            patch.object(sys, "argv", ["blackcat", "--profile-override", "[1,2]"]),
            patch.object(sys, "stdin", mock_stdin),
            pytest.raises(SystemExit) as exc_info,
        ):
            cli_main()
