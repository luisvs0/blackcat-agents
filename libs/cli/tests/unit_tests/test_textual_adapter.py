...
from blackcat_cli import config as config_module
from blackcat_cli.config import build_stream_config
from blackcat_cli.textual_adapter import (
    ModelStats,
    SessionStats,
    TextualUIAdapter,
...
from blackcat_cli.widgets.messages import SummarizationMessage
...
class TestInterruptCleanup:
    ...
    async def test_tool_only_interrupt_marks_tokens_approximate(self) -> None:
        ...
        with patch("blackcat_cli.textual_adapter.time.monotonic", return_value=101.0):
...
class TestBuildStreamConfig:
    ...
    def test_git_branch_included_when_available(self) -> None:
        ...
        with patch(
            "blackcat_cli.config._get_git_branch",
    ...
    def test_versions_contains_cli_version(self) -> None:
        ...
        assert config["metadata"]["versions"]["blackcat-cli"] == __version__

    def test_versions_contains_sdk_version_when_installed(self) -> None:
        """SDK version should be in versions when blackcat is installed."""
        with patch(
            "importlib.metadata.version",
            return_value="0.5.0",
        ):
            config = build_stream_config("t-sdk", assistant_id=None)
        assert config["metadata"]["versions"]["blackcat"] == "0.5.0"

    def test_versions_omits_sdk_when_not_installed(self) -> None:
        """SDK version key should be absent when blackcat is not installed."""
        from importlib.metadata import PackageNotFoundError

        with patch(
            "importlib.metadata.version",
            side_effect=PackageNotFoundError("blackcat"),
        ):
            config = build_stream_config("t-nosdk", assistant_id=None)
        assert "blackcat" not in config["metadata"]["versions"]
        from blackcat_cli._version import __version__

        assert config["metadata"]["versions"]["blackcat-cli"] == __version__

    def test_user_id_included_when_set(self) -> None:
        """BLACKCAT_CLI_USER_ID should appear in metadata when set."""
        with patch.dict("os.environ", {"BLACKCAT_CLI_USER_ID": "mason"}):
    ...
    def test_user_id_absent_when_unset(self) -> None:
        ...
        with patch.dict("os.environ", {"BLACKCAT_CLI_USER_ID": ""}):
