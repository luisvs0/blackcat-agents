...
1. Every `BLACKCAT_CLI_*` constant in `_env_vars.py` has a matching
...
2. No source file outside `_env_vars.py` uses a bare string literal like
   `"BLACKCAT_CLI_FOO"` -- it must import the constant instead.
"""
...
import blackcat_cli._env_vars as _mod

_SRC_DIR = Path(__file__).resolve().parents[2] / "blackcat_cli"
_REGISTRY_FILE = _SRC_DIR / "_env_vars.py"

# Matches a full BLACKCAT_CLI_* env var name inside quote characters.
# The [A-Z] after the prefix avoids matching the bare prefix constant
# (_ENV_PREFIX = "BLACKCAT_CLI_") in model_config.py.
_ENV_VAR_RE = re.compile(r"""["'](BLACKCAT_CLI_[A-Z][A-Z0-9_]+)["']""")
...
class TestEnvVarRegistryDrift:
    ...
    def test_no_bare_literals_outside_registry(self) -> None:
        ...
        assert not hits, (
            "Bare BLACKCAT_CLI_* string literals found in source "
            "(import from blackcat_cli._env_vars instead):\n"
    ...
    def test_no_stale_registry_entries(self) -> None:
        ...
        assert not stale, (
            ...
            "Remove stale entries from blackcat_cli/_env_vars.py."
        )
