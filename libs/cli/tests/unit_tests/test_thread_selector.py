...
from blackcat_cli.app import BlackCatApp
...
def _patch_columns(columns: dict[str, bool] | None = None) -> Any:  # noqa: ANN401
    ...
    from blackcat_cli.model_config import THREAD_COLUMN_DEFAULTS, ThreadConfig
    ...
    def _ctx() -> Any:  # noqa: ANN401
        with (
            patch(
                "blackcat_cli.model_config.load_thread_columns",
                return_value=dict(cols),
            ),
            patch(
                "blackcat_cli.model_config.load_thread_sort_order",
                return_value="updated_at",
            ),
            patch(
                "blackcat_cli.model_config.load_thread_config",
    ...
    async def test_click_selects_thread(self) -> None:
        ...
        with _patch_list_threads():
            ...
            async with app.run_test() as pilot:
                ...
                from blackcat_cli.widgets.thread_selector import ThreadOption
...
_WEBBROWSER_OPEN = "blackcat_cli.widgets._links.webbrowser.open"
...
class TestThreadSelectorBuildTitle:
    ...
    def test_current_thread_with_url(self) -> None:
        ...
        screen = ThreadSelectorScreen(current_thread="abc12345")
        title = screen._build_title(
            thread_url="https://smith.protohello.com/p/t/abc12345"
        )
        ...
        from blackcat_cli.theme import DARK_COLORS
...
class TestFetchThreadUrl:
    ...
    async def test_successful_url_updates_title(self) -> None:
        ...
        with (
            _patch_list_threads(),
            patch(
                "blackcat_cli.widgets.thread_selector.build_langsmith_thread_url",
                return_value="https://smith.protohello.com/p/t/abc12345",
            ),
        ):
            app = ThreadSelectorTestApp(current_thread="abc12345")
            async with app.run_test() as pilot:
    ...
    async def test_timeout_leaves_title_unchanged(self) -> None:
        ...
        with (
            _patch_list_threads(),
            patch(
                "blackcat_cli.widgets.thread_selector._URL_FETCH_TIMEOUT",
                0.01,
            ),
            patch(
                "blackcat_cli.widgets.thread_selector.build_langsmith_thread_url",
...
class TestThreadSelectorColumnHeader:
    ...
    def test_header_contains_default_column_names(self) -> None:
        ...
        from blackcat_cli.widgets.thread_selector import _format_header_label
...
class TestThreadSelectorLimit:
    ...
    async def test_custom_limit_is_forwarded(self) -> None:
        ...
        with (
            patch(
                "blackcat_cli.sessions.get_thread_limit",
                return_value=5,
            ),
    ...
    async def test_checkpoint_details_are_loaded_for_initial_render(self) -> None:
        ...
        with (
            ...
            _patch_columns(),
            patch(
                "blackcat_cli.sessions.populate_thread_checkpoint_details",
                new_callable=AsyncMock,
    ...
    async def test_cached_counts_skip_background_population(self) -> None:
        ...
        with (
            ...
            patch(
                "blackcat_cli.sessions.apply_cached_thread_message_counts",
...
class TestThreadSelectorCheckpointDetailErrors:
    ...
    async def test_unexpected_checkpoint_detail_error_logs_warning(self) -> None:
        ...
        with (
            ...
            patch(
                "blackcat_cli.widgets.thread_selector.logger.warning"
            ) as mock_warning,
        ):
...
class TestThreadSelectorPrefetchedRows:
    ...
    async def test_prefetched_prompt_is_preserved_during_refresh(self) -> None:
        ...
        from blackcat_cli import sessions
...
class TestThreadSelectorDelete:
    ...
    async def test_delete_keeps_selection_on_next_thread(self) -> None:
        ...
        with (
            _patch_list_threads(),
            patch(
                "blackcat_cli.sessions.delete_thread",
                new_callable=AsyncMock,
                return_value=None,
            ),
        ):
...
class TestThreadSelectorColumnConfig:
    ...
    def test_default_columns(self) -> None:
        ...
        from blackcat_cli.model_config import THREAD_COLUMN_DEFAULTS
    ...
    async def test_switch_toggles_column_and_persists(self) -> None:
        ...
        with (
            ...
            _patch_columns(),
            patch(
                "blackcat_cli.model_config.save_thread_columns",
                return_value=True,
            ) as mock_save,
        ):
            app = ThreadSelectorTestApp()
...
class TestResumeThread:
    """Tests for BlackCatApp._resume_thread."""
    ...
    async def test_successful_switch_updates_ids(self) -> None:
        ...
        app = BlackCatApp(thread_id="old-thread")
        app._agent = MagicMock()
        app._session_state = MagicMock()
    ...
    async def test_failure_restores_previous_thread_ids(self) -> None:
        ...
        app._pending_messages = MagicMock()
        app._queued_widgets = MagicMock()
        from blackcat_cli.app import _ThreadHistoryPayload
...
class TestFetchThreadHistoryData:
    """Tests for BlackCatApp._fetch_thread_history_data."""
    ...
    async def test_offloads_conversion_to_thread(self) -> None:
        ...
        with patch(
            "blackcat_cli.app.asyncio.to_thread",
            new_callable=AsyncMock,
...
class TestLoadThreadHistory:
    """Tests for BlackCatApp._load_thread_history."""
    ...
    async def test_preloaded_history_skips_fetch_and_schedules_link(self) -> None:
        ...
        app = BlackCatApp(thread_id="tid-1")
        app._agent = MagicMock()
        fetch_history_mock = AsyncMock()
        mount_message_mock = AsyncMock()
    ...
    async def test_assistant_render_failure_does_not_abort_history_load(self) -> None:
        ...
        from blackcat_cli.widgets.message_store import MessageData, MessageType
        from blackcat_cli.widgets.messages import AssistantMessage
    ...
    async def test_early_return_without_thread_id_logs_debug(self) -> None:
        ...
        with patch("blackcat_cli.app.logger.debug") as debug_mock:
            await app._load_thread_history()
...
class TestUpgradeThreadMessageLink:
    """Tests for BlackCatApp._upgrade_thread_message_link."""
...
class TestBuildThreadMessage:
    ...
    async def test_hyperlinked_when_tracing_configured(self) -> None:
        ...
        app = BlackCatApp()
        url = "https://smith.protohello.com/o/org/projects/p/proj/t/tid-123"
        target = "blackcat_cli.config.build_langsmith_thread_url"
    ...
    async def test_fallback_on_timeout(self) -> None:
        ...
        app = BlackCatApp()
        with patch(
            "blackcat_cli.app.asyncio.wait_for",
...
class TestConvertMessagesToData:
    """Tests for BlackCatApp._convert_messages_to_data."""
    ...
    def _make_ai(
        ...
        """Create an AIMessage."""
        from protohello_core.messages import AIMessage
    ...
    def _make_tool(
        ...
        from protohello_core.messages import ToolMessage
    ...
    def test_human_message_conversion(self) -> None:
        ...
        result = BlackCatApp._convert_messages_to_data(msgs)
    ...
    def test_tool_call_matching(self) -> None:
        ...
        from blackcat_cli.widgets.message_store import MessageType, ToolStatus
    ...
    def test_tool_call_error_status(self) -> None:
        ...
        from blackcat_cli.widgets.message_store import ToolStatus
    ...
    def test_empty_messages(self) -> None:
        ...
        result = BlackCatApp._convert_messages_to_data([])
        assert result == []
    ...
    def test_skill_message_from_additional_kwargs(self) -> None:
        ...
        result = BlackCatApp._convert_messages_to_data([msg])
