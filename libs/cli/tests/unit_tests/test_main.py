...
from blackcat_cli.app import AppResult, BlackCatApp, run_textual_app
from blackcat_cli.config import build_langsmith_thread_url, reset_langsmith_url_cache
...
class TestLangSmithTeardownUrl:
    ...
    def test_thread_url_not_shown_when_no_checkpoints(self) -> None:
        ...
        thread_url = "https://smith.protohello.com/o/org/projects/p/proj/t/abc"
        thread_exists = False
        show_link = bool(thread_url and thread_exists)
        assert not show_link
...
class TestRunTextualCliAsyncMcp:
    ...
    async def test_passes_server_and_mcp_kwargs_to_textual_app(self) -> None:
        ...
        with patch("blackcat_cli.app.run_textual_app", new=_run_textual_app_stub):
...
class TestServerCleanupLifecycle:
    ...
    async def test_deferred_server_proc_stopped_after_app_exits(self) -> None:
        ...
        async def _fake_run_async(self: BlackCatApp) -> None:  # noqa: RUF029
...
class TestCheckOptionalTools:
    ...
    def test_returns_tool_name_when_rg_not_found(self) -> None:
        ...
        with patch("blackcat_cli.main.shutil.which", return_value=None):
    ...
    def test_returns_empty_when_rg_found(self) -> None:
        ...
        with patch("blackcat_cli.main.shutil.which", return_value="/usr/bin/rg"):
            missing = check_optional_tools()
    ...
    def test_returns_tavily_when_key_missing(self) -> None:
        ...
        with (
            patch("blackcat_cli.main.shutil.which", return_value="/usr/bin/rg"),
            patch(
                "blackcat_cli.config.settings",
...
class TestRipgrepInstallHint:
    ...
    def test_macos_brew(self) -> None:
        ...
        with (
            patch("blackcat_cli.main.sys") as mock_sys,
            patch("blackcat_cli.main.shutil.which", side_effect=_which),
        ):
    ...
    def test_linux_no_manager_falls_through(self) -> None:
        ...
        with (
            patch("blackcat_cli.main.sys") as mock_sys,
            patch("blackcat_cli.main.shutil.which", return_value=None),
        ):
            mock_sys.platform = "linux"
...
class TestFormatToolWarnings:
    ...
    def test_tui_format_contains_install_hint(self) -> None:
        ...
        hint_patch = patch(
            "blackcat_cli.main._ripgrep_install_hint",
...
class TestRunTextualCliAsyncModelConfigError:
    ...
    async def test_returns_error_code_on_no_credentials(self) -> None:
        ...
        with (
            patch(
                "blackcat_cli.config._get_default_model_spec",
                side_effect=ModelConfigError("No credentials configured"),
            ),
            patch("blackcat_cli.config._get_console") as mock_console_fn,
        ):
    ...
    async def test_no_error_when_model_name_provided(self) -> None:
        ...
        with patch("blackcat_cli.app.run_textual_app", new=_stub):
