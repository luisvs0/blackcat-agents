...
from blackcat_cli import model_config
from blackcat_cli._env_vars import SERVER_ENV_PREFIX
...
from blackcat_cli.model_config import ModelConfigError, clear_caches
...
class TestProjectAgentMdFinding:
    ...
    def test_find_agent_md_in_blackcat_dir(self, tmp_path: Path) -> None:
        """Test finding AGENTS.md in .blackcat/ directory."""
        project_root = tmp_path / "project"
        project_root.mkdir()

        # Create .blackcat/AGENTS.md
        blackcat_dir = project_root / ".blackcat"
        blackcat_dir.mkdir()
        agent_md = blackcat_dir / "AGENTS.md"
    ...
    def test_find_agent_md_in_root(self, tmp_path: Path) -> None:
        ...
        # Create root-level AGENTS.md (no .blackcat/)
    ...
    def test_both_agent_md_files_combined(self, tmp_path: Path) -> None:
        ...
        blackcat_dir = project_root / ".blackcat"
        blackcat_dir.mkdir()
        blackcat_md = blackcat_dir / "AGENTS.md"
        blackcat_md.write_text("In .blackcat/")

        root_md = project_root / "AGENTS.md"
        root_md.write_text("In root")

        # Should return both, with .blackcat/ first
        result = _find_project_agent_md(project_root)
        assert len(result) == 2
        assert result[0] == blackcat_md
    ...
    def test_skips_paths_with_permission_errors(self, tmp_path: Path) -> None:
        ...
        def patched_exists(self: Path) -> bool:
            if self.name == "AGENTS.md" and ".blackcat" in str(self):
...
class TestSettingsGetProjectAgentMdPath:
    ...
    def test_returns_existing_paths(self, tmp_path: Path) -> None:
        ...
        blackcat_dir = tmp_path / ".blackcat"
        blackcat_dir.mkdir()
        blackcat_md = blackcat_dir / "AGENTS.md"
        blackcat_md.write_text("inner")
        ...
        assert result == [blackcat_md, root_md]
...
class TestNewlineShortcut:
    ...
    def test_returns_option_enter_on_macos(self) -> None:
        ...
        with patch("blackcat_cli.config.sys.platform", "darwin"):
    ...
    def test_returns_ctrl_j_on_non_macos(self) -> None:
        ...
        with patch("blackcat_cli.config.sys.platform", "linux"):
...
class TestValidateModelCapabilities:
    ...
    @patch("blackcat_cli.config.console")
...
class TestCreateModelProfileExtraction:
    ...
    def _bypass_credential_check(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(
            "blackcat_cli.model_config.has_provider_credentials", lambda _: True
        )

    @patch("protohello.chat_models.init_chat_model")
...
""")
        ...
        with (
            ...
            caplog.at_level(logging.WARNING, logger="blackcat_cli.config"),
        ):
            result = create_model("anthropic:claude-sonnet-4-5")
...
class TestGetLangsmithProjectName:
    ...
    def test_returns_project_from_settings(self) -> None:
        """Should prefer settings.blackcat_protohello_project."""
        env = {
            "LANGSMITH_API_KEY": "lsv2_test",
            "LANGSMITH_TRACING": "true",
        ...
        with (
            ...
            patch("blackcat_cli.config.settings") as mock_settings,
        ):
            mock_settings.blackcat_protohello_project = "settings-project"
    ...
    def test_falls_back_to_env_project(self) -> None:
        ...
        with (
            ...
            mock_settings.blackcat_protohello_project = None
            assert get_langsmith_project_name() == "env-project"

    def test_falls_back_to_default(self) -> None:
        """Should fall back to 'blackcat-cli' when no project name configured."""
        env = {
            "LANGSMITH_API_KEY": "lsv2_test",
            "LANGSMITH_TRACING": "true",
        }
        with (
            ...
            mock_settings.blackcat_protohello_project = None
            assert get_langsmith_project_name() == "blackcat-cli"

    def test_accepts_protohello_api_key(self) -> None:
        """Should accept PROTOHELLO_API_KEY as alternative to LANGSMITH_API_KEY."""
        env = {
            "LANGSMITH_API_KEY": "",
            "PROTOHELLO_API_KEY": "lsv2_test",
            "LANGSMITH_TRACING": "true",
        }
        with (
            patch.dict("os.environ", env, clear=False),
...
class TestFetchLangsmithProjectUrl:
    ...
    def test_returns_url_on_success(self) -> None:
        ...
        class FakeProject:
            url = "https://smith.protohello.com/o/org/projects/p/proj"
        ...
        assert result == "https://smith.protohello.com/o/org/projects/p/proj"
    ...
    def test_returns_none_when_lookup_times_out(self) -> None:
        ...
        with (
            patch(
                "blackcat_cli.config._LANGSMITH_URL_LOOKUP_TIMEOUT_SECONDS",
                0.01,
            ),
    ...
    def test_caches_result_after_first_call(self) -> None:
        ...
        assert first == "https://smith.protohello.com/o/org/projects/p/proj"
        assert second == first
    ...
    def test_different_project_name_fetches_again(self) -> None:
        ...
        class FakeProjectA:
            url = "https://smith.protohello.com/o/org/projects/p/a"

        class FakeProjectB:
            url = "https://smith.protohello.com/o/org/projects/p/b"
        ...
        assert first == "https://smith.protohello.com/o/org/projects/p/a"
        assert second == "https://smith.protohello.com/o/org/projects/p/b"
        assert mock_client_cls.return_value.read_project.call_count == 2
...
class TestBuildLangsmithThreadUrl:
    ...
    def test_returns_url_when_configured(self) -> None:
        ...
        with (
            patch(
                "blackcat_cli.config.get_langsmith_project_name",
        ...
        assert (
            result
            == "https://smith.protohello.com/o/org/projects/p/proj/t/thread-123?utm_source=blackcat-cli"
        )
...
""")
    ...
    def test_prefixed_env_var_beats_canonical(self, tmp_path: Path) -> None:
        """BLACKCAT_CLI_ prefixed var overrides canonical in provider kwargs."""
        config_path = tmp_path / "config.toml"
        config_path.write_text("""
[models.providers.fireworks]
models = ["llama-v3p1-70b"]
        ...
        with (
            ...
            patch.dict(
                "os.environ",
                {
                    ...
                    "BLACKCAT_CLI_FIREWORKS_API_KEY": "prefixed",
                },
                clear=False,
            ),
        ):
...
class TestOpenRouterVersionCheck:
    ...
    def test_rejects_old_version(self) -> None:
        """_get_provider_kwargs raises ImportError for old protohello-openrouter."""
        with (
            patch(
                "blackcat._openrouter.pkg_version",
                return_value="0.0.1",
            ),
            pytest.raises(ImportError, match="protohello-openrouter>="),
        ):
    ...
    def test_accepts_sufficient_version(self) -> None:
        ...
        from blackcat._openrouter import OPENROUTER_MIN_VERSION
    ...
    def test_skipped_for_other_providers(self) -> None:
        ...
        with patch("blackcat._openrouter.check_openrouter_version") as mock:
...
class TestOpenRouterHeaders:
    ...
    def test_injects_attribution_kwargs(self) -> None:
        ...
        assert kwargs["app_url"] == "https://pypi.org/project/blackcat-cli/"
        assert kwargs["app_title"] == "Black Cat CLI"
...
class TestCreateModelFromClass:
    ...
    def test_instantiates_valid_subclass(self) -> None:
        ...
        from protohello_core.callbacks import CallbackManagerForLLMRun
        from protohello_core.language_models import BaseChatModel
        from protohello_core.messages import BaseMessage
        from protohello_core.outputs import ChatResult
...
""")
        ...
        with (
            patch.object(model_config, "DEFAULT_CONFIG_PATH", config_path),
            patch(
                "blackcat_cli.config._create_model_from_class",
...
""")
        ...
        with (
            ...
            patch(
                "blackcat_cli.config._create_model_via_init",
                return_value=mock_instance,
            ) as mock_init,
        ):
...
class TestCreateModelEdgeCaseParsing:
    ...
    @patch("blackcat_cli.config._get_default_model_spec")
    @patch("protohello.chat_models.init_chat_model")
...
class TestCreateModelViaInitImportError:
    ...
    def test_missing_package_error(self, mock_init: Mock) -> None:
        ...
        mock_init.side_effect = ImportError(
            "No module named 'protohello_nvidia_ai_endpoints'"
        )
        with (
    ...
    def test_installed_but_broken_import(self, mock_init: Mock) -> None:
        ...
        with (
            ...
            _create_model_via_init("nemotron", "nvidia", {})
        mock_find_spec.assert_called_once_with("protohello_nvidia_ai_endpoints")
    ...
    def test_unknown_provider_fallback_package_name(self, mock_init: Mock) -> None:
        """Unknown provider falls back to protohello-{provider} package name."""
        mock_init.side_effect = ImportError("no module")
        with (
            patch("importlib.util.find_spec", return_value=None),
            pytest.raises(
                ModelConfigError,
                match=r"pip install protohello-custom_provider",
            ),
        ):
...
class TestLazyModuleAttributes:
    ...
    def test_getattr_returns_settings(self) -> None:
        ...
        from blackcat_cli.config import _get_settings
    ...
    def test_getattr_returns_console(self) -> None:
        ...
        from blackcat_cli.config import _get_console
    ...
    def test_getattr_raises_for_unknown(self) -> None:
        ...
        import blackcat_cli.config as config_mod
    ...
    def test_ensure_bootstrap_is_idempotent(self) -> None:
        ...
        from blackcat_cli.config import _ensure_bootstrap
    ...
    def test_ensure_bootstrap_marks_done_on_failure(self) -> None:
        ...
        try:
            with patch(
                "blackcat_cli.config._load_dotenv", side_effect=RuntimeError("boom")
            ):
    ...
    def test_ensure_bootstrap_langsmith_override(
        ...
        """_ensure_bootstrap copies BLACKCAT_CLI_LANGSMITH_PROJECT."""
        import blackcat_cli.config as config_mod
        from blackcat_cli.config import _ensure_bootstrap
        ...
        try:
            monkeypatch.setenv("BLACKCAT_CLI_LANGSMITH_PROJECT", "my-agent-project")
            monkeypatch.delenv("LANGSMITH_PROJECT", raising=False)

            with (
                patch("blackcat_cli.config._load_dotenv"),
                patch(
                    "blackcat_cli.project_utils.get_server_project_context",
                    return_value=None,
                ),
            ):
                _ensure_bootstrap()
    ...
    def test_ensure_bootstrap_preserves_original_langsmith(
        ...
        try:
            ...
            monkeypatch.setenv("BLACKCAT_CLI_LANGSMITH_PROJECT", "agent-project")
    ...
    def test_bootstrap_propagates_prefixed_langsmith_vars(
        ...
        try:
            monkeypatch.setenv("BLACKCAT_CLI_LANGSMITH_API_KEY", "lsv2_test")
            monkeypatch.setenv("BLACKCAT_CLI_LANGSMITH_TRACING", "true")
            monkeypatch.delenv("LANGSMITH_API_KEY", raising=False)
            monkeypatch.delenv("LANGSMITH_TRACING", raising=False)
            monkeypatch.delenv("BLACKCAT_CLI_LANGSMITH_PROJECT", raising=False)
    ...
    def test_bootstrap_overrides_canonical_with_prefixed_value(
        ...
        try:
            ...
            monkeypatch.setenv("BLACKCAT_CLI_LANGSMITH_API_KEY", "lsv2_override")
            monkeypatch.delenv("BLACKCAT_CLI_LANGSMITH_PROJECT", raising=False)
    ...
    def test_bootstrap_propagates_empty_string(
        ...
        try:
            monkeypatch.setenv("BLACKCAT_CLI_LANGSMITH_TRACING", "")
            monkeypatch.delenv("LANGSMITH_TRACING", raising=False)
            monkeypatch.delenv("BLACKCAT_CLI_LANGSMITH_PROJECT", raising=False)
...
class TestFindDotenvFromStartPath:
    ...
    def test_finds_env_in_start_dir(self, tmp_path: Path) -> None:
        ...
        from blackcat_cli.config import _find_dotenv_from_start_path
