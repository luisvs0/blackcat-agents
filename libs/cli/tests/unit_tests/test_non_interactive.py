...
from blackcat_cli.config import SHELL_ALLOW_ALL, ModelResult
from blackcat_cli.non_interactive import (
...
class TestMakeHitlDecision:
    ...
    def test_shell_without_allow_list_rejected(self, console: Console) -> None:
        ...
        with patch("blackcat_cli.non_interactive.settings") as mock_settings:
...
class TestBuildNonInteractiveHeader:
    ...
    def test_thread_clickable_when_url_available(self) -> None:
        ...
        url = "https://smith.protohello.com/o/org/projects/p/proj/t/abc123"
        with patch("blackcat_cli.non_interactive.settings") as mock_settings:
            mock_settings.model_name = None
            with patch(
                "blackcat_cli.non_interactive.build_langsmith_thread_url",
                return_value=url,
            ):
...
class TestSandboxTypeForwarding:
    ...
    async def test_sandbox_type_passed_to_server(self) -> None:
        ...
        with (
            patch(
                "blackcat_cli.non_interactive.create_model",
            ...
            patch(
                "blackcat_cli.non_interactive.generate_thread_id",
                return_value="test-thread",
            ),
            patch(
                "blackcat_cli.non_interactive.settings",
            ) as mock_settings,
            patch(
                "blackcat_cli.non_interactive.build_langsmith_thread_url",
                return_value=None,
            ...
            patch(
                "blackcat_cli.server_manager.start_server_and_get_agent",
...
class TestQuietMode:
    ...
    async def test_console_creation(
        ...
        with (
            patch(
                "blackcat_cli.non_interactive.Console",
...
class TestFastFollowLangsmithLink:
    ...
    async def test_prints_link_when_lookup_ready(self) -> None:
        ...
        ready_state.url = (
            "https://smith.protohello.com/o/org/projects/p/proj/t/test-thread"
        )
        ...
        with (
            ...
            patch(
                "blackcat_cli.non_interactive._start_langsmith_thread_url_lookup",
...
class TestStartLangsmithThreadUrlLookup:
    ...
    def test_sets_url_on_success(self) -> None:
        ...
        url = "https://smith.protohello.com/o/org/projects/p/proj/t/tid"
        with patch(
            "blackcat_cli.non_interactive.build_langsmith_thread_url",
            return_value=url,
        ):
...
class TestNonInteractivePrompt:
    ...
    async def test_initial_skill_wraps_prompt_and_metadata(self) -> None:
        ...
        with (
            ...
            patch(
                "blackcat_cli.skills.invocation.discover_skills_and_roots",
                return_value=([skill], []),
            ),
            patch(
