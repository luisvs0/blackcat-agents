...
if TYPE_CHECKING:
    ...
    from blackcat_cli.sessions import ThreadInfo
...
class TestInitialPromptOnMount:
    ...
    async def test_initial_skill_runs_after_server_ready(self) -> None:
        ...
        app.on_black_cat_app_server_ready(
            app.ServerReady(
                agent=MagicMock(),
                server_proc=None,
                mcp_server_info=[],
...
class TestThreadCachePrewarm:
    ...
    async def test_prewarm_uses_current_thread_limit(self) -> None:
        ...
        app = BlackCatApp(agent=MagicMock(), thread_id="thread-123")

        with (
            patch("blackcat_cli.sessions.get_thread_limit", return_value=7),
            patch(
                "blackcat_cli.sessions.prewarm_thread_message_counts",
    ...
    async def test_show_thread_selector_uses_cached_rows(self) -> None:
        ...
        async with app.run_test() as pilot:
            await pilot.pause()
            with (
                patch("blackcat_cli.sessions.get_thread_limit", return_value=9),
                patch(
                    "blackcat_cli.sessions.get_cached_threads",
                    return_value=cached_threads,
                ),
                patch(
                    "blackcat_cli.widgets.thread_selector.ThreadSelectorScreen"
                ) as mock_screen_cls,
...
class TestAppBindings:
    ...
    def test_ctrl_c_binding_has_priority(self) -> None:
        ...
        bindings = [b for b in BlackCatApp.BINDINGS if isinstance(b, Binding)]
...
class TestITerm2CursorGuide:
    ...
    def test_write_iterm_escape_does_nothing_when_not_iterm(self) -> None:
        ...
        with (
            patch("blackcat_cli.app._IS_ITERM", False),
    ...
    def test_write_iterm_escape_writes_sequence_when_iterm(self) -> None:
        ...
        with (
            patch("blackcat_cli.app._IS_ITERM", True),
            patch("sys.__stderr__", mock_stderr),
        ):
            _write_iterm_escape(_ITERM_CURSOR_GUIDE_ON)
...
class TestModalScreenCtrlDHandling:
    ...
    async def test_ctrl_d_deletes_in_thread_selector_instead_of_quitting(self) -> None:
        ...
        with patch(
            "blackcat_cli.sessions.list_threads",
            new_callable=AsyncMock,
    ...
    async def test_ctrl_d_quits_from_mcp_viewer(self) -> None:
        ...
        from blackcat_cli.mcp_tools import MCPServerInfo, MCPToolInfo
...
class TestMessageQueue:
    """Test message queue behavior in BlackCatApp."""
...
class TestAskUserLifecycle:
    ...
    async def test_request_ask_user_timeout_cleans_old_widget(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            with patch("blackcat_cli.app._monotonic", side_effect=[0.0, 31.0]):
...
class TestTraceCommand:
    ...
    async def test_trace_opens_browser_when_configured(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            with (
                patch(
                    "blackcat_cli.config.build_langsmith_thread_url",
                    return_value="https://smith.protohello.com/o/org/projects/p/proj/t/test-thread-123",
                ),
                patch("blackcat_cli.app.webbrowser.open") as mock_open,
            ):
                await app._handle_trace_command("/trace")
                await pilot.pause()

            mock_open.assert_called_once_with(
                "https://smith.protohello.com/o/org/projects/p/proj/t/test-thread-123"
            )
    ...
    async def test_trace_shows_link_when_browser_fails(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            with (
                patch(
                    "blackcat_cli.config.build_langsmith_thread_url",
                    return_value="https://smith.protohello.com/t/test-thread-123",
                ),
                patch(
                    "blackcat_cli.app.webbrowser.open",
                    side_effect=webbrowser.Error("no browser"),
                ) as mock_open,
                patch("blackcat_cli.app.logger") as mock_logger,
            ):
                await app._handle_trace_command("/trace")
            ...
            app_msgs = app.query(AppMessage)
            assert any(
                "https://smith.protohello.com/t/test-thread-123" in str(w._content)
                for w in app_msgs
            )
    ...
    async def test_trace_defers_output_when_busy(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            with (
                ...
                patch("blackcat_cli.app.webbrowser.open"),
            ):
                await app._handle_trace_command("/trace")
                await pilot.pause()
...
class TestRunAgentTaskMediaTracker:
    ...
    async def test_run_agent_task_passes_image_tracker(self) -> None:
        ...
        app = BlackCatApp(agent=MagicMock())
        async with app.run_test() as pilot:
            await pilot.pause()
            assert app._ui_adapter is not None

            with patch(
                "blackcat_cli.textual_adapter.execute_task_textual",
                new_callable=AsyncMock,
            ) as mock_execute:
...
class TestShellCommandInterrupt:
    ...
    async def test_posix_killpg_called(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            with (
                patch("blackcat_cli.app.sys") as mock_sys,
                patch("os.killpg") as mock_killpg,
...
class TestDeferredShowApproval:
    ...
    async def test_timeout_shows_approval_after_deadline(self) -> None:
        ...
        with patch("blackcat_cli.app._DEFERRED_APPROVAL_TIMEOUT_SECONDS", 0.05):
            await app._deferred_show_approval(placeholder, menu, future)
...
class TestActionOpenEditor:
    ...
    async def test_updates_text_on_successful_edit(self) -> None:
        ...
        with (
            ...
            patch("blackcat_cli.editor.open_in_editor", return_value="edited"),
        ):
    ...
    async def test_no_update_when_editor_returns_none(self) -> None:
        ...
        with (
            patch.object(app, "suspend"),
            patch("blackcat_cli.editor.open_in_editor", return_value=None),
        ):
            await app.action_open_editor()
    ...
    async def test_notifies_on_exception(self) -> None:
        ...
        with (
            patch.object(app, "suspend"),
            patch(
                "blackcat_cli.editor.open_in_editor",
                side_effect=RuntimeError("boom"),
            ),
...
class TestFetchThreadHistoryData:
    ...
    async def test_dict_messages_converted_to_message_objects(self) -> None:
        ...
        from blackcat_cli.widgets.message_store import MessageData, MessageType
        ...
        app = BlackCatApp(agent=mock_agent, thread_id="t-1")
    ...
    async def test_server_mode_fallback_includes_context_tokens(self) -> None:
        ...
        from protohello_core.messages import HumanMessage

        from blackcat_cli.remote_client import RemoteAgent
        from blackcat_cli.widgets.message_store import MessageType
...
class TestRemoteAgent:
    """Tests for BlackCatApp._remote_agent()."""
...
class TestBypassFrozensetDrift:
    ...
    def _handled_commands(cls) -> set[str]:
        ...
        source = textwrap.dedent(inspect.getsource(BlackCatApp._handle_command))
    ...
    def test_all_handled_commands_are_classified(self) -> None:
        ...
        from blackcat_cli.command_registry import ALL_CLASSIFIED
...
class TestDeferredActions:
    ...
    async def test_deferred_actions_cleared_on_server_failure(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            app.on_black_cat_app_server_start_failed(
                BlackCatApp.ServerStartFailed(error=RuntimeError("test"))
            )
    ...
    async def test_server_failure_stores_error(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            app.on_black_cat_app_server_start_failed(
                BlackCatApp.ServerStartFailed(error=RuntimeError("exit code 3"))
            )
