...
from blackcat_cli.server import (
    ServerProcess,
    _find_free_port,
    _port_in_use,
...
class TestWaitForServerHealthy:
    ...
    async def test_raises_on_timeout(self) -> None:
        ...
        with (
            ...
            patch("blackcat_cli.server._HEALTH_POLL_INTERVAL_LOCAL", 0),
            patch("blackcat_cli.server._HEALTH_POLL_INTERVAL_REMOTE", 0),
...
class TestServerProcess:
    async def test_start_cleans_up_partial_state_on_health_failure(
        ...
        with (
            patch("blackcat_cli.server._port_in_use", return_value=False),
            patch(
                "blackcat_cli.server.tempfile.NamedTemporaryFile",
                return_value=log_file,
            ),
            patch("blackcat_cli.server.subprocess.Popen", return_value=process),
            patch(
                "blackcat_cli.server.wait_for_server_healthy",
    ...
    async def test_update_env_and_restart(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            server.update_env(BLACKCAT_CLI_SERVER_MODEL="anthropic:claude-opus-4-6")

            # Restart: should stop the old process and start a new one
            await server.restart()

        # Env override was applied
        env_key = "BLACKCAT_CLI_SERVER_MODEL"
    ...
    async def test_restart_rollback_on_failure(self, tmp_path: Path) -> None:
        ...
        old_value = os.environ.get("BLACKCAT_CLI_SERVER_MODEL")
        ...
        server.update_env(BLACKCAT_CLI_SERVER_MODEL="should-be-rolled-back")

        with pytest.raises(RuntimeError, match="restart failed"):
            await server.restart()

        # Env should be rolled back
        assert os.environ.get("BLACKCAT_CLI_SERVER_MODEL") == old_value
        # Overrides NOT cleared (available for retry)
        assert "BLACKCAT_CLI_SERVER_MODEL" in server._env_overrides
