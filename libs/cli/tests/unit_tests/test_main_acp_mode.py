...
def test_acp_mode_loads_tools_and_mcp_and_runs_server() -> None:
    ...
    with (
        patch.object(sys, "argv", ["blackcat", "--acp"]),
        patch(
            "blackcat_cli.main.check_cli_dependencies",
            side_effect=AssertionError("check_cli_dependencies should be skipped"),
        ),
        patch("blackcat_cli.main.parse_args", return_value=args),
        patch("blackcat_cli.config.settings", new=SimpleNamespace(has_tavily=True)),
        patch("blackcat_cli.model_config.save_recent_model", return_value=True),
        patch(
            "blackcat_cli.config.create_model", return_value=model_result
        ) as mock_create_model,
        patch("blackcat_cli.mcp_tools.resolve_and_load_mcp_tools", resolve_mcp_tools),
        patch("blackcat_cli.tools.fetch_url", new=fetch_tool),
        patch("blackcat_cli.tools.web_search", new=search_tool),
        patch(
            "blackcat_cli.agent.create_cli_agent", return_value=("graph", object())
        ) as mock_create_agent,
        patch(
            "blackcat_acp.server.AgentServerACP", return_value=server
        ) as mock_server_cls,
...
def test_acp_mode_omits_web_search_without_tavily() -> None:
    ...
    with (
        ...
        patch("blackcat_cli.main.parse_args", return_value=args),
        patch("blackcat_cli.config.settings", new=SimpleNamespace(has_tavily=False)),
        patch("blackcat_cli.model_config.save_recent_model", return_value=True),
        patch("blackcat_cli.config.create_model", return_value=model_result),
        patch("blackcat_cli.mcp_tools.resolve_and_load_mcp_tools", resolve_mcp_tools),
        patch("blackcat_cli.tools.fetch_url", new=fetch_tool),
        patch("blackcat_cli.tools.web_search", new=search_tool),
        patch(
        ...
        patch("blackcat_acp.server.AgentServerACP", return_value=server),
        patch("acp.run_agent", run_agent),
        pytest.raises(SystemExit) as exc_info,
    ):
        cli_main()
...
def test_non_acp_mode_checks_dependencies_before_parsing() -> None:
    ...
    with (
        patch.object(sys, "argv", ["blackcat"]),
        patch(
            "blackcat_cli.main.check_cli_dependencies", side_effect=SystemExit(7)
        ) as mock_check,
        patch("blackcat_cli.main.parse_args") as mock_parse,
        pytest.raises(SystemExit) as exc_info,
    ):
        cli_main()
