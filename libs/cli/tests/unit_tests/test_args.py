...
from blackcat_cli.agent import DEFAULT_AGENT_NAME
from blackcat_cli.main import _DEFAULT_AGENT_NAME, parse_args
from blackcat_cli.ui import show_help, show_threads_list_help
...
class TestInitialPromptArg:
    ...
    def test_with_other_args(self) -> None:
        ...
        with patch.object(
            sys, "argv", ["blackcat", "--agent", "myagent", "-m", "do something"]
        ):
            args = parse_args()
    ...
    def test_empty_string(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "-m", ""]):
            args = parse_args()
...
class TestInitialSkillArg:
    ...
    def test_flag_sets_initial_skill(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "--skill", "code-review"]):
            args = parse_args()
...
class TestResumeArg:
    ...
    def test_short_flag_no_value(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "-r"]):
            args = parse_args()
    ...
    def test_short_flag_with_value(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "-r", "abc12345"]):
            args = parse_args()
    ...
    def test_long_flag_no_value(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "--resume"]):
            args = parse_args()
        assert args.resume_thread == "__MOST_RECENT__"

    def test_long_flag_with_value(self) -> None:
        """Verify --resume with ID sets resume_thread to that ID."""
        with patch.object(sys, "argv", ["blackcat", "--resume", "xyz99999"]):
            args = parse_args()
    ...
    def test_with_other_args(self) -> None:
        ...
        with patch.object(
            sys, "argv", ["blackcat", "--agent", "myagent", "-r", "thread123"]
        ):
            args = parse_args()
    ...
    def test_resume_with_message(self) -> None:
        ...
        with patch.object(
            sys, "argv", ["blackcat", "-r", "thread456", "-m", "continue work"]
        ):
            args = parse_args()
...
class TestTopLevelHelp:
    """Test that `blackcat -h` shows the global help screen via _make_help_action."""

    def test_top_level_help_exits_cleanly(self) -> None:
        """Running `blackcat -h` should show help and exit with code 0."""
        buf = io.StringIO()
        test_console = Console(file=buf, highlight=False, width=120)

        with (
            patch.object(sys, "argv", ["blackcat", "-h"]),
            patch("blackcat_cli.ui.console", test_console),
        ...
        assert "blackcat" in output.lower()
        assert "--help" in output

    def test_help_subcommand_parses(self) -> None:
        """Running `blackcat help` should parse as command='help'.

        The actual help display happens in `cli_main()`, not `parse_args()`.
        """
        with patch.object(sys, "argv", ["blackcat", "help"]):
            args = parse_args()
...
class TestSubcommandHelpFlags:
    ...
    def test_agents_list_help(self) -> None:
        """Running `blackcat agents list -h` should show list-specific help."""
        self._run_help(
            ["blackcat", "agents", "list", "-h"],
            must_contain="List all agents",
            must_not_contain="--sandbox",
        )

    def test_agents_reset_help(self) -> None:
        """Running `blackcat agents reset -h` should show reset-specific help."""
        self._run_help(
            ["blackcat", "agents", "reset", "-h"],
            must_contain="--agent",
            must_not_contain="Start interactive thread",
        )

    def test_threads_list_help(self) -> None:
        """Running `blackcat threads list -h` should show threads list help."""
        self._run_help(
            ["blackcat", "threads", "list", "-h"],
            must_contain="--limit",
            must_not_contain="--sandbox",
        )

    def test_threads_delete_help(self) -> None:
        """Running `blackcat threads delete -h` should show threads delete help."""
        self._run_help(
            ["blackcat", "threads", "delete", "-h"],
            must_contain="delete",
            must_not_contain="--sandbox",
        )
...
class TestShortFlags:
    ...
    def test_short_agent_flag(self) -> None:
        """Verify -a sets agent."""
        with patch.object(sys, "argv", ["blackcat", "-a", "mybot"]):
            args = parse_args()
        assert args.agent == "mybot"

    def test_short_model_flag(self) -> None:
        """Verify -M sets model."""
        with patch.object(sys, "argv", ["blackcat", "-M", "gpt-4o"]):
            args = parse_args()
    ...
    def test_short_version_flag(self) -> None:
        ...
        with (
            patch.object(sys, "argv", ["blackcat", "-v"]),
            pytest.raises(SystemExit) as exc_info,
        ):
            parse_args()
        assert exc_info.value.code in (0, None)
...
class TestQuietArg:
    ...
    def test_short_flag(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "-q", "-n", "task"]):
            args = parse_args()
        assert args.quiet is True

    def test_long_flag(self) -> None:
        """Verify --quiet sets quiet=True."""
        with patch.object(sys, "argv", ["blackcat", "--quiet", "-n", "task"]):
            args = parse_args()
        assert args.quiet is True
    ...
    def test_combined_with_non_interactive(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "-q", "-n", "run tests"]):
            args = parse_args()
        assert args.quiet is True
    ...
    def test_quiet_without_non_interactive_parses(self) -> None:
        ...
        """
        with patch.object(sys, "argv", ["blackcat", "-q"]):
            args = parse_args()
        assert args.quiet is True
...
class TestNoMcpArg:
    ...
    def test_no_mcp_flag_parsed(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "--no-mcp"]):
            args = parse_args()
    ...
    def test_no_mcp_and_mcp_config_mutual_exclusion(self) -> None:
        ...
        with (  # noqa: SIM117  # separate to satisfy PT012
            patch.object(
                sys,
                "argv",
                ["blackcat", "--no-mcp", "--mcp-config", "/some/path"],
            ),
            patch("blackcat_cli.main.check_cli_dependencies"),
            patch("blackcat_cli.main.apply_stdin_pipe"),
        ):
...
class TestHelpScreenDrift:
    ...
    def test_all_parser_flags_appear_in_help(self) -> None:
        ...
        with (
            patch.object(sys, "argv", ["blackcat", "--_x_"]),
        ...
        with patch("blackcat_cli.ui.console", test_console):
            show_help()
    ...
    def test_threads_list_flags_appear_in_help(self) -> None:
        ...
        with (
            patch.object(sys, "argv", ["blackcat", "threads", "list", "-h"]),
            patch("sys.stdout", stdout_buf),
            patch("blackcat_cli.ui.console", Console(file=io.StringIO())),
            pytest.raises(SystemExit),
        ):
            parse_args()
...
class TestJsonArg:
    ...
    def test_json_shortcut(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "--json"]):
            args = parse_args()
        assert args.output_format == "json"

    def test_json_before_subcommand(self) -> None:
        """Verify --json works before a subcommand."""
        with patch.object(sys, "argv", ["blackcat", "--json", "agents", "list"]):
            args = parse_args()
    ...
    def test_json_after_subcommand(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "agents", "list", "--json"]):
            args = parse_args()
        assert args.command == "agents"
        assert args.output_format == "json"
    ...
    def test_output_format_flag_removed(self) -> None:
        ...
        with (
            patch.object(sys, "argv", ["blackcat", "--output-format", "json"]),
            pytest.raises(SystemExit) as exc_info,
        ):
            parse_args()
        assert exc_info.value.code == 2
    ...
    def test_json_after_nested_subcommand(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "skills", "list", "--json"]):
            args = parse_args()
