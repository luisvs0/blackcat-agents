...
def _make_app() -> MagicMock:
    ...
    app = MagicMock(spec=BlackCatApp)
    ...
    app._invoke_skill = BlackCatApp._invoke_skill.__get__(app)
    app._handle_skill_command = BlackCatApp._handle_skill_command.__get__(app)
    app._discover_skills_and_roots = BlackCatApp._discover_skills_and_roots.__get__(
        app
    )
    return app
...
def _app_message_texts(app: MagicMock) -> list[str]:
    ...
    from blackcat_cli.widgets.messages import AppMessage
...
class TestHandleSkillCommand:
    ...
    async def test_skill_not_found(self) -> None:
        app = _make_app()
        with (
            patch("blackcat_cli.skills.load.list_skills", return_value=[]),
            patch("blackcat_cli.config.settings"),
        ):
    ...
    async def test_content_none_shows_error(self) -> None:
        ...
        skill = _fake_skill()
        with (
            patch("blackcat_cli.skills.load.list_skills", return_value=[skill]),
            patch("blackcat_cli.skills.load.load_skill_content", return_value=None),
            patch("blackcat_cli.config.settings"),
        ):
    ...
    async def test_containment_violation_shows_specific_message(self) -> None:
        ...
        with (
            patch("blackcat_cli.skills.load.list_skills", return_value=[skill]),
            patch(
                "blackcat_cli.skills.load.load_skill_content",
    ...
    async def test_empty_content_shows_error(self) -> None:
        ...
        with (
            patch("blackcat_cli.skills.load.list_skills", return_value=[skill]),
            patch("blackcat_cli.skills.load.load_skill_content", return_value=""),
            patch("blackcat_cli.config.settings"),
        ):
            await app._handle_skill_command("/skill:test-skill")
    ...
    async def test_happy_path_sends_prompt(self) -> None:
        from blackcat_cli.widgets.messages import SkillMessage
    ...
    async def test_filesystem_error_shows_specific_message(self) -> None:
        ...
        with (
            patch(
                "blackcat_cli.skills.load.list_skills",
    ...
    async def test_cache_hit_skips_list_skills(self) -> None:
        ...
        with (
            ...
            patch("blackcat_cli.skills.load.list_skills") as mock_list,
        ):
            await app._handle_skill_command("/skill:test-skill")
