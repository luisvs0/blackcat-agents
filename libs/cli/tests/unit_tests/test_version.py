...
def test_cli_version_flag() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "blackcat_cli.main", "--version"],
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    assert f"blackcat-cli {__version__}" in result.stdout
    sdk_version = pkg_version("blackcat")
    assert f"blackcat (SDK) {sdk_version}" in result.stdout
...
async def test_version_slash_command_message_format() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert f"blackcat-cli version: {__version__}" in content
        assert f"blackcat (SDK) version: {sdk_version}" in content
...
async def test_version_slash_command_sdk_unavailable() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        content = str(app_msgs[-1]._content)
        assert f"blackcat-cli version: {__version__}" in content
        assert "blackcat (SDK) version: unknown" in content
...
async def test_version_slash_command_cli_version_unavailable() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        with patch.dict(sys.modules, {"blackcat_cli._version": None}):
            await app._handle_command("/version")
        await pilot.pause()

        app_msgs = app.query(AppMessage)
        content = str(app_msgs[-1]._content)
        assert "blackcat-cli version: unknown" in content


def test_help_mentions_version_flag() -> None:
    """Verify that the CLI help text mentions `--version` and SDK."""
    result = subprocess.run(
        [sys.executable, "-m", "blackcat_cli.main", "help"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_cli_help_flag_short() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "blackcat_cli.main", "-h"],
        capture_output=True,
        text=True,
        check=False,
    )
