...
def cache_file(tmp_path):
    ...
    with patch("blackcat_cli.update_check.CACHE_FILE", path):
        yield path
...
class TestIsUpdateAvailable:
    def test_newer_available(self) -> None:
        with patch(
            "blackcat_cli.update_check.get_latest_version", return_value="99.0.0"
        ):
    ...
    def test_current_version(self) -> None:
        with (
            patch(
                "blackcat_cli.update_check.get_latest_version", return_value="0.0.1"
            ),
            patch("blackcat_cli.update_check.__version__", "0.0.1"),
        ):
            available, latest = is_update_available()
    ...
    def test_ahead_of_pypi(self) -> None:
        ...
        with (
            ...
            patch("blackcat_cli.update_check.__version__", "99.0.0"),
        ):
            available, latest = is_update_available()
    ...
    def test_fetch_failure(self) -> None:
        with patch("blackcat_cli.update_check.get_latest_version", return_value=None):
            available, latest = is_update_available()
    ...
    def test_prerelease_user_sees_newer_prerelease(self) -> None:
        ...
        with (
            patch(
                "blackcat_cli.update_check.get_latest_version",
                return_value="1.0.0a2",
            ),
            patch("blackcat_cli.update_check.__version__", "1.0.0a1"),
        ):
            available, latest = is_update_available()
    ...
    def test_stable_user_does_not_see_prerelease(self) -> None:
        ...
        with (
            ...
            patch("blackcat_cli.update_check.__version__", "1.0.0"),
        ):
            available, latest = is_update_available()
    ...
    def test_invalid_installed_version(self) -> None:
        ...
        with patch("blackcat_cli.update_check.__version__", "not-a-version"):
            available, latest = is_update_available()
...
class TestSetAutoUpdate:
    ...
    def config_path(self, tmp_path):
        ...
        with patch("blackcat_cli.update_check.DEFAULT_CONFIG_PATH", path):
            yield path
    ...
    def test_round_trip_with_is_auto_update_enabled(self, config_path) -> None:  # noqa: ARG002
        ...
        set_auto_update(True)
        with (
            patch("blackcat_cli.config._is_editable_install", return_value=False),
            patch.dict("os.environ", {}, clear=False),
        ):
            import os

            os.environ.pop("BLACKCAT_CLI_AUTO_UPDATE", None)
...
class TestIsAutoUpdateEnabled:
    ...
    def test_env_var_enables(self, config_path) -> None:  # noqa: ARG002
        """BLACKCAT_CLI_AUTO_UPDATE=1 enables auto-update."""
        with (
            patch("blackcat_cli.config._is_editable_install", return_value=False),
            patch.dict("os.environ", {"BLACKCAT_CLI_AUTO_UPDATE": "1"}),
        ):
            assert is_auto_update_enabled() is True
    ...
    def test_editable_install_always_disabled(self, config_path) -> None:
        ...
        with patch("blackcat_cli.config._is_editable_install", return_value=True):
            assert is_auto_update_enabled() is False
