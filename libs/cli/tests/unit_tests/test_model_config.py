...
class TestHasProviderCredentials:
    ...
    def test_returns_true_with_prefixed_env_var(self):
        """Returns True when only the BLACKCAT_CLI_ prefixed var is set."""
        with patch.dict(
            "os.environ",
            {"BLACKCAT_CLI_ANTHROPIC_API_KEY": "sk-prefixed"},
            clear=True,
        ):
            assert has_provider_credentials("anthropic") is True
...
class TestThreadRelativeTimePersistence:
    ...
    def test_default_is_true(self, tmp_path: Path) -> None:
        ...
        from blackcat_cli.model_config import load_thread_relative_time
    ...
    def test_preserves_other_config_sections(self, tmp_path: Path) -> None:
        ...
        from blackcat_cli.model_config import save_thread_relative_time
...
class TestThreadSortOrderPersistence:
    ...
    def test_default_is_updated_at(self, tmp_path: Path) -> None:
        ...
        from blackcat_cli.model_config import load_thread_sort_order
    ...
    def test_preserves_other_config_sections(self, tmp_path: Path) -> None:
        ...
        from blackcat_cli.model_config import save_thread_sort_order
...
class TestThreadConfigCoalesced:
    ...
    def test_defaults_when_no_file(self, tmp_path: Path) -> None:
        ...
        from blackcat_cli.model_config import load_thread_config
...
"""
    ...
    def test_save_invalidates_cache(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            from blackcat_cli.model_config import _thread_config_cache
...
class TestResolveEnvVar:
    ...
    def test_returns_canonical_value(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.delenv("BLACKCAT_CLI_ANTHROPIC_API_KEY", raising=False)
    ...
    def test_prefix_beats_canonical(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """BLACKCAT_CLI_ prefixed var takes priority over canonical."""
        monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-canonical")
        monkeypatch.setenv("BLACKCAT_CLI_ANTHROPIC_API_KEY", "sk-override")
        from blackcat_cli.model_config import resolve_env_var
    ...
    def test_empty_string_treated_as_unset(
        ...
        monkeypatch.setenv("BLACKCAT_CLI_ANTHROPIC_API_KEY", "")
        from blackcat_cli.model_config import resolve_env_var
    ...
    def test_prefix_only(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setenv("BLACKCAT_CLI_OPENAI_API_KEY", "sk-prefixed")
        from blackcat_cli.model_config import resolve_env_var
    ...
    def test_skips_double_prefix(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setenv("BLACKCAT_CLI_MY_KEY", "direct")
        monkeypatch.delenv("BLACKCAT_CLI_BLACKCAT_CLI_MY_KEY", raising=False)
        from blackcat_cli.model_config import resolve_env_var

        assert resolve_env_var("BLACKCAT_CLI_MY_KEY") == "direct"
...
class TestModelPersistenceBetweenSessions:
    ...
    def test_saved_model_is_used_when_no_model_specified(self, tmp_path):
        ...
        """
        from blackcat_cli.config import _get_default_model_spec

        # Use a temporary config path
        config_path = tmp_path / ".blackcat" / "config.toml"
    ...
    def test_config_file_default_takes_priority_over_env_detection(self, tmp_path):
        ...
        """
        from blackcat_cli.config import _get_default_model_spec
        from blackcat_cli.model_config import save_default_model
...
class TestGetAvailableModels:
    ...
    def test_returns_discovered_models_when_package_installed(self):
        ...
        def mock_load(module_path: str) -> dict[str, Any]:
            if module_path == "protohello_anthropic.data._profiles":
                return fake_profiles
            msg = "not installed"
            raise ImportError(msg)

        with patch(
            "blackcat_cli.model_config._load_provider_profiles",
    ...
    def test_logs_debug_on_import_error(self, caplog):
        ...
        with (
            ...
            caplog.at_level(logging.DEBUG, logger="blackcat_cli.model_config"),
        ):
            get_available_models()
...
enabled = "false"
""")
        with caplog.at_level(logging.WARNING, logger="blackcat_cli.model_config"):
            config = ModelConfig.load(config_path)
...
class TestProfileModuleFromClassPath:
    ...
    def test_derives_module_path(self):
        ...
        result = _profile_module_from_class_path(
            "protohello_baseten.chat_models:ChatBaseten"
        )
        assert result == "protohello_baseten.data._profiles"
...
class_path = "protohello_baseten.chat_models:ChatBaseten"
api_key_env = "BASETEN_API_KEY"
""")

        def mock_load(module_path: str) -> dict[str, Any]:
            if module_path == "protohello_baseten.data._profiles":
...
""")
        ...
        with (
            ...
            patch.object(model_config, "DEFAULT_CONFIG_PATH", config_path),
            caplog.at_level(logging.WARNING, logger="blackcat_cli.model_config"),
        ):
            models = get_available_models()
...
""")
        ...
        with (
            ...
            patch.object(model_config, "DEFAULT_CONFIG_PATH", config_path),
            patch(
                "blackcat_cli.model_config._get_builtin_providers",
                return_value={},
            ),
        ):
            models = get_available_models()
    ...
    def test_skips_builtin_registry_providers(self, tmp_path):
        ...
        config_path.write_text("""
[models.providers.anthropic]
class_path = "protohello_anthropic.chat_models:ChatAnthropic"
""")
...
class TestGetProviderProfileModules:
    ...
    def test_builds_from_builtin_providers(self):
        ...
        fake_registry = {
            "anthropic": ("protohello_anthropic", "ChatAnthropic", None),
            "openai": ("protohello_openai", "ChatOpenAI", None),
            "ollama": ("protohello_ollama", "ChatOllama", None),
            "fireworks": ("protohello_fireworks", "ChatFireworks", None),
        }
        with patch(
            "blackcat_cli.model_config._get_builtin_providers",
        ...
        assert ("anthropic", "protohello_anthropic.data._profiles") in result
        assert ("openai", "protohello_openai.data._profiles") in result
        assert ("ollama", "protohello_ollama.data._profiles") in result
        assert ("fireworks", "protohello_fireworks.data._profiles") in result
        assert len(result) == 4
    ...
    def test_handles_submodule_paths(self):
        ...
        fake_registry = {
            "google_anthropic_vertex": (
                "protohello_google_vertexai.model_garden",
                "ChatAnthropicVertex",
                None,
            ),
        }
        ...
        assert result == [
            ("google_anthropic_vertex", "protohello_google_vertexai.data._profiles"),
        ]
...
class TestGetBuiltinProviders:
    ...
    def test_prefers_builtin_providers(self):
        ...
        import protohello.chat_models.base as base_module

        builtin = {"anthropic": ("protohello_anthropic", "ChatAnthropic", None)}
        legacy = {"openai": ("protohello_openai", "ChatOpenAI", None)}
...
""")
    ...
    def test_env_used_when_neither_set(self, tmp_path):
        ...
        from blackcat_cli.config import _get_default_model_spec, settings
