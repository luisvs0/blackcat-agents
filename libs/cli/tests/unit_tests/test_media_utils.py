...
class TestImageData:
    ...
    def test_to_message_content_png(self) -> None:
        """Test converting PNG image data to ProtoHello message format."""
        image = ImageData(
    ...
    def test_to_message_content_jpeg(self) -> None:
        """Test converting JPEG image data to ProtoHello message format."""
        image = ImageData(
            base64_data="abc123",
            format="jpeg",
            placeholder="[image 2]",
...
class TestGetClipboardImage:
    ...
    @patch("blackcat_cli.media_utils.sys.platform", "linux")
    def test_unsupported_platform_returns_none_and_warns(self) -> None:
        ...
        with patch("blackcat_cli.media_utils.logger") as mock_logger:
            result = get_clipboard_image()
            assert result is None
            mock_logger.warning.assert_called_once()
            assert "linux" in mock_logger.warning.call_args[0][1]

    @patch("blackcat_cli.media_utils.sys.platform", "darwin")
    @patch("blackcat_cli.media_utils._get_macos_clipboard_image")
    ...
    @patch("blackcat_cli.media_utils.sys.platform", "darwin")
    @patch("blackcat_cli.media_utils.subprocess.run")
    @patch("blackcat_cli.media_utils._get_executable")
    ...
    @patch("blackcat_cli.media_utils.sys.platform", "darwin")
    @patch("blackcat_cli.media_utils._get_clipboard_via_osascript")
    @patch("blackcat_cli.media_utils.subprocess.run")
...
class TestVideoData:
    ...
    def test_to_message_content_mp4(self) -> None:
        """Test converting MP4 video data to ProtoHello video block format."""
        video = VideoData(
    ...
    def test_to_message_content_mov(self) -> None:
        """Test converting MOV video data to ProtoHello video block format."""
        video = VideoData(
            base64_data="abc123",
            format="quicktime",
            placeholder="[video 2]",
