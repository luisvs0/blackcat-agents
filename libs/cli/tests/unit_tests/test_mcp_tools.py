...
class TestGetMCPTools:
    ...
    def _bypass_health_checks(self) -> Generator[None]:
        ...
        with (
            patch("blackcat_cli.mcp_tools._check_stdio_server"),
            patch(
                "blackcat_cli.mcp_tools._check_remote_server",
                new_callable=AsyncMock,
            ),
        ):
            yield

    @patch("protohello_mcp_adapters.tools.load_mcp_tools")
    @patch("protohello_mcp_adapters.client.MultiServerMCPClient")
...
class TestDiscoverMcpConfigs:
    ...
    def test_no_configs_found(
        ...
        monkeypatch.setattr(
            "blackcat_cli.project_utils.find_project_root",
    ...
    def test_user_level_only(
        ...
        """Only ~/.blackcat/.mcp.json exists."""
        home = tmp_path / "home"
        user_dir = home / ".blackcat"
    ...
    def test_project_blackcat_subdir_only(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Only <project>/.blackcat/.mcp.json exists."""
        home = tmp_path / "home"
        home.mkdir()
        project = tmp_path / "project"
        subdir = project / ".blackcat"
...
class TestResolveAndLoadMcpTools:
    ...
    @patch("blackcat_cli.mcp_tools._load_tools_from_config")
    @patch("blackcat_cli.mcp_tools.discover_mcp_configs")
    ...
    @patch("blackcat_cli.mcp_tools._load_tools_from_config")
    @patch("blackcat_cli.mcp_trust.is_project_mcp_trusted")
...
class TestClassifyDiscoveredConfigs:
    ...
    def test_user_config_classified(self) -> None:
        """Paths under ~/.blackcat/ are classified as user."""
        user_path = Path.home() / ".blackcat" / ".mcp.json"
    ...
    def test_project_config_classified(self, tmp_path: Path) -> None:
        """Paths outside ~/.blackcat/ are classified as project."""
...
class TestCheckStdioServer:
    ...
    def test_command_not_found(self) -> None:
        ...
        with (
            patch("blackcat_cli.mcp_tools.shutil.which", return_value=None),
            pytest.raises(
                RuntimeError,
    ...
    def test_command_exists(self) -> None:
        ...
        with patch(
            "blackcat_cli.mcp_tools.shutil.which", return_value="/usr/bin/npx"
        ):
