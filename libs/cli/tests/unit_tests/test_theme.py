"""Tests for blackcat_cli.theme module."""
...
from blackcat_cli.theme import (
    _BUILTIN_NAMES,
    DARK_COLORS,
    DEFAULT_THEME,
    LIGHT_COLORS,
...
class TestThemeEntryRegistry:
    ...
    @pytest.mark.parametrize(
        ("name", "dark", "custom"),
        [
            ("protohello", True, True),
            ("protohello-light", False, True),
...
class TestLoadThemePreference:
    ...
    def test_returns_default_when_no_config(
        ...
        from blackcat_cli.app import _load_theme_preference

        monkeypatch.setattr("blackcat_cli.app.theme.DEFAULT_THEME", "protohello")
        missing = tmp_path / "nonexistent" / "config.toml"
        monkeypatch.setattr("blackcat_cli.model_config.DEFAULT_CONFIG_PATH", missing)
        assert _load_theme_preference() == "protohello"
    ...
    def test_returns_saved_theme(
        ...
        monkeypatch.setattr("blackcat_cli.model_config.DEFAULT_CONFIG_PATH", config)
        assert _load_theme_preference() == "protohello-light"
...
class TestSaveThemePreference:
    ...
    def test_creates_config_from_scratch(
        ...
        from blackcat_cli.app import save_theme_preference

        config = tmp_path / "config.toml"
        monkeypatch.setattr("blackcat_cli.model_config.DEFAULT_CONFIG_PATH", config)
        assert save_theme_preference("protohello-light") is True
        data = tomllib.loads(config.read_text())
        assert data["ui"]["theme"] == "protohello-light"
    ...
    def test_preserves_existing_config_keys(
        ...
        config.write_text('[model]\nname = "gpt-4"\n')
        monkeypatch.setattr("blackcat_cli.model_config.DEFAULT_CONFIG_PATH", config)
        assert save_theme_preference("protohello") is True
        data = tomllib.loads(config.read_text())
        assert data["model"]["name"] == "gpt-4"
        assert data["ui"]["theme"] == "protohello"
    ...
    def test_returns_false_on_write_error(
        ...
        result = save_theme_preference("protohello")
...
""",
        ...
        original_label = builtins["protohello"].label
        original_dark = builtins["protohello"].dark
        original_custom = builtins["protohello"].custom
        _load_user_themes(builtins, config_path=config)
        entry = builtins["protohello"]
        # Color overridden
        assert entry.colors.primary == "#FF0000"
...
""",
        ...
        builtins = _builtin_themes()
        original = builtins["protohello"]
        _load_user_themes(builtins, config_path=config)
        assert builtins["protohello"] is original
...
def _register_lc_theme(app: object) -> None:
    """Register the ProtoHello theme on a test app so ThemeSelectorScreen works."""
    ...
    app.theme = "protohello"  # type: ignore[attr-defined]
...
class TestThemeSelectorScreen:
    ...
    async def test_compose_shows_all_registry_themes(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            screen = ThemeSelectorScreen(current_theme="protohello")
    ...
    async def test_current_theme_highlighted(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            assert highlighted.id == "protohello"
