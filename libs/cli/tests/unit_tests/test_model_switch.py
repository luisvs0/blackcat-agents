...
from blackcat_cli.app import BlackCatApp, _extract_model_params_flag
from blackcat_cli.config import settings
from blackcat_cli.model_config import ModelSpec, clear_caches
...
def mock_create_model() -> Iterator[Mock]:
    ...
    with patch(
        "blackcat_cli.config.create_model",
...
class TestModelSwitchNoOp:
    ...
    async def test_no_message_when_switching_to_same_model(self) -> None:
        ...
        with (
            patch(
                "blackcat_cli.model_config.has_provider_credentials",
                return_value=True,
            ),
...
class TestModelSwitchErrorHandling:
    ...
    async def test_missing_credentials_shows_error(self) -> None:
        ...
        with (
            ...
            patch(
                "blackcat_cli.model_config.get_credential_env_var",
    ...
    async def test_save_recent_model_failure_shows_warning(self) -> None:
        ...
        with (
            ...
            patch("blackcat_cli.model_config.save_recent_model", return_value=False),
        ...
        assert "~/.blackcat/" in captured_errors[0]
    ...
    async def test_remote_agent_sets_model_override(self) -> None:
        ...
        with (
            ...
            patch(
                "blackcat_cli.model_config.save_recent_model", return_value=True
            ) as mock_save,
            patch.object(AppMessage, "__init__", capture_init),
        ):
            await app._switch_model("anthropic:claude-sonnet-4-5")
...
class TestModelSwitchBareModelName:
    ...
    async def test_bare_model_name_auto_detects_provider(self) -> None:
        ...
        with (
            patch("blackcat_cli.config.detect_provider", return_value="openai"),
            patch(
                "blackcat_cli.model_config.has_provider_credentials",
                return_value=True,
            ),
