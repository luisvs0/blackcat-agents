...
from blackcat_cli.editor import (
    GUI_WAIT_FLAG,
    VIM_EDITORS,
    _prepare_command,
    open_in_editor,
...
class TestResolveEditor:
    ...
    def test_default_vi_on_unix(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch("blackcat_cli.editor.sys") as mock_sys:
...
class TestOpenInEditor:
    ...
    @patch("blackcat_cli.editor.subprocess.run")
    def test_returns_edited_text(self, mock_run: MagicMock) -> None:
        ...
        with patch("blackcat_cli.editor.resolve_editor", return_value=["nano"]):
    ...
    def test_returns_none_on_editor_not_found(self) -> None:
        with (
            patch(
                "blackcat_cli.editor.subprocess.run",
                side_effect=FileNotFoundError("not found"),
            ),
            patch(
                "blackcat_cli.editor.resolve_editor",
    ...
    def test_returns_none_when_resolve_editor_is_none(self) -> None:
        with patch("blackcat_cli.editor.resolve_editor", return_value=None):
            result = open_in_editor("text")
        assert result is None
    ...
    def test_handles_permission_error_on_cleanup(self, mock_run: MagicMock) -> None:
        ...
        mock_run.side_effect = fake_run
        with (
            patch("blackcat_cli.editor.resolve_editor", return_value=["nano"]),
            patch.object(
                pathlib.Path,
                "unlink",
