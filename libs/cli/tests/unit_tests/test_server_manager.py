...
from blackcat_cli._server_config import ServerConfig
from blackcat_cli.project_utils import ProjectContext
from blackcat_cli.server_manager import (
...
class TestStartServerAndGetAgent:
    ...
    async def test_uses_relative_graph_and_checkpointer_refs(
        ...
        with (
            ...
            patch(
                "blackcat_cli.server_manager.tempfile.mkdtemp",
                return_value=str(work_dir),
            ),
            patch("blackcat_cli.server_manager.shutil.copy2"),
            patch("blackcat_cli.server_manager._write_checkpointer"),
            patch("blackcat_cli.server_manager._write_pyproject"),
            patch(
                "blackcat_cli.server.generate_langgraph_json"
            ) as mock_generate_langgraph_json,
            patch("blackcat_cli.server.ServerProcess", return_value=mock_server),
            patch("blackcat_cli.remote_client.RemoteAgent", return_value=mock_agent),
        ):
    ...
    def test_relative_paths_written_verbatim_to_langgraph_json(
        ...
        from blackcat_cli.server import generate_langgraph_json
...
class TestWritePyproject:
    ...
    def test_runtime_pyproject_relies_on_cli_dependency_only(
        ...
        """The runtime should inherit `langgraph-cli` from `blackcat-cli`."""
        _write_pyproject(tmp_path)

        content = (tmp_path / "pyproject.toml").read_text()

        assert '"blackcat-cli @ file://' in content
