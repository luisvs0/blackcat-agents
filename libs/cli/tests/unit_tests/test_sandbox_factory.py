...
@pytest.mark.parametrize(
    ("provider", "package"),
    [
        ("daytona", "protohello-daytona"),
        ("modal", "protohello-modal"),
        ("runloop", "protohello-runloop"),
    ],
)
def test_get_provider_raises_helpful_error_for_missing_optional_dependency(
    ...
    with (
        patch(
            "blackcat_cli.integrations.sandbox_factory.importlib.import_module",
...
def test_agentcore_get_or_create_raises_for_missing_dep() -> None:
    ...
    error = (
        ...
        r"'protohello-agentcore-codeinterpreter' package"
    )
...
def test_agentcore_get_or_create_happy_path() -> None:
    ...
    with patch(
        "blackcat_cli.integrations.sandbox_factory._import_provider_module",
...
class TestVerifySandboxDeps:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("agentcore", "protohello_agentcore_codeinterpreter"),
            ("daytona", "protohello_daytona"),
            ("modal", "protohello_modal"),
            ("runloop", "protohello_runloop"),
        ],
    )
    def test_raises_import_error_when_backend_missing(
        ...
        mock_find_spec = patch(
            "blackcat_cli.integrations.sandbox_factory.importlib.util.find_spec",
            return_value=None,
        )
        with (
            ...
            pytest.raises(
                ...
                rf"pip install 'blackcat-cli\[{provider}\]'",
            ),
        ):
