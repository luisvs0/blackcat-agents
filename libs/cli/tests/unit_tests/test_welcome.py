...
from blackcat_cli.widgets.welcome import (
    _TIPS,
    WelcomeBanner,
...
def _make_banner(
    ...
    """
    import blackcat_cli.config as _cfg
    ...
    if project_name:
        ...
        env["BLACKCAT_CLI_LANGSMITH_PROJECT"] = project_name
...
class TestBuildBannerThreadLink:
    ...
    def test_thread_id_linked_when_project_url_provided(self) -> None:
        ...
        project_url = "https://smith.protohello.com/o/org/projects/p/abc123"
        ...
        assert links[0] == f"{project_url}/t/99999?utm_source=blackcat-cli"
    ...
    def test_thread_link_coexists_with_langsmith_project(self) -> None:
        ...
        links = _extract_links(banner, thread_id_start, thread_id_end)
        assert links
        assert links[0] == f"{project_url}/t/77777?utm_source=blackcat-cli"
...
class TestUpdateThreadId:
    ...
    def test_update_thread_id_preserves_project_url(self) -> None:
        ...
        links = _extract_links(banner, thread_start, thread_end)
        assert links
        assert links[0] == f"{project_url}/t/new_id?utm_source=blackcat-cli"
...
class TestBuildBannerEditableInstall:
    ...
    def test_build_banner_with_editable_install(self) -> None:
        ...
        with (
            ...
            patch(
                "blackcat_cli.widgets.welcome._is_editable_install",
                return_value=True,
            ),
            patch(
                "blackcat_cli.widgets.welcome._get_editable_install_path",
                return_value="~/dev/blackcat",
            ),
        ):
            widget = WelcomeBanner()
            banner = widget._build_banner()
        assert "Installed from: ~/dev/blackcat" in banner.plain
