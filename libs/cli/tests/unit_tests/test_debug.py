...
class TestConfigureDebugLogging:
    def test_noop_when_env_unset(self) -> None:
        """No handlers should be added when BLACKCAT_CLI_DEBUG is unset."""
    ...
    def test_adds_handler_when_env_set(self, tmp_path) -> None:
        ...
        with patch.dict(
            os.environ,
            {"BLACKCAT_CLI_DEBUG": "1", "BLACKCAT_CLI_DEBUG_FILE": str(log_file)},
        ):
            configure_debug_logging(logger)
    ...
    def test_bad_path_prints_warning_no_crash(self, capsys) -> None:
        ...
        with patch.dict(
            os.environ,
            {
                "BLACKCAT_CLI_DEBUG": "1",
                "BLACKCAT_CLI_DEBUG_FILE": "/nonexistent_dir/debug.log",
            },
        ):
            configure_debug_logging(logger)
        assert len(logger.handlers) == original_count
