...
if TYPE_CHECKING:
    from protohello.agents.middleware.types import AgentState
    from protohello.messages import ToolCall
    from langgraph.runtime import Runtime

from blackcat_cli.agent import (
    DEFAULT_AGENT_NAME,
...
from blackcat_cli.config import Settings, get_glyphs
...
def test_format_web_search_description_default_max_results():
    ...
    assert "Query: protohello tutorial" in description
...
class TestGetSystemPromptModelIdentity:
    ...
    def test_includes_model_identity_when_all_settings_present(self) -> None:
        ...
        with patch("blackcat_cli.agent.settings", mock_settings):
...
class TestGetSystemPromptCwdOSError:
    ...
    def test_falls_back_on_cwd_oserror(self) -> None:
        ...
        with (
            patch("blackcat_cli.agent.settings", mock_settings),
            patch("blackcat_cli.agent.Path.cwd", side_effect=OSError("deleted")),
        ):
            prompt = get_system_prompt("test-agent")
...
class TestCreateCliAgentInteractiveForwarding:
    ...
    def test_forwards_interactive_false_to_get_system_prompt(
        ...
        with (
            patch("blackcat_cli.agent.settings", mock_settings),
            patch("blackcat_cli.agent.SkillsMiddleware"),
            patch("blackcat_cli.agent.MemoryMiddleware"),
            patch("blackcat_cli.agent.create_black_cat", return_value=mock_agent),
            patch(
                "blackcat._models.init_chat_model",
                return_value=fake_model,
            ),
            patch("blackcat_cli.agent.get_system_prompt") as mock_get_prompt,
        ):
...
class TestDefaultAgentName:
    ...
    def test_default_agent_name_value(self) -> None:
        ...
        the directory name under `~/.blackcat/`.
        """
...
class TestListAgents:
    ...
    def test_default_agent_marked(self, tmp_path: Path) -> None:
        ...
        mock_settings = Mock()
        mock_settings.user_blackcat_dir = agents_dir
        ...
        with (
            patch("blackcat_cli.agent.settings", mock_settings),
            patch("blackcat_cli.agent.console") as mock_console,
        ):
...
class TestCreateCliAgentSkillsSources:
    ...
    def test_skills_source_precedence_order(self, tmp_path: Path) -> None:
        ...
        with (
            patch("blackcat_cli.agent.settings", mock_settings),
            patch("blackcat_cli.agent.SkillsMiddleware", FakeSkillsMiddleware),
            patch("blackcat_cli.agent.MemoryMiddleware"),
            patch("blackcat_cli.agent.create_black_cat", return_value=mock_agent),
            patch(
                "blackcat._models.init_chat_model",
...
class TestCreateCliAgentMemorySources:
    ...
    def test_project_agent_md_paths_in_memory_sources(self, tmp_path: Path) -> None:
        ...
        project_inner = tmp_path / ".blackcat" / "AGENTS.md"
        ...
        with (
            patch("blackcat_cli.agent.settings", mock_settings),
            patch("blackcat_cli.agent.SkillsMiddleware"),
            patch("blackcat_cli.agent.MemoryMiddleware", FakeMemoryMiddleware),
            patch("blackcat_cli.agent.FilesystemBackend"),
            patch(
                "blackcat_cli.agent.create_black_cat",
...
class TestCreateCliAgentProjectContext:
    ...
    def test_project_context_drives_project_skills_and_subagents(
        ...
        project_skills_dir = project_root / ".blackcat" / "skills"
        ...
        project_agents_dir = project_root / ".blackcat" / "agents"
        ...
        mock_settings.model_context_limit = None
        mock_settings.project_root = None
        mock_settings.user_protohello_project = None
        ...
        with (
            ...
            patch("blackcat_cli.agent.SkillsMiddleware", FakeSkillsMiddleware),
            patch("blackcat_cli.agent.MemoryMiddleware"),
            patch("blackcat_cli.agent.list_subagents", return_value=[]) as mock_list,
            patch("blackcat_cli.agent.create_black_cat", return_value=mock_agent),
            patch("blackcat._models.init_chat_model", return_value=fake_model),
        ):
            create_cli_agent(
                model="fake-model",
                assistant_id="test",
    ...
    def test_project_context_drives_project_agents_md_paths(
        ...
        blackcat_md = project_root / ".blackcat" / "AGENTS.md"
        blackcat_md.parent.mkdir(parents=True)
        blackcat_md.write_text("blackcat instructions")
        ...
        sources = captured_sources[0]
        assert sources[0] == str(agent_dir / "AGENTS.md")
        assert sources[1:] == [str(blackcat_md), str(root_md)]
    ...
    def test_project_context_sets_local_shell_root_dir(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("blackcat_cli.agent.SkillsMiddleware"),
            patch(
                "blackcat_cli.agent.LocalShellBackend", return_value=mock_backend
            ) as mock_shell,
            patch("blackcat_cli.agent.create_black_cat", return_value=mock_agent),
            patch("blackcat._models.init_chat_model", return_value=fake_model),
        ):
    ...
    def test_cwd_sets_local_filesystem_root_dir_without_shell(
        ...
        with (
            ...
            patch("blackcat_cli.agent.MemoryMiddleware"),
            patch("blackcat_cli.agent.SkillsMiddleware"),
            patch("blackcat_cli.agent.FilesystemBackend") as mock_filesystem,
            patch("blackcat_cli.agent.create_black_cat", return_value=mock_agent),
            patch("blackcat._models.init_chat_model", return_value=fake_model),
        ):
            create_cli_agent(
...
class TestMiddlewareStackConformance:
    """Verify all middleware passed to create_black_cat inherits AgentMiddleware."""
    ...
    def test_all_middleware_inherit_agent_middleware(self, tmp_path: Path) -> None:
        ...
        """
        from protohello.agents.middleware.types import AgentMiddleware
...
class TestShellAllowListMiddleware:
    ...
    def test_allows_approved_shell_command_sync(self) -> None:
        ...
        from blackcat_cli.agent import ShellAllowListMiddleware
    ...
    def test_rejects_shell_allow_all(self) -> None:
        ...
        from blackcat_cli.agent import ShellAllowListMiddleware
        from blackcat_cli.config import SHELL_ALLOW_ALL
...
class TestCreateCliAgentShellMiddlewareWiring:
    ...
    def test_interrupt_shell_only_adds_middleware_to_subagents(
        ...
        with (
            ...
            patch("blackcat_cli.agent.MemoryMiddleware"),
            patch(
                "blackcat_cli.agent.list_subagents",
