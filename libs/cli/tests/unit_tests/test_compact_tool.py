...
`libs/blackcat/tests/unit_tests/middleware/test_compact_tool.py`.
"""
...
class TestHITLGating:
    ...
    def test_hitl_gating_when_enabled(self) -> None:
        ...
        with patch("blackcat_cli.agent.REQUIRE_COMPACT_TOOL_APPROVAL", True):
            from blackcat_cli.agent import _add_interrupt_on
    ...
    def test_hitl_gating_when_disabled(self) -> None:
        ...
        with patch("blackcat_cli.agent.REQUIRE_COMPACT_TOOL_APPROVAL", False):
            from blackcat_cli.agent import _add_interrupt_on
