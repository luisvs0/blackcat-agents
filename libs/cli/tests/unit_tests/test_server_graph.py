...
def _import_fresh_server_graph() -> ModuleType:
    """Import `blackcat_cli.server_graph` from a clean module state."""
    sys.modules.pop("blackcat_cli.server_graph", None)
    return importlib.import_module("blackcat_cli.server_graph")
    ...
    def test_auto_discovery_loads_mcp_without_explicit_config(self) -> None:
        ...
        with (
            ...
            patch.dict(
                sys.modules,
                {
                    "blackcat_cli.agent": agent_module,
                    "blackcat_cli.config": config_module,
                    "blackcat_cli.tools": tools_module,
                    "blackcat_cli.mcp_tools": mcp_module,
                },
            ),
            patch(
