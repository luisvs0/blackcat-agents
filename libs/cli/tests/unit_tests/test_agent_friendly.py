...
from blackcat_cli.main import parse_args
from blackcat_cli.ui import (
    show_agents_help,
    show_help,
    show_list_help,
    show_reset_help,
...
class TestHelpScreenExamples:
    ...
    def test_list_help_has_examples(self) -> None:
        ...
        assert "blackcat list" in text
        assert "blackcat list --json" in text

    def test_skills_list_help_has_examples(self) -> None:
        text = self._render(show_skills_list_help)
        assert "Examples:" in text
        assert "blackcat skills list --project" in text
        assert "blackcat skills list --json" in text

    def test_skills_info_help_has_examples(self) -> None:
        text = self._render(show_skills_info_help)
        assert "Examples:" in text
        assert "blackcat skills info web-research" in text
        assert "blackcat skills info my-skill --project" in text

    def test_agents_help_has_examples(self) -> None:
        text = self._render(show_agents_help)
        assert "Examples:" in text
        assert "blackcat agents list" in text
        assert "blackcat agents reset --agent coder" in text

    def test_update_help_has_examples(self) -> None:
        text = self._render(show_update_help)
        assert "Examples:" in text
        assert "blackcat update" in text
        assert "blackcat update --json" in text
...
class TestResetDryRun:
    ...
    def test_dry_run_text_no_mutation(self, tmp_path: Path) -> None:
        ...
        test_console = Console(file=buf, highlight=False, width=200)
        with (
            patch("blackcat_cli.agent.settings") as mock_settings,
            patch("blackcat_cli.agent.console", test_console),
            patch(
                "blackcat_cli.agent.get_default_coding_instructions",
                return_value="default",
            ),
        ):
            mock_settings.user_blackcat_dir = tmp_path
    ...
    def test_dry_run_json(self, tmp_path: Path) -> None:
        ...
        with (
            patch("blackcat_cli.agent.settings") as mock_settings,
            patch("blackcat_cli.agent.console"),
            patch(
                "blackcat_cli.agent.get_default_coding_instructions",
                return_value="default",
            ),
...
class TestThreadsDeleteDryRun:
    """Tests for blackcat threads delete --dry-run."""
    ...
    def test_dry_run_text_existing(self) -> None:
        ...
        with (
            ...
            patch("blackcat_cli.config.console", test_console),
        ):
...
class TestAgentsSubcommand:
    ...
    def test_agents_list_parses(self) -> None:
        """Running `blackcat agents list` should parse correctly."""
        with patch.object(sys, "argv", ["blackcat", "agents", "list"]):
            args = parse_args()
        assert args.command == "agents"
        assert args.agents_command == "list"

    def test_agents_ls_alias(self) -> None:
        """Running `blackcat agents ls` should parse as list."""
        with patch.object(sys, "argv", ["blackcat", "agents", "ls"]):
            args = parse_args()
        assert args.command == "agents"
        assert args.agents_command == "ls"

    def test_agents_reset_parses(self) -> None:
        """Running `blackcat agents reset --agent coder` should parse."""
        with patch.object(
            sys, "argv", ["blackcat", "agents", "reset", "--agent", "coder"]
        ):
            args = parse_args()
        assert args.command == "agents"
    ...
    def test_agents_reset_with_target(self) -> None:
        """Running `blackcat agents reset --agent coder --target researcher`."""
        with patch.object(
            sys,
            "argv",
            [
    ...
    def test_agents_reset_dry_run(self) -> None:
        """Running `blackcat agents reset --agent coder --dry-run`."""
        with patch.object(
            sys,
            "argv",
            ["blackcat", "agents", "reset", "--agent", "coder", "--dry-run"],
        ):
            args = parse_args()
        assert args.dry_run is True

    def test_agents_help_exits_clean(self) -> None:
        """Running `blackcat agents -h` should show help and exit 0."""
        buf = io.StringIO()
        test_console = Console(file=buf, highlight=False, width=120)
        with (
            patch.object(sys, "argv", ["blackcat", "agents", "-h"]),
    ...
    def test_top_level_list_rejected(self) -> None:
        """Top-level `blackcat list` should error — use `agents list` instead."""
        with (
            patch.object(sys, "argv", ["blackcat", "list"]),
            pytest.raises(SystemExit) as exc_info,
        ):
            parse_args()
...
class TestUpdateSubcommand:
    ...
    def test_update_parses(self) -> None:
        """Running `blackcat update` should parse as command='update'."""
        with patch.object(sys, "argv", ["blackcat", "update"]):
            args = parse_args()
        assert args.command == "update"

    def test_update_help_exits_clean(self) -> None:
        """Running `blackcat update -h` should show help and exit 0."""
        buf = io.StringIO()
        test_console = Console(file=buf, highlight=False, width=120)
        with (
            patch.object(sys, "argv", ["blackcat", "update", "-h"]),
            patch("blackcat_cli.ui.console", test_console),
            pytest.raises(SystemExit) as exc_info,
        ):
            parse_args()
...
class TestSkillsCreateIdempotency:
    ...
    def test_already_exists_text_no_error(self, tmp_path: Path) -> None:
        ...
        from blackcat_cli.skills.commands import _create
        ...
        with (
            patch("blackcat_cli.config.Settings") as settings_cls,
            patch("blackcat_cli.config.console", test_console),
        ):
...
class TestStdinFlag:
    ...
    def test_stdin_flag_parses(self) -> None:
        ...
        with patch.object(sys, "argv", ["blackcat", "--stdin"]):
            args = parse_args()
    ...
    def test_stdin_with_tty_errors(self) -> None:
        ...
        from blackcat_cli.main import apply_stdin_pipe
...
class TestErrorMessageHints:
    ...
    def test_quiet_without_n_has_hint(self) -> None:
        ...
        stderr_buf = io.StringIO()
        with (
            patch.object(sys, "argv", ["blackcat", "--quiet"]),
            patch("blackcat_cli.main.check_cli_dependencies"),
            patch("blackcat_cli.main.apply_stdin_pipe"),
            patch("sys.stderr", stderr_buf),
    ...
    def test_reset_source_not_found_has_hint(self, tmp_path: Path) -> None:
        ...
        output = buf.getvalue()
        assert "blackcat agents list" in output
