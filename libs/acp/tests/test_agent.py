...
from protohello.agents.middleware import HumanInTheLoopMiddleware
from protohello.tools import ToolRuntime
from protohello_core.messages import AIMessage, AIMessageChunk, ToolMessage
...
async def test_acp_agent_prompt_streams_text() -> None:
    ...
    graph = create_black_cat(model=model, checkpointer=MemorySaver())
...
async def test_acp_black_cat_hitl_interrupt_on_edit_file_requests_permission() -> None:
    model = GenericFakeChatModel(
        messages=iter(
            [
                AIMessage(
...
async def test_acp_agent_nested_agent_tool_call_returns_final_text() -> None:
    ...
    subagent = create_black_cat(model=subagent_model, checkpointer=MemorySaver())
    ...
    outer = create_black_cat(model=outer_model, tools=[call_agent1], checkpointer=MemorySaver())
...
async def test_acp_agent_with_prebuilt_protohello_agent_end_to_end() -> None:
    model = GenericFakeChatModel(
        messages=iter([AIMessage(content="Hello!")]), stream_delimiter=r"(\s)"
    )
...
async def test_acp_protohello_create_agent_nested_agent_tool_call_messages() -> None:
    @tool
...
async def test_set_session_mode_resets_agent_with_new_mode() -> None:
    ...
    def agent_factory(context: AgentSessionContext) -> CompiledStateGraph:
        ...
        agent = create_black_cat(model=model, checkpointer=MemorySaver())
...
async def test_reset_agent_with_compiled_state_graph() -> None:
    ...
    compiled_graph = create_black_cat(model=model, checkpointer=MemorySaver())
...
async def test_reset_agent_preserves_session_cwd() -> None:
    ...
    def agent_factory(context: AgentSessionContext) -> CompiledStateGraph:
        ...
        return create_black_cat(model=model, checkpointer=MemorySaver())
