"""ACP server implementation for Black Cat."""
...
from blackcat.backends import CompositeBackend, FilesystemBackend, StateBackend
...
if TYPE_CHECKING:
    ...
    from blackcat.graph import Checkpointer
    from protohello_core.runnables import RunnableConfig

from blackcat_acp.utils import (
...
class AgentServerACP(ACPAgent):
    """ACP agent server that bridges Black Cat with the Agent Client Protocol."""
    ...
    async def _log_text(self, session_id: str, text: str) -> None:
        ...
        await self._conn.session_update(session_id=session_id, update=update, source="BlackCat")
    ...
    async def prompt(  # noqa: C901, PLR0912, PLR0915  # Complex streaming protocol handler with many branches
        ...
        **kwargs: Any,  # noqa: ARG002  # ACP protocol interface parameter
        ...
        # Convert ACP content blocks to ProtoHello multimodal content format
        content_blocks = []
            ...
            async for stream_chunk in agent.astream(
                                ...
                                if not isinstance(interrupt_value, dict):
                                    raise RequestError(
                                        ...
                                        (
                                            ...
                                            "Fix: use ProtoHello HumanInTheLoopMiddleware-style "
                                            "interrupts (action_requests/review_configs).\n"
                                            "Docs: https://docs.protohello.com/oss/python/protohello/"
                                            "human-in-the-loop\n\n"
                ...
                # Check for tool results (ToolMessage responses)
                    ...
                    if (
                        ...
                        await self._conn.session_update(
                            session_id=session_id, update=update, source="BlackCat"
                        )
        ...
        if current_state.next and current_state.interrupts:
            ...
            for interrupt in current_state.interrupts:
                ...
                if isinstance(interrupt_value, dict):
                    # Black Cat wraps tool calls in action_requests
