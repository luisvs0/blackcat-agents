"""Utility functions for converting ACP content blocks to ProtoHello formats."""
...
def convert_text_block_to_content_blocks(block: TextContentBlock) -> list[dict[str, str]]:
    """Convert an ACP text block to ProtoHello content blocks."""
    return [{"type": "text", "text": block.text}]


def convert_image_block_to_content_blocks(block: ImageContentBlock) -> list[dict[str, object]]:
    """Convert an ACP image block to ProtoHello content blocks."""
...
def convert_audio_block_to_content_blocks(block: AudioContentBlock) -> list[dict[str, str]]:
    """Convert an ACP audio block to ProtoHello content blocks.
...
def convert_resource_block_to_content_blocks(
    ...
    *,
    ...
    """Convert an ACP resource block to ProtoHello content blocks."""
    file_prefix = "file://"
...
def convert_embedded_resource_block_to_content_blocks(
    ...
    """Convert an ACP embedded resource block to ProtoHello content blocks.
...
Ported from ``blackcat_cli.config.DANGEROUS_SHELL_PATTERNS``.  Used by
