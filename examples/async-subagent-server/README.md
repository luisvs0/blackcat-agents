...
A self-hosted [Agent Protocol](https://github.com/protohello-ai/agent-protocol) server that exposes a BlackCat researcher as an async subagent. Use this as a starting point for hosting your own agent on any infrastructure and connecting it to a BlackCat supervisor.
...
These are the Agent Protocol endpoints the BlackCat async subagent middleware calls (via the LangGraph SDK):
...
Replace the `create_black_cat` call in `server.py` with your own agent. The Agent Protocol layer stays the same regardless of what the agent does.

```python
_agent = create_black_cat(
