# Text-to-SQL Black Cat

A natural language to SQL query agent powered by ProtoHello's **Black Cat** framework.  This is an advanced version of a text-to-SQL agent with planning, filesystem, and subagent capabilities.

## What is Black Cat?

Black Cat is a sophisticated agent framework built on LangGraph that provides:
...
- (Optional) LangSmith API key for tracing ([sign up here](https://smith.protohello.com/))

### Installation

1. Clone the blackcat repository and navigate to this example:

```bash
git clone https://github.com/protohello-ai/blackcat.git
cd blackcat/examples/text-to-sql-agent
```
...
```
PROTOHELLO_TRACING_V2=true
LANGSMITH_ENDPOINT=https://api.smith.protohello.com
PROTOHELLO_API_KEY=your_langsmith_api_key_here
PROTOHELLO_PROJECT=text2sql-blackcat
```
...
```python
from agent import create_sql_black_cat

# Create the agent
agent = create_sql_black_cat()
...
Black Cat uses **progressive disclosure** with memory files and skills:
...
## Black Cat Output Example

The Black Cat shows its reasoning process:
...
```
text-to-sql-agent/
├── agent.py                      # Core Black Cat implementation with CLI
...
- blackcat >= 0.3.5
- protohello >= 1.2.3
- protohello-anthropic >= 1.3.1
- protohello-community >= 0.3.0
- langgraph >= 1.0.6
- sqlalchemy >= 2.0.0
- python-dotenv >= 1.0.0
- tavily-python >= 0.5.0
...
1. Sign up for a free account at [LangSmith](https://smith.protohello.com/)
...
![Black Cat LangSmith Trace Example](text-to-sql-langsmith-trace.png)
...
View your traces at: <https://smith.protohello.com/>

## Resources

- [Black Cat Documentation](https://docs.protohello.com/oss/python/blackcat/overview)
- [ProtoHello](https://www.protohello.com/)
