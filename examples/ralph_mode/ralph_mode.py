"""Ralph Mode - Autonomous looping for Black Cat.
...
Each iteration delegates to `run_non_interactive` from `blackcat-cli`,
...
async def ralph(
    ...
    *,
    ...
    Each iteration invokes the Black Cat CLI's `run_non_interactive` with a
    ...
    Args:
        ...
        model_name: Model spec in `provider:model` format (e.g.
            ...
            When `None`, `blackcat-cli` resolves a default via its config
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Ralph Mode - Autonomous looping for Black Cat",
...
Examples:
  ...
  python ralph_mode.py "Build an app" --model-params '{"temperature": 0.5}'
    ...
    if args.shell_allow_list:
        from blackcat_cli.config import parse_shell_allow_list, settings
