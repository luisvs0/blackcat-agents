...
This document describes the release process for packages in the Black Cat monorepo using [release-please](https://github.com/googleapis/release-please).

## Managed Packages

| Package | Path | Component | PyPI |
| ------- | ---- | --------- | ---- |
| `blackcat` (SDK) | `libs/blackcat` | `blackcat` | [`blackcat`](https://pypi.org/project/blackcat/) |
| `blackcat-cli` | `libs/cli` | `blackcat-cli` | [`blackcat-cli`](https://pypi.org/project/blackcat-cli/) |
| `blackcat-acp` | `libs/acp` | `blackcat-acp` | [`blackcat-acp`](https://pypi.org/project/blackcat-acp/) |
| `protohello-daytona` | `libs/partners/daytona` | `protohello-daytona` | [`protohello-daytona`](https://pypi.org/project/protohello-daytona/) |
| `protohello-modal` | `libs/partners/modal` | `protohello-modal` | [`protohello-modal`](https://pypi.org/project/protohello-modal/) |
| `protohello-runloop` | `libs/partners/runloop` | `protohello-runloop` | [`protohello-runloop`](https://pypi.org/project/protohello-runloop/) |
| `protohello-quickjs` | `libs/partners/quickjs` | `protohello-quickjs` | [`protohello-quickjs`](https://pypi.org/project/protohello-quickjs/) |
| `protohello-repl` | `libs/repl` | `protohello-repl` | [`protohello-repl`](https://pypi.org/project/protohello-repl/) |
...
2. Creates/updates a release PR [(example)](https://github.com/protohello-ai/blackcat/pull/1956) with automated changelog and version bumps
...
All commits must follow [Conventional Commits](https://www.conventionalcommits.org/) format with types and scopes defined in [`.github/workflows/pr_lint.yml`](https://github.com/protohello-ai/blackcat/blob/main/.github/workflows/pr_lint.yml). **Scope is required** — PRs without a scope will fail the title lint check.
...
The [release-please workflow (`.github/workflows/release-please.yml`)](https://github.com/protohello-ai/blackcat/blob/main/.github/workflows/release-please.yml) detects releases by checking two conditions on the merge commit:
...
The [release workflow (`.github/workflows/release.yml`)](https://github.com/protohello-ai/blackcat/blob/main/.github/workflows/release.yml) runs when a release PR is merged:
...
6. **Mark Release** - Creates a published GitHub release with the built artifacts; updates PR labels. For the SDK (`libs/blackcat`), we set it as the repository's `latest` (unless it's a pre-release).
...
1. **Create a branch from `main`:**
   ...
   Replace `<PACKAGE>` with the PyPI name (e.g., `blackcat-cli`) and `<VERSION>` with the alpha version using hyphens instead of periods (e.g., `0-0-35a1`).
...
2. **Bump the version** in both files to a [PEP 440 pre-release](https://peps.python.org/pep-0440/#pre-releases) (e.g., `0.0.35a1`):
   ...
   - `libs/cli/blackcat_cli/_version.py` — `__version__ = "0.0.35a1"`
...
After validating the alpha, merge the pending release PR (e.g., `release(blackcat-cli): 0.0.35`) as normal from `main` — release-please handles the GA version, changelog, and tag. No extra steps needed.
...
- The release commit (e.g., `release(blackcat): 0.5.1` or `release(blackcat-cli): 0.0.35`)
...
SDK version (libs/blackcat/pyproject.toml): 0.4.2
CLI SDK pin (libs/cli/pyproject.toml): 0.4.1
```

This means the CLI's pinned `blackcat` dependency in `libs/cli/pyproject.toml` doesn't match the current SDK version. This can happen when the SDK is released independently and the CLI's pin isn't updated before the CLI release PR is merged.
...
1. **Hotfix the pin on `main`:**
   ...
   # e.g., change blackcat==0.4.1 to blackcat==0.4.2
   cd libs/cli && uv lock
...
2. **Manually trigger the release** (the push to `main` won't re-trigger the release because the commit doesn't modify `libs/cli/CHANGELOG.md`):
   - Go to **Actions** > `⚠️ Manual Package Release`
   - Click **Run workflow**
   - Select `main` branch and `blackcat-cli` package
...
gh api -X PATCH repos/protohello-ai/blackcat/releases/$(gh api repos/protohello-ai/blackcat/releases --jq '.[] | select(.tag_name == "<PACKAGE>==<VERSION>") | .id') \
